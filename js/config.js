...
const STRATEGY_REGISTRY_SAMPLE_CANDIDATES = {
    longEntry: [
...
const strategyRolePresets = {
    entry: STRATEGY_REGISTRY_SAMPLE_CANDIDATES.longEntry.map((item) => item.strategyId),
    exit: STRATEGY_REGISTRY_SAMPLE_CANDIDATES.longExit.map((item) => item.strategyId),
    shortEntry: STRATEGY_REGISTRY_SAMPLE_CANDIDATES.shortEntry.map((item) => item.strategyId),
    shortExit: STRATEGY_REGISTRY_SAMPLE_CANDIDATES.shortExit.map((item) => item.strategyId),
};
...
if (typeof window !== 'undefined') {
    ...
    window.strategyRegistrySampleCandidates = STRATEGY_REGISTRY_SAMPLE_CANDIDATES;
    window.strategyRolePresets = strategyRolePresets;
    window.LazyStrategyCatalog = window.LazyStrategyCatalog || {};
    window.LazyStrategyCatalog.rolePresets = window.LazyStrategyCatalog.rolePresets || strategyRolePresets;
}
