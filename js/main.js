...
function applyMarketPreset(market) {
    ...
    if (marketHint) {
        let hintMessage = '';
        if (market === 'US') {
            hintMessage = '美股回測目前採用原始股價，已自動停用除權息還原與拆分選項。';
        } else if (market === 'INDEX') {
            hintMessage = '指數資料由 Yahoo Finance 提供，已自動停用還原價與拆分選項。';
        }
        marketHint.textContent = hintMessage;
        marketHint.classList.toggle('hidden', !hintMessage);
    }
}
