...
function initLoadingMascotSanitiser() {
    const VERSION = 'LB-MASCOT-DIRECT-20251208A';
    const FALLBACK_SRC = 'assets/mascot/hachiware-dance-fallback.svg';

    const image = document.getElementById('loadingGif');
    if (!image) {
        return;
    }

    if (image.dataset.lbMascotSanitiser === VERSION) {
        return;
    }

    const primarySrc = image.getAttribute('data-primary-src')?.trim() || image.getAttribute('src')?.trim() || '';
    const fallbackSrc = image.getAttribute('data-fallback-src')?.trim() || FALLBACK_SRC;

    image.classList.add('loading-mascot-image');
    ...
    const updateSourceDataset = () => {
        const currentSrc = image.getAttribute('src')?.trim();
        if (!currentSrc) {
            delete image.dataset.lbMascotSource;
            return;
        }
        if (fallbackSrc && currentSrc === fallbackSrc) {
            image.dataset.lbMascotSource = 'fallback';
        } else if (primarySrc && currentSrc === primarySrc) {
            image.dataset.lbMascotSource = 'primary';
        } else {
            image.dataset.lbMascotSource = `custom:${currentSrc}`;
        }
    };

    const handleError = () => {
        if (!fallbackApplied && fallbackSrc && primarySrc !== fallbackSrc) {
            fallbackApplied = true;
            image.src = fallbackSrc;
            return;
        }
        fallbackApplied = true;
        image.dataset.lbMascotSource = 'unavailable';
        image.removeEventListener('error', handleError);
    };

    image.addEventListener('load', updateSourceDataset);
    image.addEventListener('error', handleError);

    if (primarySrc && image.getAttribute('src') !== primarySrc) {
        image.src = primarySrc;
    } else {
        updateSourceDataset();
    }

    image.dataset.lbMascotSanitiser = VERSION;
}
