...
function cancelLoadingMascotRotationTimer() {
    if (loadingMascotState.rotationTimerId) {
        const win = getMascotWindow();
        if (win) {
            win.clearTimeout(loadingMascotState.rotationTimerId);
        } else {
            clearTimeout(loadingMascotState.rotationTimerId);
        }
        loadingMascotState.rotationTimerId = null;
    }
}

function scheduleLoadingMascotRotation() {
    cancelLoadingMascotRotationTimer();
    if (!loadingMascotState.autoRotate) return;
    const win = getMascotWindow();
    if (!win) return;
    loadingMascotState.rotationTimerId = win.setTimeout(() => {
        loadingMascotState.rotationTimerId = null;
        if (!loadingMascotState.autoRotate) return;
...
function showMascotHourglassFallback(container) {
    ...
    const poolSize = Number.parseInt(container.dataset.lbMascotPoolSize || '0', 10);
    if (loadingMascotState.autoRotate && Number.isFinite(poolSize) && poolSize > 0) {
        scheduleLoadingMascotRotation();
    } else {
        cancelLoadingMascotRotationTimer();
    }
}
...
function refreshLoadingMascotImage(options = {}) {
    ...
    const attemptNext = () => {
        ...
        const finalize = () => {
            ...
            if (loadingMascotState.autoRotate) {
                scheduleLoadingMascotRotation();
            }
        };
...
function hideLoading() {
    ...
    loadingMascotState.autoRotate = false;
    cancelLoadingMascotRotationTimer();
