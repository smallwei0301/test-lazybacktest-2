...
function getTesterSourceConfigs(market, adjusted, splitEnabled) {
    ...
    if (market === 'TPEX') {
        return [
            { id: 'fugle', label: 'Fugle 主來源', description: 'Fugle 上櫃日線資料' },
            { id: 'finmind', label: 'FinMind 備援', description: 'Fugle 失效時啟用' },
            { id: 'yahoo', label: 'Yahoo 備援', description: 'Fugle/FinMind 皆失效時啟用' },
        ];
    }
    return [
        { id: 'fugle', label: 'Fugle 主來源', description: 'Fugle 上市日線資料' },
        { id: 'twse', label: 'TWSE 備援', description: '官方日成交資訊' },
        { id: 'finmind', label: 'FinMind 備援', description: 'Fugle/TWSE 失效時啟用' },
    ];
}
...
function refreshDataSourceTester() {
    ...
    if (missingInputs) {
        ...
        messageLines.push('Fugle 為主來源，FinMind 與 Yahoo 為備援來源。請確認 FUGLE_API_TOKEN 與 FINMIND_TOKEN 設定並依序測試。');
    } else {
        messageLines.push('Fugle 為主來源，TWSE 與 FinMind 為備援來源。建議三種來源都測試一次並確認 FUGLE_API_TOKEN 設定。');
    }
