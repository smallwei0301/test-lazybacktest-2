...
const STRATEGY_ROLE_LEGACY_ID_MAP = {
    exit: {
...
function resolveStrategyComparisonAliases(strategyId, roleType) {
    ...
    if (roleType === 'exit' || roleType === 'shortEntry' || roleType === 'shortExit') {
        const normalised = normaliseStrategyIdForRole(roleType, strategyId);
        if (normalised && normalised !== strategyId && typeof strategyDescriptions === 'object' && strategyDescriptions?.[normalised]) {
...
function getStrategyParams(type) {
    ...
    const normalisedKey = normaliseStrategyIdForRole(type, rawKey);
    const lookupKey = (strategyDescriptions && strategyDescriptions[normalisedKey]) ? normalisedKey : rawKey;
    const cfg = strategyDescriptions?.[lookupKey];
    if (!cfg?.defaultParams) {
        return {};
    }

    const params = {};
    for (const paramName in cfg.defaultParams) {
        if (!Object.prototype.hasOwnProperty.call(cfg.defaultParams, paramName)) {
            continue;
        }
        let idSuffix = paramName.charAt(0).toUpperCase() + paramName.slice(1);
        if (lookupKey === 'k_d_cross' && paramName === 'thresholdX') idSuffix = 'KdThresholdX';
        else if (lookupKey === 'k_d_cross_exit' && paramName === 'thresholdY') idSuffix = 'KdThresholdY';
        else if (lookupKey === 'turtle_stop_loss' && paramName === 'stopLossPeriod') idSuffix = 'StopLossPeriod';
        else if ((lookupKey === 'macd_cross' || lookupKey === 'macd_cross_exit') && paramName === 'signalPeriod') idSuffix = 'SignalPeriod';
        else if (lookupKey === 'short_k_d_cross' && paramName === 'thresholdY') idSuffix = 'ShortKdThresholdY';
        else if (lookupKey === 'cover_k_d_cross' && paramName === 'thresholdX') idSuffix = 'CoverKdThresholdX';
        else if (lookupKey === 'short_macd_cross' && paramName === 'signalPeriod') idSuffix = 'ShortSignalPeriod';
        else if (lookupKey === 'cover_macd_cross' && paramName === 'signalPeriod') idSuffix = 'CoverSignalPeriod';
        else if (lookupKey === 'short_turtle_stop_loss' && paramName === 'stopLossPeriod') idSuffix = 'ShortStopLossPeriod';
        else if (lookupKey === 'cover_turtle_breakout' && paramName === 'breakoutPeriod') idSuffix = 'CoverBreakoutPeriod';
        else if (lookupKey === 'cover_trailing_stop' && paramName === 'percentage') idSuffix = 'CoverTrailingStopPercentage';

        const inputId = `${type}${idSuffix}`;
        const input = document.getElementById(inputId);
        if (input) {
            params[paramName] = (input.type === 'number')
                ? (parseFloat(input.value) || cfg.defaultParams[paramName])
                : input.value;
        } else {
            params[paramName] = cfg.defaultParams[paramName];
        }
    }
...
function getBacktestParams() {
    ...
    const entryStrategy = normaliseStrategyIdForRole('entry', document.getElementById('entryStrategy')?.value);
    const exitStrategy = normaliseStrategyIdForRole('exit', document.getElementById('exitStrategy')?.value);
    ...
    let shortExitParams = {};
    if (enableShorting) {
        shortEntryStrategy = normaliseStrategyIdForRole('shortEntry', document.getElementById('shortEntryStrategy')?.value);
        shortExitStrategy = normaliseStrategyIdForRole('shortExit', document.getElementById('shortExitStrategy')?.value);
