...
const PROGRESS_MASCOT_POOL = Object.freeze([
...
let loadingMascotController = null;

function initLoadingMascotSanitiser() {
    const VERSION = 'LB-PROGRESS-MASCOT-20250626A';
    const container = document.getElementById('loadingGif');

    if (!container) {
        loadingMascotController = null;
        return null;
    }

    if (loadingMascotController && loadingMascotController.version === VERSION) {
        return loadingMascotController;
    }

    const sanitizedPool = Array.from(
        new Set(
            PROGRESS_MASCOT_POOL.map((src) => (typeof src === 'string' ? src.trim() : ''))
                .filter((src) => {
                    if (!src) return false;
                    try {
                        ...
                        console.warn('[Mascot] 忽略無效的圖片來源：', src);
                        return false;
                    }
                })
        )
    ...
    const applyFallback = () => {
        ...
        container.dataset.lbMascotSource = 'fallback-hourglass';
        return false;
    };
    ...
    const applySource = (src) => {
        ...
        const handleError = () => {
            ...
            console.warn('[Mascot] 進度吉祥物載入失敗，改用後備圖示：', src);
            applyFallback();
        };
        ...
        container.classList.remove('loading-mascot-fallback');
        container.dataset.lbMascotSource = `pool:${src}`;
        return true;
    };
    ...
    const applyRandom = () => {
        ...
        const index = Math.floor(Math.random() * sanitizedPool.length);
        const selected = sanitizedPool[index];
        return applySource(selected);
    };
    ...
    container.dataset.lbMascotManager = VERSION;
    loadingMascotController = controller;
    applyRandom();
...
function showLoading(m = "處理中...") {
    ...
    const mascotController = initLoadingMascotSanitiser();
    if (mascotController && typeof mascotController.applyRandom === 'function') {
        mascotController.applyRandom();
    }
