...
// Patch Tag: LB-AI-TF-LAZYLOAD-20260729A — 延後載入 TensorFlow.js，僅在 AI 訓練/推論時啟動。
...
let tfScriptsLoaded = typeof tf !== 'undefined';
let tfWasmBackendAttempted = false;
let tfBackendReadyPromise = null;

function loadTfScriptsIfNeeded() {
  if (!tfScriptsLoaded || typeof tf === 'undefined') {
    importScripts(`https://cdn.jsdelivr.net/npm/@tensorflow/tfjs@${TFJS_VERSION}/dist/tf.min.js`);
    tfScriptsLoaded = typeof tf !== 'undefined';
  }
}

function loadTfWasmBackendIfNeeded() {
  if (tfWasmBackendAttempted) {
    return;
  }
  tfWasmBackendAttempted = true;
  try {
...
function ensureTfBackendPromise() {
  ...
  tfBackendReadyPromise = pendingPromise.catch((error) => {
...
async function ensureTFReady() {
  try {
    return await ensureTfBackendPromise();
  } catch (error) {
