...
function runStrategy(data, params, options = {}) {
  ...
  const resolveStrategy =
    typeof pluginRegistry?.getStrategyById === 'function'
      ? (id) => pluginRegistry.getStrategyById(id)
  ...
  function callStrategyPlugin(strategyId, role, index, baseParams, extras) {
    ...
    const plugin = resolveStrategy(strategyId);
...
function resolveStrategyName(strategyKey) {
  ...
  if (typeof self !== "undefined") {
    const registry = self.StrategyPluginRegistry;
    if (registry) {
      const lookup =
        typeof registry.getStrategyById === "function"
          ? registry.getStrategyById.bind(registry)
          : typeof registry.get === "function"
          ? registry.get.bind(registry)
          : null;
      if (lookup) {
        const plugin = lookup(strategyKey);
        const label = plugin && plugin.meta && plugin.meta.label;
