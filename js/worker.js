...
function calculateAllIndicators(data, params) {
  ...
  try {
    ...
    const volumeAvgEntryPeriodRaw = Number(ep?.period);
    const volumeAvgEntryPeriod = Number.isFinite(volumeAvgEntryPeriodRaw)
      ? Math.max(1, Math.round(volumeAvgEntryPeriodRaw))
      : 20;
    indic.volumeAvgEntry = maCalculator(volumes, volumeAvgEntryPeriod);

    const volumeAvgExitPeriodRaw = Number(xp?.period);
    const volumeAvgExitPeriod = Number.isFinite(volumeAvgExitPeriodRaw)
      ? Math.max(1, Math.round(volumeAvgExitPeriodRaw))
      : volumeAvgEntryPeriod;
    indic.volumeAvgExit =
      volumeAvgExitPeriod === volumeAvgEntryPeriod
        ? indic.volumeAvgEntry
        : maCalculator(volumes, volumeAvgExitPeriod);

    const volumeAvgShortEntryPeriodRaw = Number(sep?.period);
    const volumeAvgShortEntryPeriod = Number.isFinite(volumeAvgShortEntryPeriodRaw)
      ? Math.max(1, Math.round(volumeAvgShortEntryPeriodRaw))
      : volumeAvgEntryPeriod;
    indic.volumeAvgShortEntry =
      volumeAvgShortEntryPeriod === volumeAvgEntryPeriod
        ? indic.volumeAvgEntry
        : maCalculator(volumes, volumeAvgShortEntryPeriod);

    const volumeAvgCoverPeriodRaw = Number(sxp?.period);
    const volumeAvgCoverPeriod = Number.isFinite(volumeAvgCoverPeriodRaw)
      ? Math.max(1, Math.round(volumeAvgCoverPeriodRaw))
      : volumeAvgExitPeriod;
    indic.volumeAvgCover =
      volumeAvgCoverPeriod === volumeAvgExitPeriod
        ? indic.volumeAvgExit
        : maCalculator(volumes, volumeAvgCoverPeriod);
          ...
          case "volume_spike_exit":
            {
              ...
              const avgSeries = indicators.volumeAvgExit || [];
              const avgValue = avgSeries[i];
              ...
              sellSignal = check(avgValue) && check(curV) && curV > avgValue * multiplier;
              if (sellSignal)
                exitIndicatorValues = {
    ...
    if (shortPos === 0 && filledEntryStages < entryStagePercents.length) {
      ...
      switch (entryStrategy) {
        ...
        case "volume_spike": {
          ...
          const avgSeries = indicators.volumeAvgEntry || [];
          const avgValue = avgSeries[i];
          const multiplierRaw = Number(entryParams?.multiplier);
          const multiplier = Number.isFinite(multiplierRaw) && multiplierRaw > 0 ? multiplierRaw : 2;
          buySignal = check(avgValue) && check(curV) && curV > avgValue * multiplier;
          if (buySignal)
            entryIndicatorValues = {
              成交量: [volumes[i - 1] ?? null, curV, volumes[i + 1] ?? null],
              均量: [
