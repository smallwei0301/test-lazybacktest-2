...
function runStrategy(data, params, options = {}) {
  ...
  if (startIdx >= n || n < 2) {
    ...
    const buildAggregatedLongEntry = () => {
      // Patch Tag: LB-TRADE-ENTRY-INDICATOR-20240910A
      if (currentLongEntryBreakdown.length === 0) return null;
      const cloneIndicatorValueObject = (source) => {
      ...
      const stageSnapshots = currentLongEntryBreakdown.map((info) => {
        ...
        const stageClone = { ...info };
        if (info.indicatorValues && typeof info.indicatorValues === "object") {
          stageClone.indicatorValues = cloneIndicatorValueObject(
            info.indicatorValues,
          );
        }
        if (info.kdValues && typeof info.kdValues === "object") {
          stageClone.kdValues = { ...info.kdValues };
        }
        if (info.macdValues && typeof info.macdValues === "object") {
          stageClone.macdValues = { ...info.macdValues };
        }
        return stageClone;
      });
      const primaryStage = currentLongEntryBreakdown[0] || null;
      const fallbackStage =
      ...
      const attachStageDiagnostics = (stage) => {
        if (!stage || typeof stage !== "object") {
          return;
        }
        if (stage.indicatorValues && typeof stage.indicatorValues === "object") {
          aggregatedEntry.indicatorValues = cloneIndicatorValueObject(
            stage.indicatorValues,
          );
        }
        if (stage.kdValues && typeof stage.kdValues === "object") {
          aggregatedEntry.kdValues = { ...stage.kdValues };
        }
        if (stage.macdValues && typeof stage.macdValues === "object") {
          aggregatedEntry.macdValues = { ...stage.macdValues };
        }
      };
      attachStageDiagnostics(primaryStage);
      if (
        !aggregatedEntry.indicatorValues &&
        !aggregatedEntry.kdValues &&
        !aggregatedEntry.macdValues
      ) {
        attachStageDiagnostics(fallbackStage);
      }
...
// --- 參數優化邏輯 ---
// Patch Tag: LB-OPT-DSL-SYNC-20240910A
const SINGLE_PARAMETER_OPTIMIZER_VERSION = "LB-SINGLE-OPT-20251115A";

function cloneStrategyDslNode(node) {
  if (!node || typeof node !== "object") {
    return node ?? null;
  }
  if (Array.isArray(node)) {
    return node.map((child) => cloneStrategyDslNode(child));
  }
  const clone = {};
  Object.keys(node).forEach((key) => {
    clone[key] = cloneStrategyDslNode(node[key]);
  });
  return clone;
}

function cloneStrategyDsl(root) {
  if (!root || typeof root !== "object") {
    return null;
  }
  const clone = {};
  Object.keys(root).forEach((key) => {
    clone[key] = cloneStrategyDslNode(root[key]);
  });
  return clone;
}

function syncStrategyDslParamValue(dslRoot, roleKey, strategyId, paramName, value) {
  if (!dslRoot || typeof dslRoot !== "object") {
    return;
  }
  const roleToNodeKey = {
    entry: "longEntry",
  ...
  const nodeKey = roleToNodeKey[roleKey];
  if (!nodeKey) {
    return;
  }
  const rootNode = dslRoot[nodeKey];
  if (!rootNode || typeof rootNode !== "object") {
    return;
  }
  const targetId = typeof strategyId === "string" ? strategyId : null;
  const visit = (node) => {
    if (!node || typeof node !== "object") {
      return;
    }
    ...
    const matches =
      nodeType === "plugin"
        ...
        : targetId && nodeId === targetId;
    if (matches && node.params && typeof node.params === "object") {
    ...
    if (node.otherwise && typeof node.otherwise === "object") {
...
function createOptimizationParamTemplate(baseParams = {}) {
  ...
  if (baseParams && typeof baseParams === "object") {
    Object.keys(baseParams).forEach((key) => {
      ...
      if (key === "strategyDsl") {
        template.base[key] = cloneStrategyDsl(baseParams[key]);
        return;
      }
...
function instantiateOptimizationParams(template) {
  ...
  if (template.base && template.base.strategyDsl) {
    params.strategyDsl = cloneStrategyDsl(template.base.strategyDsl);
  }
  return params;
}
...
async function runOptimization(
  ...
  for (const curVal of sweepValues) {
    ...
    if (optimizeTargetStrategy === "risk") {
      ...
      const strategyIdForRole =
        optimizeTargetStrategy === "entry"
          ? testParams.entryStrategy
          : optimizeTargetStrategy === "exit"
            ...
            : optimizeTargetStrategy === "shortEntry"
              ...
              : optimizeTargetStrategy === "shortExit"
                ...
                : null;
      if (!testParams.strategyDsl && baseParams?.strategyDsl) {
        testParams.strategyDsl = cloneStrategyDsl(baseParams.strategyDsl);
      }
      if (testParams.strategyDsl) {
        syncStrategyDslParamValue(
