...
function calculateAllIndicators(data, params) {
  ...
  try {
    ...
    const resolveVolumeAverage = (rawPeriod) => {
      const period = Number(rawPeriod) || 0;
      if (!Number.isFinite(period) || period <= 0) {
        return new Array(volumes.length).fill(null);
      }
      if (!volumePeriodCache.has(period)) {
        volumePeriodCache.set(period, maCalculator(volumes, period));
      }
      return volumePeriodCache.get(period);
    };
    ...
    indic.volumeAvgExit = resolveVolumeAverage(volumeExitPeriod);
    if (enableShorting) {
      const volumeShortEntryPeriod = sep?.period || volumeEntryPeriod;
      indic.volumeAvgShortEntry = resolveVolumeAverage(volumeShortEntryPeriod);
      const volumeCoverPeriod = sxp?.period || volumeExitPeriod;
      indic.volumeAvgCover = resolveVolumeAverage(volumeCoverPeriod);
    }
          ...
          case "volume_spike":
            {
              ...
              const avgSeries =
                indicators.volumeAvgExit || indicators.volumeAvgEntry || [];
              const avgVolume = avgSeries[i];
