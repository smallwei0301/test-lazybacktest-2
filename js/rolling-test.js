...
(function() {
    ...
    function clearRollingReport() {
        ...
        if (gradeLabel) {
            ...
            gradeLabel.style.color = 'var(--foreground)';
        }
    ...
    function renderRollingReport() {
        ...
        const aggregate = computeAggregateReport(analysisEntries, state.config?.thresholds || DEFAULT_THRESHOLDS);
    ...
    function renderGradeCard(aggregate) {
        ...
        const labelEl = document.getElementById('rolling-grade-label');
        if (!card || !labelEl) return;

        if (!aggregate || !aggregate.gradeLabel) {
            labelEl.textContent = '—';
        ...
        const accent = aggregate.gradeColor || 'var(--primary)';
        labelEl.textContent = aggregate.gradeLabel;
    ...
    function renderScoreboard(aggregate) {
        ...
        const cards = [
            {
                ...
                value: formatScorePoints(aggregate.totalScore, 0),
                accent: aggregate.gradeColor,
                description: describeTotalScoreStatus(aggregate),
                details: buildTotalScoreDetails(aggregate),
            },
            {
                title: 'OOS 品質 / 信度',
        ...
        cards.forEach((card) => {
            ...
            el.appendChild(desc);
            appendCardDetails(el, card.details);
            scoreboard.appendChild(el);
        });
    }

    function appendCardDetails(container, details) {
        if (!Array.isArray(details) || details.length === 0) return;
        const detailsEl = document.createElement('details');
        detailsEl.className = 'mt-2 text-[11px] leading-relaxed';
        detailsEl.style.color = 'var(--muted-foreground)';

        const summary = document.createElement('summary');
        summary.textContent = '查看計算明細';
        summary.className = 'cursor-pointer font-medium';
        summary.style.color = 'var(--foreground)';
        detailsEl.appendChild(summary);

        const list = document.createElement('ul');
        list.className = 'mt-2 space-y-1 ml-4 list-disc';
    ...
    function buildTotalScoreDetails(aggregate) {
        const details = [];
        if (Number.isFinite(aggregate?.medianWindowScore) && Number.isFinite(aggregate?.wfeAdjustment)) {
            details.push(`窗分數中位 ${formatScorePoints(aggregate.medianWindowScore, 0)} × WFE 調整 ${formatNumber(aggregate.wfeAdjustment)} → ${formatScorePoints(aggregate.totalScore, 0)}`);
        } else if (Number.isFinite(aggregate?.totalScore)) {
            details.push(`總分換算為 ${formatScorePoints(aggregate.totalScore, 0)}，未滿足條件時會限制於 0～100 分之間。`);
        }
        if (Number.isFinite(aggregate?.passCount) && Number.isFinite(aggregate?.totalWindows)) {
            const rateText = Number.isFinite(aggregate.passRate) ? `${aggregate.passRate.toFixed(1)}%` : '—';
            details.push(`符合門檻視窗 ${aggregate.passCount}/${aggregate.totalWindows}（${rateText}）`);
        }
        if (aggregate?.summaryText) {
            details.push(aggregate.summaryText);
        }
        return details;
    }

    function buildQualityDetails(aggregate) {
        const details = [];
        if (Number.isFinite(aggregate?.medianOosQuality) || Number.isFinite(aggregate?.medianOosPassRatio) || Number.isFinite(aggregate?.medianCredibility)) {
            const parts = [];
            if (Number.isFinite(aggregate?.medianOosQuality)) parts.push(`OOS 品質中位 ${formatScore(aggregate.medianOosQuality)}`);
            if (Number.isFinite(aggregate?.medianOosPassRatio)) parts.push(`達標權重比 ${formatProbability(aggregate.medianOosPassRatio)}`);
            if (Number.isFinite(aggregate?.medianCredibility)) parts.push(`可信度中位 ${formatProbability(aggregate.medianCredibility)}`);
            if (parts.length > 0) details.push(parts.join('，'));
        }
        if (Array.isArray(aggregate?.qualityDetails)) {
            aggregate.qualityDetails.forEach((component) => {
                const text = formatQualityComponentDetail(component);
    ...
    function buildWfeDetails(aggregate) {
        const details = [];
        if (Number.isFinite(aggregate?.medianWfePercent)) {
            details.push(`Walk-Forward Efficiency 中位 ${formatPercent(aggregate.medianWfePercent)}`);
        }
        if (Number.isFinite(aggregate?.wfeAdjustment)) {
            details.push(`調整區間 ${Math.round(WFE_ADJUST_MIN * 100)}%~${Math.round(WFE_ADJUST_MAX * 100)}%，目前係數 ${aggregate.wfeAdjustment.toFixed(2)}`);
        }
        return details;
    }

    function buildCredibilityDetails(aggregate) {
        const details = [];
        if (Number.isFinite(aggregate?.medianCredibility)) {
            details.push(`統計可信度中位 ${formatProbability(aggregate.medianCredibility)}`);
        }
        if (Number.isFinite(aggregate?.medianStatWeight)) {
            details.push(`統計權重中位 ${(aggregate.medianStatWeight * 100).toFixed(1)}%`);
        }
        if (Number.isFinite(aggregate?.psrAbove95Ratio)) {
            details.push(`PSR≥95% 視窗比 ${formatProbability(aggregate.psrAbove95Ratio)}`);
        }
        if (Number.isFinite(aggregate?.medianPsr) || Number.isFinite(aggregate?.medianDsr)) {
            const psrText = Number.isFinite(aggregate?.medianPsr) ? `PSR 中位 ${formatProbability(aggregate.medianPsr)}` : null;
            const dsrText = Number.isFinite(aggregate?.medianDsr) ? `DSR 中位 ${formatProbability(aggregate.medianDsr)}` : null;
            const combined = [psrText, dsrText].filter(Boolean).join('，');
            if (combined) details.push(combined);
        }
        if (Number.isFinite(aggregate?.overallPsr) || Number.isFinite(aggregate?.overallDsr)) {
            const psrText = Number.isFinite(aggregate?.overallPsr) ? `整體 PSR ${formatProbability(aggregate.overallPsr)}` : null;
            const dsrText = Number.isFinite(aggregate?.overallDsr) ? `整體 DSR ${formatProbability(aggregate.overallDsr)}` : null;
            const combined = [psrText, dsrText].filter(Boolean).join('，');
            if (combined) details.push(combined);
        }
        return details;
    ...
    function buildSharpeDetails(aggregate) {
        const details = [];
        if (Number.isFinite(aggregate?.overallSharpe)) {
            details.push(`整體 Sharpe ${formatNumber(aggregate.overallSharpe)}`);
        }
        if (Number.isFinite(aggregate?.overallDsr)) {
            details.push(`整體 DSR ${formatProbability(aggregate.overallDsr)}`);
        }
        if (Number.isFinite(aggregate?.medianSharpe) || Number.isFinite(aggregate?.medianSortino)) {
            const sharpeText = Number.isFinite(aggregate?.medianSharpe) ? `視窗 Sharpe 中位 ${formatNumber(aggregate.medianSharpe)}` : null;
            const sortinoText = Number.isFinite(aggregate?.medianSortino) ? `視窗 Sortino 中位 ${formatNumber(aggregate.medianSortino)}` : null;
            const combined = [sharpeText, sortinoText].filter(Boolean).join('，');
            if (combined) details.push(combined);
        }
        return details;
    }

    function buildPassRateDetails(aggregate) {
        const details = [];
        if (Number.isFinite(aggregate?.passCount) && Number.isFinite(aggregate?.totalWindows)) {
            const rateText = Number.isFinite(aggregate?.passRate) ? `${aggregate.passRate.toFixed(1)}%` : '—';
            details.push(`符合門檻視窗 ${aggregate.passCount}/${aggregate.totalWindows}（${rateText}）`);
        }
        if (aggregate?.thresholds) {
            details.push(`門檻：年化 ≥ ${aggregate.thresholds.annualizedReturn}%、Sharpe ≥ ${aggregate.thresholds.sharpeRatio}、Sortino ≥ ${aggregate.thresholds.sortinoRatio}、MaxDD ≤ ${formatPercent(aggregate.thresholds.maxDrawdown)}、勝率 ≥ ${aggregate.thresholds.winRate}%`);
        }
        return details;
    }

    function formatQualityComponentDetail(component) {
        if (!component) return null;
        const label = component.label || component.key;
        const pieces = [];
        if (Number.isFinite(component.medianValue)) {
            pieces.push(`中位 ${formatMetricByKey(component.key, component.medianValue)}`);
        }
        if (component.direction === 'lower') {
            if (Number.isFinite(component.threshold)) {
                pieces.push(`門檻 ≤ ${formatMetricByKey(component.key, component.threshold)}`);
            }
            if (Number.isFinite(component.best) && Number.isFinite(component.worst)) {
                pieces.push(`建議 ${formatMetricByKey(component.key, component.best)} ~ ${formatMetricByKey(component.key, component.worst)}`);
            }
        } else {
            if (Number.isFinite(component.threshold)) {
                pieces.push(`門檻 ≥ ${formatMetricByKey(component.key, component.threshold)}`);
            }
            if (Number.isFinite(component.target)) {
                pieces.push(`目標 ${formatMetricByKey(component.key, component.target)}`);
            }
        }
        if (Number.isFinite(component.medianScore)) {
            pieces.push(`評分 ${component.medianScore.toFixed(2)}`);
        }
        if (Number.isFinite(component.passRatio)) {
            pieces.push(`達標率 ${(component.passRatio * 100).toFixed(0)}%`);
        }
        if (Number.isFinite(component.weight)) {
            pieces.push(`權重 ${(component.weight * 100).toFixed(0)}%`);
        }
    ...
    function formatMetricByKey(key, value) {
        ...
        if (key === 'annualizedReturn' || key === 'maxDrawdown' || key === 'winRate') {
    ...
    function renderWindowTable(aggregate) {
        ...
        const headerRow = document.getElementById('rolling-window-header');
        if (!tbody) return;
        tbody.innerHTML = '';
        ...
        if (headerRow) {
            ...
            titleCell.className = 'px-3 py-2 text-left font-semibold';
            titleCell.textContent = '指標';
            ...
            evaluations.forEach((entry) => {
                ...
                const title = document.createElement('div');
                title.textContent = `視窗 ${entry.index + 1}`;
                const subtitle = document.createElement('div');
                subtitle.className = 'text-[11px] font-normal';
                subtitle.style.color = 'var(--muted-foreground)';
                subtitle.textContent = `${entry.window.testingStart} ~ ${entry.window.testingEnd}`;
        ...
        const rowDefinitions = [
            {
                label: '訓練期間',
                align: 'text-left',
                value: (entry) => `${entry.window.trainingStart} ~ ${entry.window.trainingEnd}`,
            },
            {
                label: '測試期間',
                align: 'text-left',
                value: (entry) => `${entry.window.testingStart} ~ ${entry.window.testingEnd}`,
            },
            {
                label: '測試樣本日數',
                align: 'text-right',
            ...
            },
            {
                label: '年化報酬率',
                align: 'text-right',
                value: (entry) => formatPercent(entry.metrics.annualizedReturn),
                getClass: (entry) => {
                    if (!Number.isFinite(entry.metrics.annualizedReturn)) return '';
                    return entry.metrics.annualizedReturn >= 0 ? 'text-emerald-600' : 'text-rose-600';
                },
            },
            {
                label: 'Sharpe',
                align: 'text-right',
                value: (entry) => formatNumber(entry.metrics.sharpeRatio),
            },
            {
                label: 'Sortino',
                align: 'text-right',
                value: (entry) => formatNumber(entry.metrics.sortinoRatio),
            },
            {
                label: '最大回撤',
                align: 'text-right',
                value: (entry) => formatPercent(entry.metrics.maxDrawdown),
            },
            {
                label: '勝率',
                align: 'text-right',
                value: (entry) => formatPercent(entry.metrics.winRate),
            },
            {
                label: '交易數',
                align: 'text-right',
                value: (entry) => (Number.isFinite(entry.metrics.tradesCount) ? entry.metrics.tradesCount : '—'),
            },
            {
                label: 'PSR≥95%',
                align: 'text-right',
                value: (entry) => formatProbability(entry.metrics?.analysis?.psrProbability),
            },
            {
                label: 'DSR',
                align: 'text-right',
                value: (entry) => formatProbability(entry.metrics?.analysis?.dsrProbability),
            },
            {
                label: '可信度',
                align: 'text-right',
                value: (entry) => formatProbability(entry.metrics?.analysis?.credibility),
            },
            {
                label: 'WFE',
                align: 'text-right',
                value: (entry) => (Number.isFinite(entry.metrics?.analysis?.wfe)
            ...
            },
            {
                label: '窗分數',
                align: 'text-right',
                value: (entry) => formatScorePoints(entry.metrics?.analysis?.windowScore, 0),
            },
            {
                label: '參數摘要',
                align: 'text-left',
                value: (entry) => buildParameterSummary(entry.paramsSnapshot),
                wrap: true,
            },
            {
                label: '評語',
                align: 'text-left',
                value: (entry) => entry.comment || '—',
                wrap: true,
            },
        ];
        ...
        rowDefinitions.forEach((row) => {
            ...
            labelCell.className = 'px-3 py-2 text-left font-semibold';
            labelCell.textContent = row.label;
            tr.appendChild(labelCell);

            evaluations.forEach((entry) => {
                const td = document.createElement('td');
                td.className = `px-3 py-2 ${row.align || 'text-left'}`;
                let valueText = typeof row.value === 'function' ? row.value(entry) : '—';
                if (valueText === undefined || valueText === null || valueText === '') {
                    valueText = '—';
                }
                td.textContent = valueText;
                if (typeof row.getClass === 'function') {
                    const extra = row.getClass(entry);
    ...
    function computeAggregateReport(entries, thresholds, options = {}) {
        ...
        const qualityDetails = summarizeQualityComponents(analyses);
    ...
    function summarizeQualityComponents(analyses) {
        const summaryMap = new Map();
        analyses.forEach((analysis) => {
            const components = analysis?.oosQuality?.components;
            ...
            Object.entries(components).forEach(([key, data]) => {
                ...
                if (!summaryMap.has(key)) {
                    summaryMap.set(key, {
                        ...
                        weight: Number.isFinite(data.weight) ? data.weight : QUALITY_WEIGHTS[key] || null,
                        direction: data.direction || (key === 'maxDrawdown' ? 'lower' : 'higher'),
                        values: [],
                        scores: [],
                        passes: 0,
                        count: 0,
                        ...
                        worst: Number.isFinite(data.worst) ? data.worst : null,
                    });
                }
                const bucket = summaryMap.get(key);
                if (Number.isFinite(data.value)) bucket.values.push(data.value);
                if (Number.isFinite(data.score)) bucket.scores.push(data.score);
                if (data.passed) bucket.passes += 1;
                bucket.count += 1;
            });
        });
        ...
        return Array.from(summaryMap.values()).map((entry) => ({
            ...
            label: entry.label,
            weight: entry.weight,
            medianValue: median(entry.values),
            medianScore: median(entry.scores),
            passRatio: entry.count > 0 ? entry.passes / entry.count : null,
    ...
    function buildSummaryText(context) {
        const parts = [];
        parts.push(`${context.gradeLabel} · Walk-Forward 總分 ${formatScorePoints(context.totalScore, 0)}`);
    ...
    function computeOosQualityScore(metrics, thresholds) {
        ...
        const accumulate = (key, options) => {
            const weight = Number.isFinite(options?.weight) ? options.weight : 0;
            const score = Number.isFinite(options?.score) ? clamp01(options.score) : 0;
            ...
            components[key] = {
                ...
                weight,
                passed,
                value: Number.isFinite(options?.value) ? options.value : null,
                threshold: Number.isFinite(options?.threshold) ? options.threshold : null,
                target: Number.isFinite(options?.target) ? options.target : null,
                best: Number.isFinite(options?.best) ? options.best : null,
                worst: Number.isFinite(options?.worst) ? options.worst : null,
                direction: options?.direction || (key === 'maxDrawdown' ? 'lower' : 'higher'),
            };
        ...
        accumulate('annualizedReturn', {
            score: annScore,
            weight: QUALITY_WEIGHTS.annualizedReturn,
            passed: annPass,
            value: annValue,
        ...
        accumulate('sharpeRatio', {
            score: sharpeScore,
            weight: QUALITY_WEIGHTS.sharpeRatio,
            passed: sharpePass,
            value: sharpeValue,
        ...
        accumulate('sortinoRatio', {
            score: sortinoScore,
            weight: QUALITY_WEIGHTS.sortinoRatio,
            passed: sortinoPass,
            value: sortinoValue,
            threshold: sortinoThreshold,
            target: sortinoTarget,
            direction: 'higher',
        });
        ...
        accumulate('maxDrawdown', {
            score: drawdownScore,
            weight: QUALITY_WEIGHTS.maxDrawdown,
            passed: drawdownPass,
            value: drawdownValue,
        ...
        accumulate('winRate', {
            score: winRateScore,
            weight: QUALITY_WEIGHTS.winRate,
            passed: winRatePass,
            value: winRateValue,
        ...
        const value = Number.isFinite(rawValue) ? rawValue * passRatio : null;
    ...
    function handleVisibleDataChange() {
        planRefreshAttempts = 0;
        schedulePlanRefresh(0);
    }

    function schedulePlanRefresh(delay) {
        if (planRefreshTimer) {
        ...
        const success = updateRollingPlanPreview();
        planRefreshAttempts += 1;
        if (!success && planRefreshAttempts < 3) {
            schedulePlanRefresh(planRefreshAttempts * 220);
        }
    }
    ...
    function formatScorePoints(value, decimals = 1) {
        ...
        return `${(bounded * 100).toFixed(decimals)} 分`;
    }
