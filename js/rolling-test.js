...
(function() {
    ...
    function resolveTradeTimingLabel(value) {
        if (value === 'close') return '當日收盤';
        if (value === 'open' || value === 'open_next') return '隔日開盤';
        if (value === 'close_next') return '隔日收盤';
        return '';
    }

    function resolvePositionBasisLabel(value) {
        if (value === 'initialCapital') return '初始本金買入';
        if (value === 'totalCapital') return '總資金買入';
        return '';
    }
    ...
    function buildTrainingOverrideParams(baseParams, windowInfo) {
        const override = deepClone(baseParams || {});
        if (!override || typeof override !== 'object') return {};

        if (windowInfo?.trainingStart) override.startDate = windowInfo.trainingStart;
        if (windowInfo?.trainingEnd) override.endDate = windowInfo.trainingEnd;

        if (!Array.isArray(override.entryStages) || override.entryStages.length === 0) {
            override.entryStages = Array.isArray(baseParams?.entryStages)
                ? [...baseParams.entryStages]
                : [];
        }
        if (!Array.isArray(override.exitStages) || override.exitStages.length === 0) {
            override.exitStages = Array.isArray(baseParams?.exitStages)
                ? [...baseParams.exitStages]
                : [];
        }

        const numericFields = ['initialCapital', 'positionSize', 'buyFee', 'sellFee', 'stopLoss', 'takeProfit'];
        numericFields.forEach((key) => {
            if (Number.isFinite(baseParams?.[key])) {
                override[key] = baseParams[key];
            }
        });

        const booleanFields = ['adjustedPrice', 'splitAdjustment', 'enableShorting'];
        booleanFields.forEach((key) => {
            if (typeof baseParams?.[key] === 'boolean') {
                override[key] = baseParams[key];
            }
        });
        ...
        if (baseParams?.entryParams) override.entryParams = deepClone(baseParams.entryParams);
        if (baseParams?.exitParams) override.exitParams = deepClone(baseParams.exitParams);
        if (baseParams?.shortEntryParams) override.shortEntryParams = deepClone(baseParams.shortEntryParams);
        if (baseParams?.shortExitParams) override.shortExitParams = deepClone(baseParams.shortExitParams);
    ...
    async function optimizeParametersForWindow(baseWindowParams, windowInfo, plan) {
        ...
        const originalRange = {
            startDate: baseWindowParams?.startDate || null,
            endDate: baseWindowParams?.endDate || null,
        };

        const trainingOverride = buildTrainingOverrideParams(baseWindowParams, windowInfo);
        const outputParams = deepClone(trainingOverride);
        ...
        const workingParams = deepClone(trainingOverride);

        const baselineSnapshots = captureOptimizationBaselines(plan.scopes, trainingOverride);
        const scopeResults = [];
        ...
        if (originalRange.startDate) outputParams.startDate = originalRange.startDate;
        else delete outputParams.startDate;
        if (originalRange.endDate) outputParams.endDate = originalRange.endDate;
        else delete outputParams.endDate;
    ...
    async function runCombinationOptimizationForWindow({ baseParams, plan, windowInfo, combinationScopes, baselineSnapshots }) {
        ...
        const overrideParams = buildTrainingOverrideParams(baseParams, windowInfo);
