...
(function() {
        ...
        for (let i = 0; i < state.windows.length; i += 1) {
            ...
            if (optimizationPlan.enabled) {
                try {
                    ...
                    if (optimizationResult?.trainingParams) {
                        trainingContext = deepClone(optimizationResult.trainingParams);
                    }
            ...
            state.results.push({
                ...
                trainingParams: trainingContext ? deepClone(trainingContext) : null,
            });
        }
    }
    ...
    function renderRollingReport() {
        ...
        const analysisEntries = state.results.map((entry, index) => {
            ...
            return {
                ...
                trainingParamsSnapshot: entry.trainingParams || null,
            };
        });
    ...
    function resolveTradeTimingLabel(value) {
        ...
        if (value === 'next_open') return '隔日開盤';
        return '';
    }
    ...
    async function optimizeParametersForWindow(baseWindowParams, windowInfo, plan) {
        ...
        const baseSnapshot = deepClone(baseWindowParams);
        const trainingBaseParams = buildWindowTrainingParams(baseWindowParams, windowInfo);
        const outputParams = deepClone(baseSnapshot);

        if (!plan?.enabled || !Array.isArray(plan.scopes) || plan.scopes.length === 0) {
            mergeGlobalExecutionSettings(outputParams, trainingBaseParams);
            return { params: outputParams, summary, trainingParams: trainingBaseParams };
        }
        ...
        const workingParams = deepClone(trainingBaseParams);
    ...
    async function runCombinationOptimizationForWindow({ baseParams, plan, windowInfo, combinationScopes, baselineSnapshots, trainingBaseParams }) {
        const combination = buildCombinationFromParams(baseParams);
        if (!combination) return null;

        const overrideParams = deepClone(trainingBaseParams || baseParams);
        overrideParams.startDate = windowInfo.trainingStart;
        overrideParams.endDate = windowInfo.trainingEnd;
        overrideParams.effectiveStartDate = windowInfo.trainingStart;
        overrideParams.dataStartDate = null;
        overrideParams.lookbackDays = null;
    ...
    function buildWindowTrainingParams(baseParams, windowInfo) {
        const snapshot = deepClone(baseParams || {});
        if (windowInfo?.trainingStart) {
            snapshot.startDate = windowInfo.trainingStart;
            snapshot.effectiveStartDate = windowInfo.trainingStart;
        }
        if (windowInfo?.trainingEnd) {
            snapshot.endDate = windowInfo.trainingEnd;
        }
        delete snapshot.testingStart;
        delete snapshot.testingEnd;
        delete snapshot.recentYears;
        snapshot.dataStartDate = null;
        snapshot.lookbackDays = null;

        if (!Array.isArray(snapshot.entryStages) || snapshot.entryStages.length === 0) {
            if (Number.isFinite(snapshot.positionSize) && snapshot.positionSize > 0) {
                snapshot.entryStages = [Number(snapshot.positionSize)];
            } else {
                snapshot.entryStages = [];
            }
        } else {
            snapshot.entryStages = snapshot.entryStages
                .map((stage) => (Number.isFinite(stage) ? Number(stage) : stage))
                .filter((stage) => stage !== null && stage !== undefined);
        }

        if (!Array.isArray(snapshot.exitStages) || snapshot.exitStages.length === 0) {
            snapshot.exitStages = [100];
        } else {
            snapshot.exitStages = snapshot.exitStages
                .map((stage) => (Number.isFinite(stage) ? Number(stage) : stage))
                .filter((stage) => stage !== null && stage !== undefined);
        }

        if (!Number.isFinite(snapshot.initialCapital)) {
            const fallbackCapital = Number(baseParams?.initialCapital);
            snapshot.initialCapital = Number.isFinite(fallbackCapital) && fallbackCapital > 0 ? fallbackCapital : 100000;
        }
        if (!Number.isFinite(snapshot.positionSize)) {
            const fallbackSize = Number(baseParams?.positionSize);
            snapshot.positionSize = Number.isFinite(fallbackSize) && fallbackSize > 0 ? fallbackSize : 100;
        }

        if (baseParams?.buyFee !== undefined) snapshot.buyFee = baseParams.buyFee;
        if (baseParams?.sellFee !== undefined) snapshot.sellFee = baseParams.sellFee;
        snapshot.tradeTiming = baseParams?.tradeTiming || snapshot.tradeTiming || 'close';
        snapshot.positionBasis = baseParams?.positionBasis || snapshot.positionBasis || 'initialCapital';
        snapshot.enableShorting = Boolean(baseParams?.enableShorting);

        if (snapshot.enableShorting) {
            if (baseParams?.shortEntryStrategy) snapshot.shortEntryStrategy = baseParams.shortEntryStrategy;
            if (baseParams?.shortExitStrategy) snapshot.shortExitStrategy = baseParams.shortExitStrategy;
            snapshot.shortEntryParams = deepClone(baseParams?.shortEntryParams || snapshot.shortEntryParams || {});
            snapshot.shortExitParams = deepClone(baseParams?.shortExitParams || snapshot.shortExitParams || {});
        } else {
            snapshot.shortEntryStrategy = null;
            snapshot.shortExitStrategy = null;
            snapshot.shortEntryParams = {};
            snapshot.shortExitParams = {};
        }

        return snapshot;
    }

    function mergeGlobalExecutionSettings(target, source) {
        if (!target || !source) return;
        const numericKeys = ['initialCapital', 'positionSize', 'stopLoss', 'takeProfit'];
        numericKeys.forEach((key) => {
            if (Number.isFinite(source[key])) {
                target[key] = source[key];
            }
        });
        ['tradeTiming', 'positionBasis', 'market', 'marketType'].forEach((key) => {
            if (source[key] !== undefined) {
                target[key] = source[key];
            }
        });
        ['entryStages', 'exitStages'].forEach((key) => {
            if (Array.isArray(source[key])) {
                target[key] = source[key].map((value) => (Number.isFinite(value) ? Number(value) : value));
            }
        });
        if (source.buyFee !== undefined) target.buyFee = source.buyFee;
        if (source.sellFee !== undefined) target.sellFee = source.sellFee;
        if (source.enableShorting !== undefined) target.enableShorting = source.enableShorting;
        if (source.enableShorting) {
            if (source.shortEntryStrategy !== undefined) target.shortEntryStrategy = source.shortEntryStrategy;
            if (source.shortExitStrategy !== undefined) target.shortExitStrategy = source.shortExitStrategy;
            if (source.shortEntryParams) target.shortEntryParams = deepClone(source.shortEntryParams);
            if (source.shortExitParams) target.shortExitParams = deepClone(source.shortExitParams);
        } else {
            target.shortEntryStrategy = null;
            target.shortExitStrategy = null;
            target.shortEntryParams = {};
            target.shortExitParams = {};
        }
    }
