// --- 滾動測試模組 - v1.8 ---
// Patch Tag: LB-ROLLING-TEST-20250927B
...
(function() {
    const state = {
        ...
        version: 'LB-ROLLING-TEST-20250927B',
    ...
    function stripWindowAnchors(params) {
        if (!params || typeof params !== 'object') return params;
        Object.keys(params).forEach((key) => {
            const lower = key.toLowerCase();
            if (lower === 'startdate' || lower === 'enddate') return;
            if (lower === 'originalstartdate') {
                delete params[key];
                return;
            }
            if (lower.includes('effectivestart') || lower.includes('datastart') || lower.includes('lookback')) {
                delete params[key];
            }
        });
        return params;
    ...
    function prepareParamsForRange(params, startIso, endIso) {
        if (!params || typeof params !== 'object') return params;
        stripWindowAnchors(params);
        if (startIso) {
            params.startDate = startIso;
            params.originalStartDate = startIso;
        }
        if (endIso) {
    ...
    function prepareWorkerPayload(baseParams, startIso, endIso) {
        ...
        const clone = prepareParamsForRange(
            deepClone(baseParams),
            startIso,
            endIso,
        );
    ...
    async function optimizeParametersForWindow(baseWindowParams, windowInfo, plan) {
        ...
        stripWindowAnchors(outputParams);
        ...
        const workingParams = prepareParamsForRange(
            deepClone(baseWindowParams),
            windowInfo.trainingStart,
            windowInfo.trainingEnd,
        );
    ...
    async function runCombinationOptimizationForWindow({ baseParams, plan, windowInfo, combinationScopes, baselineSnapshots }) {
        ...
        const overrideParams = prepareParamsForRange(
            deepClone(baseParams),
            windowInfo.trainingStart,
            windowInfo.trainingEnd,
        );
