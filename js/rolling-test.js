// --- 滾動測試模組 - v2.1 ---
// Patch Tag: LB-ROLLING-TEST-20251012A
...
(function() {
    const state = {
        ...
        version: 'LB-ROLLING-TEST-20251012A',
        ...
        const cards = [
            {
                ...
                value: formatScore(aggregate.totalScore),
            ...
            },
            {
                ...
                description: `統計權重中位 ${formatScore(aggregate.medianStatWeight)}`,
            },
            {
                title: 'WFE 中位',
                value: formatPercent(aggregate.medianWfePercent),
                description: `調整係數 ${formatScore(aggregate.wfeAdjustment)}`,
            },
            {
                title: 'PSR / DSR',
                value: `${formatProbability(aggregate.medianPsr)} / ${formatProbability(aggregate.medianDsr)}`,
                description: `PSR≥95% 視窗比 ${formatProbability(aggregate.psrAbove95Ratio)}`,
            },
            {
                title: '整體 Sharpe',
                value: formatNumber(aggregate.overallSharpe),
                description: `PSR ${formatProbability(aggregate.overallPsr)} · DSR ${formatProbability(aggregate.overallDsr)}`,
            },
            {
                title: '通過視窗比例',
    ...
    function renderWindowTable(aggregate) {
        ...
        aggregate.evaluations.forEach((entry) => {
            ...
            windowScoreCell.textContent = formatScore(entry.metrics?.analysis?.windowScore);
    ...
    function buildSummaryText(context) {
        const parts = [];
        parts.push(`${context.gradeLabel} · Walk-Forward 總分 ${formatScore(context.totalScore)}`);
    ...
    function computeOosQualityScore(metrics, thresholds) {
        ...
        const annTarget = Math.max(Number(thresholds?.annualizedReturn) || 0, QUALITY_TARGETS.annualizedReturn);
        const annScore = normalizeRange(toFiniteNumber(metrics.annualizedReturn), 0, Math.max(annTarget, 1));
        if (annScore !== null) {
            weightedSum += QUALITY_WEIGHTS.annualizedReturn * annScore;
            weightTotal += QUALITY_WEIGHTS.annualizedReturn;
            components.annualizedReturn = annScore;
        }

        const sharpeTarget = Math.max(Number(thresholds?.sharpeRatio) || 0, QUALITY_TARGETS.sharpeRatio);
        const sharpeScore = normalizeRange(toFiniteNumber(metrics.sharpeRatio), 0, Math.max(sharpeTarget, 0.01));
        if (sharpeScore !== null) {
            weightedSum += QUALITY_WEIGHTS.sharpeRatio * sharpeScore;
            weightTotal += QUALITY_WEIGHTS.sharpeRatio;
            components.sharpeRatio = sharpeScore;
        }

        const sortinoTarget = Math.max(Number(thresholds?.sortinoRatio) || 0, QUALITY_TARGETS.sortinoRatio);
        const sortinoScore = normalizeRange(toFiniteNumber(metrics.sortinoRatio), 0, Math.max(sortinoTarget, 0.01));
        if (sortinoScore !== null) {
            weightedSum += QUALITY_WEIGHTS.sortinoRatio * sortinoScore;
            weightTotal += QUALITY_WEIGHTS.sortinoRatio;
            components.sortinoRatio = sortinoScore;
        }

        const drawdownWorst = Math.max(Number(thresholds?.maxDrawdown) || QUALITY_TARGETS.maxDrawdownSpan + QUALITY_TARGETS.maxDrawdownFloor, QUALITY_TARGETS.maxDrawdownFloor + 1);
        ...
        if (drawdownScore !== null) {
            weightedSum += QUALITY_WEIGHTS.maxDrawdown * drawdownScore;
            weightTotal += QUALITY_WEIGHTS.maxDrawdown;
            components.maxDrawdown = drawdownScore;
        }

        const winRateBase = Number(thresholds?.winRate) || 0;
        const winRateTarget = winRateBase + QUALITY_TARGETS.winRateBonus;
        const winRateScore = normalizeRange(toFiniteNumber(metrics.winRate), winRateBase, winRateTarget);
        if (winRateScore !== null) {
            weightedSum += QUALITY_WEIGHTS.winRate * winRateScore;
            weightTotal += QUALITY_WEIGHTS.winRate;
            components.winRate = winRateScore;
        }
