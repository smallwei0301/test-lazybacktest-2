...
(function() {
    ...
    function clearRollingReport() {
        ...
        if (gradeLabel) {
            gradeLabel.textContent = '尚未評分';
            gradeLabel.style.color = 'var(--muted-foreground)';
        }
        const gradeCard = document.getElementById('rolling-grade-card');
        if (gradeCard) {
            gradeCard.style.borderColor = 'var(--border)';
            gradeCard.style.background = 'color-mix(in srgb, var(--primary) 6%, transparent)';
        }
        const gradeDetails = document.getElementById('rolling-grade-details');
        if (gradeDetails) {
    ...
    function updateGradeCard(aggregate) {
        const gradeLabel = document.getElementById('rolling-grade-label');
        if (gradeLabel) {
            gradeLabel.textContent = aggregate?.gradeLabel || '尚未評分';
            gradeLabel.style.color = aggregate?.gradeColor || 'var(--muted-foreground)';
        }

        const gradeCard = document.getElementById('rolling-grade-card');
        if (gradeCard) {
            const accent = aggregate?.gradeColor || 'var(--border)';
            gradeCard.style.borderColor = `color-mix(in srgb, ${accent} 45%, transparent)`;
            gradeCard.style.background = `color-mix(in srgb, ${accent} 12%, transparent)`;
        }

        const details = document.getElementById('rolling-grade-details');
        if (details) {
            details.open = false;
        }
    ...
    function renderScoreboard(aggregate) {
        ...
        const componentStats = aggregate?.oosComponentStats || {};

        const formatCount = (value) => (Number.isFinite(value) ? Math.round(value) : '—');
        ...
        const oosDetailItems = [];
        oosDetailItems.push({
            type: 'text',
            text: `加權得分中位 ${formatScore(aggregate.medianOosQuality)}，原始 ${formatScore(aggregate.medianOosQualityRaw)} × 達標權重 ${formatProbability(aggregate.medianOosPassRatio)}`,
        });
        oosDetailItems.push({ type: 'heading', label: '指標得分與門檻' });
        Object.keys(QUALITY_WEIGHTS).forEach((key) => {
            const label = METRIC_LABELS[key] || key;
            const score = componentStats?.[key]?.median;
            const medianValue = (
                key === 'annualizedReturn' ? aggregate?.medianAnnualizedReturn
                    : key === 'sharpeRatio' ? aggregate?.medianSharpe
                        : key === 'sortinoRatio' ? aggregate?.medianSortino
                            : key === 'maxDrawdown' ? aggregate?.medianMaxDrawdown
                                : key === 'winRate' ? aggregate?.medianWinRate
                                    : null
            );
            const thresholdValue = thresholds?.[key];
            const formattedMedian = formatMetricValue(key, medianValue);
            const formattedThreshold = formatMetricValue(key, thresholdValue);
            const formattedScore = Number.isFinite(score) ? formatProbability(score) : '—';
            oosDetailItems.push({
                type: 'text',
                text: `${label}：中位 ${formattedMedian}｜門檻 ${formattedThreshold}｜得分 ${formattedScore}`,
            });
        });
        ...
        const credibilityDetails = [
            {
                type: 'text',
                text: `可信度中位 ${formatProbability(aggregate.medianCredibility)}，統計權重中位 ${formatNumber(aggregate.medianStatWeight)}`,
            },
            {
                type: 'text',
                text: `PSR 中位 ${formatProbability(aggregate.medianPsr)}（PSR≥95% 視窗比 ${formatProbability(aggregate.psrAbove95Ratio)}）`,
            },
            {
                type: 'text',
                text: `DSR 中位 ${formatProbability(aggregate.medianDsr)}，視窗樣本中位 ${formatCount(aggregate.medianSampleCount)}，最短需求 ${formatNumber(aggregate.medianMinTrackRecordLength)}`,
            },
        ];
        ...
        const wfeDetails = [
            {
                type: 'text',
                text: `WFE 中位 ${formatPercent(aggregate.medianWfePercent)}，調整倍數 ×${formatNumber(aggregate.wfeAdjustment)}`,
            },
            {
                type: 'text',
                text: `調整範圍 ${formatPercent(WFE_ADJUST_MIN * 100)}～${formatPercent(WFE_ADJUST_MAX * 100)}`,
            },
        ];
        ...
        const sharpeDetails = [
            {
                type: 'text',
                text: `整體 Sharpe ${formatNumber(aggregate.overallSharpe)}，PSR ${formatProbability(aggregate.overallPsr)}，DSR ${formatProbability(aggregate.overallDsr)}`,
            },
            {
                type: 'text',
                text: `樣本總數 ${formatCount(aggregate.overallSampleCount)}`,
            },
        ];
        ...
        const passRateDetails = [
            {
                type: 'text',
                text: `通過視窗 ${aggregate.passCount || 0} / ${aggregate.totalWindows || 0}，門檻 60%（合格）／40%（可觀察）`,
            },
        ];

        const totalScoreDetails = [
            {
                type: 'text',
                text: `單窗分數中位 ${formatScore(aggregate.medianWindowScore)}，WFE 調整 ×${formatNumber(aggregate.wfeAdjustment)}`,
            },
            {
                type: 'text',
                text: `WFE 中位 ${formatPercent(aggregate.medianWfePercent)}，總分採 0～100 分顯示`,
            },
        ];
        ...
        const cards = [
            ...
            },
            {
                ...
                details: oosDetailItems,
                detailSummary: '查看指標計算過程',
            },
            {
                title: 'WFE 中位',
        ...
        cards.forEach((card) => {
            ...
            if (Array.isArray(card.details) && card.details.length > 0) {
                ...
                summary.textContent = card.detailSummary || '查看詳情';
                ...
                card.details.forEach((item) => {
                    ...
                    if (item.type === 'heading') {
                        ...
                        heading.className = 'pt-1 text-[11px] font-semibold';
    ...
    function renderWindowTable(aggregate) {
        ...
        const thead = document.getElementById('rolling-window-header');
        ...
        labelHeader.className = 'px-3 py-2 text-left font-semibold';
        labelHeader.textContent = '指標';
        ...
        evaluations.forEach((entry, index) => {
            ...
            const title = document.createElement('div');
            title.textContent = `視窗 ${index + 1}`;
            const period = document.createElement('div');
            period.textContent = `${entry.window.testingStart} ~ ${entry.window.testingEnd}`;
            period.className = 'text-[11px] font-normal';
            period.style.color = 'var(--muted-foreground)';
        ...
        summaryHeader.className = 'px-3 py-2 text-right font-semibold';
        summaryHeader.textContent = '中位/整體';
        ...
        const rows = [
            ...
            },
            {
                ...
                getSummary: () => formatPercent(aggregate.medianAnnualizedReturn),
                getClass: (entry) => {
                    const value = entry.metrics?.annualizedReturn;
                    if (Number.isFinite(value)) {
                        return `px-3 py-2 text-right ${value >= 0 ? 'text-emerald-600' : 'text-rose-600'}`;
                    }
            ...
            },
            {
                ...
                getSummary: () => formatNumber(aggregate.medianSharpe),
            },
            {
                label: 'Sortino Ratio',
                align: 'right',
                getValue: (entry) => formatNumber(entry.metrics?.sortinoRatio),
                getSummary: () => formatNumber(aggregate.medianSortino),
            },
            {
                label: '最大回撤',
                align: 'right',
                getValue: (entry) => formatPercent(entry.metrics?.maxDrawdown),
                getSummary: () => formatPercent(aggregate.medianMaxDrawdown),
            },
            {
                label: '勝率',
                align: 'right',
                getValue: (entry) => formatPercent(entry.metrics?.winRate),
                getSummary: () => formatPercent(aggregate.medianWinRate),
            },
            {
                label: '交易數',
                align: 'right',
                getValue: (entry) => formatCount(entry.metrics?.tradesCount),
                getSummary: () => formatCount(aggregate.medianTradesCount),
            },
            {
                label: 'PSR95',
                align: 'right',
                getValue: (entry) => formatProbability(entry.metrics?.analysis?.psrProbability),
                getSummary: () => formatProbability(aggregate.medianPsr),
            },
            {
                label: 'DSR',
                align: 'right',
                getValue: (entry) => formatProbability(entry.metrics?.analysis?.dsrProbability),
                getSummary: () => formatProbability(aggregate.medianDsr),
            },
            {
                label: '可信度',
                align: 'right',
                getValue: (entry) => formatProbability(entry.metrics?.analysis?.credibility),
                getSummary: () => formatProbability(aggregate.medianCredibility),
            },
            {
                label: '統計權重',
                align: 'right',
                getValue: (entry) => formatNumber(entry.metrics?.analysis?.statWeight),
                getSummary: () => formatNumber(aggregate.medianStatWeight),
            },
            {
                label: 'WFE',
                align: 'right',
                getValue: (entry) => (Number.isFinite(entry.metrics?.analysis?.wfe) ? formatPercent(entry.metrics.analysis.wfe) : '—'),
                getSummary: () => formatPercent(aggregate.medianWfePercent),
            },
            {
                label: '單窗分數',
                align: 'right',
                getValue: (entry) => formatScorePoints(entry.metrics?.analysis?.windowScore),
                getSummary: () => formatScorePoints(aggregate.medianWindowScore),
            },
            {
                label: '樣本數',
                align: 'right',
                getValue: (entry) => formatCount(entry.metrics?.analysis?.sampleCount),
                getSummary: () => formatCount(aggregate.medianSampleCount),
                getClass: (entry) => {
                    const sample = entry.metrics?.analysis?.sampleCount;
                    const requirement = entry.metrics?.analysis?.minTrackRecordLength;
                    if (Number.isFinite(sample) && Number.isFinite(requirement) && requirement > sample) {
                        return 'px-3 py-2 text-right text-amber-600';
                    }
                    return 'px-3 py-2 text-right';
                },
            },
            {
                ...
                getValue: (entry) => {
                    ...
                    const sample = entry.metrics?.analysis?.sampleCount;
                    const text = `≥ ${formatCount(requirement)}`;
                    if (Number.isFinite(sample) && requirement > sample) {
                        return `${text}（不足）`;
                    }
                    return text;
                },
                getSummary: () => {
                    if (!Number.isFinite(aggregate.medianMinTrackRecordLength)) return '—';
                    return `≥ ${formatCount(aggregate.medianMinTrackRecordLength)}`;
                },
                getClass: (entry) => {
                    const sample = entry.metrics?.analysis?.sampleCount;
                    const requirement = entry.metrics?.analysis?.minTrackRecordLength;
        ...
        rows.forEach((row) => {
            ...
            evaluations.forEach((entry) => {
                ...
                const baseClass = `px-3 py-2 ${row.align === 'right' ? 'text-right' : 'text-left'}`;
                cell.className = row.getClass ? row.getClass(entry) : baseClass;
                if (!row.getClass) {
                    cell.className = baseClass;
                }
                if (row.multiLine) {
                    cell.classList.add('whitespace-pre-wrap');
                }
                cell.textContent = row.getValue ? row.getValue(entry) : '—';
                tr.appendChild(cell);
            });

            const summaryCell = document.createElement('td');
            summaryCell.className = `px-3 py-2 ${row.align === 'right' ? 'text-right font-semibold' : 'text-left font-semibold'}`;
            if (row.multiLine) {
                summaryCell.classList.add('whitespace-pre-wrap');
            }
            summaryCell.textContent = row.getSummary ? row.getSummary() : '—';
    ...
    function computeAggregateReport(entries, thresholds, options = {}) {
        ...
        const medianTradesCount = median(validMetrics.map((m) => m.tradesCount));
        ...
        const oosComponentStats = {};
        Object.keys(QUALITY_WEIGHTS).forEach((key) => {
            const componentValues = analyses.map((analysis) => {
                const value = analysis?.oosQuality?.components?.[key];
                return Number.isFinite(value) ? value : null;
            });
            oosComponentStats[key] = {
                median: median(componentValues),
                average: average(componentValues),
            };
        });
        ...
        const minTrlValues = analyses.map((analysis) => (Number.isFinite(analysis?.minTrackRecordLength) ? analysis.minTrackRecordLength : null));
        const medianMinTrackRecordLength = median(minTrlValues);
        ...
        const overallSampleCount = Number.isFinite(overallMoments?.sampleCount) ? overallMoments.sampleCount : null;
    ...
    function computeOosQualityScore(metrics, thresholds) {
        ...
        const value = Number.isFinite(rawValue) ? clamp01(rawValue * passRatio) : null;
