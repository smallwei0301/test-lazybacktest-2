// --- 滾動測試模組 - v2.3 ---
// Patch Tag: LB-ROLLING-TEST-20251024A
...
(function() {
    const state = {
        ...
        version: 'LB-ROLLING-TEST-20251024A',
    ...
    function describeQualityStatus(aggregate) {
        ...
        const passRatio = Number.isFinite(aggregate?.medianQualityPassRatio) ? aggregate.medianQualityPassRatio : null;
        if (quality === null || credibility === null) {
            return '資料不足，建議增加視窗或延長樣本';
        }
        const qualityPass = quality >= 0.7;
        const credibilityPass = credibility >= 0.5;
        const ratioPass = passRatio === null ? qualityPass : passRatio >= 0.6;
        if (qualityPass && credibilityPass && ratioPass) return '合格';
        if (!ratioPass) return '多數指標未達門檻，建議檢查弱項';
        if (!qualityPass && credibilityPass) return '品質未達標，建議調整策略';
        if (qualityPass && !credibilityPass) return '可信度不足，建議增加樣本';
        return '未達標，建議調整參數';
    }

    function describeWfeStatus(aggregate) {
        const wfe = Number.isFinite(aggregate?.medianWfePercent) ? aggregate.medianWfePercent : null;
        if (wfe === null) return '資料不足，建議檢查視窗樣本';
        if (wfe >= 80) return '合格';
        if (wfe >= 60) return '接近門檻，建議增加視窗或樣本';
        return '未達標，建議調整訓練與測試期';
    }
    ...
    function describeCredibilityStatus(aggregate) {
        ...
        if (psrPass && !dsrPass) return '多次嘗試折現偏弱，建議收斂參數';
        return '未達標，建議增加樣本與視窗';
    }
    ...
    function describeSharpeStatus(aggregate) {
        ...
        if (sharpe === null) return '資料不足，建議完成更多視窗';
    ...
    function describePassRateStatus(aggregate) {
        ...
        if (passRate === null) return '資料不足，建議完成更多視窗';
        if (passRate >= 60) return '合格';
        if (passRate >= 40) return '接近門檻，建議持續觀察';
        return '未達標，建議調整參數與視窗';
    }
    ...
    function computeAggregateReport(entries, thresholds, minTrades, options = {}) {
        ...
        const qualityPassRatios = analyses
            .map((analysis) => (Number.isFinite(analysis?.oosQuality?.passRatio) ? analysis.oosQuality.passRatio : null));
        const medianQualityPassRatio = median(qualityPassRatios);
    ...
    function computeOosQualityScore(metrics, thresholds) {
        if (!metrics || metrics.error) {
            return { value: null, components: {}, passRatio: null, raw: null };
        }
        const components = {};
        let weightedSum = 0;
        let weightTotal = 0;
        ...
        const passRatio = weightTotal > 0 ? passWeight / weightTotal : null;
        let value = rawValue;
        if (Number.isFinite(value) && Number.isFinite(passRatio) && passRatio < 1) {
            value = Math.min(value, passRatio);
        }

        return { value, components, passRatio, raw: rawValue };
    }
    ...
    function ensureRollingCacheHydrated() {
        ...
        const entry = getLastCacheEntry();
        if (!entry) {
            const fallbackRows = resolveLastResultRows();
            if (Array.isArray(fallbackRows) && fallbackRows.length > 0) {
                cachedStockData = fallbackRows;
                return cachedStockData;
            }
            return null;
        }
    ...
    function resolveLastResultRows() {
        const candidates = [];
        if (Array.isArray(lastOverallResult?.priceRows)) candidates.push(lastOverallResult.priceRows);
        if (Array.isArray(lastOverallResult?.rows)) candidates.push(lastOverallResult.rows);
        if (Array.isArray(lastOverallResult?.dataset)) candidates.push(lastOverallResult.dataset);
        if (Array.isArray(lastOverallResult?.data)) candidates.push(lastOverallResult.data);
        if (Array.isArray(lastOverallResult?.priceData?.rows)) candidates.push(lastOverallResult.priceData.rows);
        for (let i = 0; i < candidates.length; i += 1) {
