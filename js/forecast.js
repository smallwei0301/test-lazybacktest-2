const FORECAST_VERSION_CODE = 'LB-FORECAST-LSTMGA-20251116A';
...
function describeCorrection(correction) {
    ...
    const carryText = Number.isFinite(correction.finalCumulativeError)
        ...
        : '--';
    const trainingMseText = Number.isFinite(correction.baselineTrainingMse) && Number.isFinite(correction.trainingMse)
        ? ` ｜ 訓練 MSE ${formatNumber(correction.baselineTrainingMse, 4)} → ${formatNumber(correction.trainingMse, 4)}`
        : '';
    return `校正閾值 δ = ${deltaText}% ｜ 訓練累積誤差 = ${carryText}${trainingMseText}`;
}
...
function runGeneticOptimization({ rawPreds, actuals }) {
    ...
    let population = Array.from({ length: populationSize }, () => evaluate(randomDelta()));
    let bestIndividual = population.reduce((best, individual) => {
        if (!best || individual.mse < best.mse) {
            return individual;
        }
        return best;
    }, null);
    ...
    const selectParent = (pool) => {
        const fitnessTotal = pool.reduce((sum, individual) => sum + individual.fitness, 0);
        if (fitnessTotal <= 0) {
            return pool[Math.floor(Math.random() * pool.length)];
        }
        let threshold = Math.random() * fitnessTotal;
        for (const individual of pool) {
            threshold -= individual.fitness;
            if (threshold <= 0) {
                return individual;
            }
        }
    ...
    for (let generation = 0; generation < generations; generation += 1) {
        const sortedPopulation = [...population].sort((a, b) => a.mse - b.mse);
        const elites = sortedPopulation.slice(0, 2);
        const nextPopulation = elites.slice();

        while (nextPopulation.length < populationSize) {
            const parentA = selectParent(population);
            const parentB = selectParent(population);
            let childDelta = clampDelta((parentA.delta + parentB.delta) / 2);
            if (Math.random() < mutationRate) {
                const jitter = (Math.random() * 2 - 1) * mutationScale * childDelta;
                childDelta = clampDelta(childDelta + jitter);
            }
            const child = evaluate(childDelta);
            nextPopulation.push(child);
            if (!Number.isFinite(bestIndividual?.mse) || (Number.isFinite(child.mse) && child.mse < bestIndividual.mse)) {
    ...
    return {
        delta: clampDelta(bestIndividual.delta),
        mse: bestIndividual.mse,
...
async function runForecastWorkflow() {
    ...
    const { corrected: correctedTrainingSeries, finalCumulativeError: finalTrainingCarry } = applyErrorCorrection({
        rawPreds: trainPredActuals,
        actuals: trainActuals,
        initialCumulativeError: 0,
        delta: gaResult.delta,
    });
    const correctedTrainingMse = computeMse(correctedTrainingSeries, trainActuals);
    ...
    const correctedResult = applyErrorCorrection({
        ...
        initialCumulativeError: finalTrainingCarry,
        delta: gaResult.delta,
    });
    ...
    return {
        ...
        correction: {
            delta: gaResult.delta,
            finalCumulativeError: finalTrainingCarry,
            trainingMse: correctedTrainingMse,
            baselineTrainingMse: trainingBaselineMse,
