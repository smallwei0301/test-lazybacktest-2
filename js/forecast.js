...
function describeCorrection(correction) {
    ...
    const carryText = Number.isFinite(correction.finalCumulativeError)
        ...
        : '--';
    const baselineText = Number.isFinite(correction.trainingBaselineMse)
        ? formatNumber(correction.trainingBaselineMse, 4)
        : '--';
    const optimisedText = Number.isFinite(correction.trainingOptimisedMse)
        ? formatNumber(correction.trainingOptimisedMse, 4)
        : '--';
    const improvementValue = Number.isFinite(correction.trainingBaselineMse)
        && Number.isFinite(correction.trainingOptimisedMse)
        ? correction.trainingBaselineMse - correction.trainingOptimisedMse
        : NaN;
    const improvementText = Number.isFinite(improvementValue)
        ? formatNumber(improvementValue, 4)
        : '--';
    return `校正閾值 δ = ${deltaText}% ｜ 訓練累積誤差 = ${carryText} ｜ 訓練 MSE ${baselineText} → ${optimisedText} （改善 ${improvementText}）`;
}
...
function applyErrorCorrection({ rawPreds, actuals, initialCumulativeError = 0, delta }) {
    ...
    const effectiveDelta = Number.isFinite(delta) && delta > 0 ? delta : Number.POSITIVE_INFINITY;
    ...
    for (let idx = 0; idx < rawPreds.length; idx += 1) {
        ...
        if (Array.isArray(actuals) && Number.isFinite(actuals[idx])) {
            ...
            if (Math.abs(error) > thresholdBase * effectiveDelta) {
...
function runGeneticOptimization({ rawPreds, actuals }) {
    ...
    if (!Array.isArray(rawPreds) || rawPreds.length === 0 || !Array.isArray(actuals) || actuals.length !== rawPreds.length) {
        return { delta: minDelta, mse: NaN, fitness: 0, baselineMse: NaN, improved: false };
    }

    const baselineMse = computeMse(rawPreds, actuals);
    if (!Number.isFinite(baselineMse)) {
        return { delta: minDelta, mse: NaN, fitness: 0, baselineMse, improved: false };
    }
    ...
    const initialDelta = clampDelta(population[0]);
    const initialEvaluation = evaluate(initialDelta);
    let best = {
        delta: initialDelta,
        mse: initialEvaluation.mse,
        fitness: initialEvaluation.fitness,
    };

    const selectParent = (evaluations) => {
        const tournamentSize = 3;
        let bestIdx = 0;
        let bestFitness = -Infinity;
        for (let i = 0; i < tournamentSize; i += 1) {
            const candidateIndex = Math.floor(Math.random() * population.length);
            const candidateFitness = evaluations[candidateIndex]?.fitness ?? -Infinity;
    ...
    for (let generation = 0; generation < generations; generation += 1) {
        const evaluations = population.map((delta) => evaluate(delta));

        evaluations.forEach((evaluation, idx) => {
            if (evaluation.fitness > best.fitness) {
                best = {
                    delta: clampDelta(population[idx]),
                    mse: evaluation.mse,
        ...
        const nextPopulation = [best.delta];
        while (nextPopulation.length < populationSize) {
            const parentA = selectParent(evaluations);
            const parentB = selectParent(evaluations);
    ...
    return {
        ...
        baselineMse,
        improved: Number.isFinite(best.mse) && best.mse < baselineMse,
    };
}
...
function updateForecastMetrics(result) {
    ...
    if (mseEl) {
        const improvementText = Number.isFinite(metrics.mseImprovement)
            ? `，改善 ${formatNumber(metrics.mseImprovement, 2)}`
            : '';
        mseEl.textContent = `${formatNumber(metrics.mse, 2)} （基準 ${formatNumber(metrics.baselineMse, 2)}${improvementText}）`;
    }
...
function summariseForecastCompletion(result) {
    ...
    const mseImprovementText = Number.isFinite(result.metrics.mseImprovement)
        ? `（改善 ${formatNumber(result.metrics.mseImprovement, 2)})`
        : '';
    const summary = `完成預測：命中率 ${formatRatio(result.metrics.baselineHitRate)} → ${formatRatio(result.metrics.hitRate)}，MSE ${formatNumber(result.metrics.baselineMse, 2)} → ${formatNumber(result.metrics.mse, 2)} ${mseImprovementText}，耗時 ${formatNumber(durationSeconds, 1)} 秒。`;
    updateForecastStatus(summary.trim(), 'success');
}
...
async function runForecastWorkflow() {
    ...
    const trainingBaselineMse = computeMse(trainPredActuals, trainActuals);
    const fallbackFitness = Number.isFinite(trainingBaselineMse) ? 1 / (1 + trainingBaselineMse) : 0;
    const gaResult = trainPredActuals.length >= 5
        ? runGeneticOptimization({ rawPreds: trainPredActuals, actuals: trainActuals })
        : {
            delta: Number.NaN,
            mse: trainingBaselineMse,
            fitness: fallbackFitness,
            baselineMse: trainingBaselineMse,
            improved: false,
        };

    const resolvedTrainingBaselineMse = Number.isFinite(gaResult.baselineMse)
        ? gaResult.baselineMse
        : trainingBaselineMse;
    ...
    const trainingOptimisedMse = gaResult.mse;
    ...
    const mseImprovement = Number.isFinite(baselineMse) && Number.isFinite(mse)
        ? baselineMse - mse
        : NaN;
    ...
    return {
        ...
        correction: {
            ...
            trainingBaselineMse: resolvedTrainingBaselineMse,
            trainingOptimisedMse,
            bestFitness: gaResult.fitness,
            improved: gaResult.improved,
        },
        durationMs: 0,
        metrics: {
