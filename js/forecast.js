const FORECAST_VERSION_CODE = 'LB-FORECAST-LSTMGA-20251114B';
...
function describeCorrectionSummary(summary) {
    if (!summary || !Number.isFinite(summary.delta)) {
        return '尚未建立校正閥值。';
    }
    const parts = [`閥值 δ = ${formatRatio(summary.delta, 2)}`];
    if (Number.isFinite(summary.trainingCumulative)) {
        parts.push(`訓練累積誤差 ${formatNumber(summary.trainingCumulative, 4)}`);
    }
    if (Number.isFinite(summary.finalCumulative)) {
        parts.push(`測試期累積誤差 ${formatNumber(summary.finalCumulative, 4)}`);
    }
    if (Number.isFinite(summary.trainingTriggers)) {
        parts.push(`訓練觸發 ${summary.trainingTriggers} 次`);
    }
    if (Number.isFinite(summary.testTriggers)) {
        parts.push(`測試觸發 ${summary.testTriggers} 次`);
    }
    return parts.join(' ｜ ');
}
...
function computeMse(predicted, actuals) {
    ...
    return count > 0 ? sumSq / count : NaN;
}
...
function computeMae(predicted, actuals) {
    ...
    for (let idx = 0; idx < predicted.length; idx += 1) {
        ...
        const actual = actuals[idx];
        if (!Number.isFinite(pred) || !Number.isFinite(actual)) continue;
        sumAbs += Math.abs(pred - actual);
        count += 1;
    }
    return count > 0 ? sumAbs / count : NaN;
}

function applyThresholdCorrection(rawPreds, actuals, initialCumulative = 0, delta = 0.01) {
    if (!Array.isArray(rawPreds) || !Array.isArray(actuals) || rawPreds.length !== actuals.length) {
        return { corrected: Array.isArray(rawPreds) ? rawPreds.slice() : [], finalCumulative: initialCumulative, triggerCount: 0 };
    }
    const corrected = [];
    let cumulative = Number.isFinite(initialCumulative) ? initialCumulative : 0;
    let triggerCount = 0;
    for (let idx = 0; idx < rawPreds.length; idx += 1) {
        const basePred = rawPreds[idx];
        const actual = actuals[idx];
        if (!Number.isFinite(basePred) || !Number.isFinite(actual)) {
            corrected.push(Number.isFinite(basePred) ? basePred + cumulative : NaN);
            continue;
        }
        const correctedValue = basePred + cumulative;
        corrected.push(correctedValue);
        const residual = actual - correctedValue;
        const thresholdBasis = Math.max(Math.abs(actual), 1);
        const threshold = thresholdBasis * Math.max(delta, 0);
        if (Math.abs(residual) > threshold) {
            cumulative += residual;
            triggerCount += 1;
        }
    }
    return { corrected, finalCumulative: cumulative, triggerCount };
}

function runGeneticOptimization({ rawPreds, actuals, initialCumulative = 0 }) {
    if (!Array.isArray(rawPreds) || rawPreds.length === 0 || !Array.isArray(actuals) || actuals.length !== rawPreds.length) {
        return { delta: 0, score: NaN };
    }
    ...
    const randomDelta = () => Math.random() * (maxDelta - minDelta) + minDelta;
    ...
    const evaluate = (delta) => {
        const { corrected } = applyThresholdCorrection(rawPreds, actuals, initialCumulative, delta);
        const mse = computeMse(corrected, actuals);
        return Number.isFinite(mse) ? mse : Infinity;
    };

    const population = Array.from({ length: populationSize }, randomDelta);
    let bestDelta = population[0];
    let bestScore = evaluate(bestDelta);

    const selectParent = (scores) => {
        const tournamentSize = 3;
        let chosenDelta = population[0];
        ...
        for (let i = 0; i < tournamentSize; i += 1) {
            ...
            if (candidateScore < chosenScore) {
                ...
                chosenDelta = population[candidateIndex];
            }
        }
        return chosenDelta;
    };
    ...
    for (let generation = 0; generation < generations; generation += 1) {
        ...
        while (nextPopulation.length < populationSize) {
            ...
            nextPopulation.push(clamp(child));
        }
        population.splice(0, population.length, ...nextPopulation);
    }

    return { delta: clamp(bestDelta), score: bestScore };
}
...
function updateForecastMetrics(result) {
    ...
    const correctionEl = document.getElementById('forecastCorrectionSummary');
    ...
    if (correctionEl) correctionEl.textContent = describeCorrectionSummary(result.correction);
}
...
async function runForecastWorkflow() {
    ...
    const optimization = trainPredActuals.length >= 5
        ? runGeneticOptimization({ rawPreds: trainPredActuals, actuals: trainActuals })
        : { delta: 0, score: NaN };
    const delta = Number.isFinite(optimization.delta) ? optimization.delta : 0;

    const trainingCorrection = applyThresholdCorrection(trainPredActuals, trainActuals, 0, delta);
    ...
    const correctedResult = applyThresholdCorrection(
        testResult.predicted,
        testResult.actuals,
        trainingCorrection.finalCumulative,
        delta,
    );
    ...
    const correctionSummary = {
        delta,
        trainingCumulative: trainingCorrection.finalCumulative,
        finalCumulative: correctedResult.finalCumulative,
        trainingTriggers: trainingCorrection.triggerCount,
        testTriggers: correctedResult.triggerCount,
