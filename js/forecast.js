const FORECAST_VERSION_CODE = 'LB-FORECAST-LSTMGA-20251212A';
...
function describeCorrectionParameters(params) {
    if (!params || !Number.isFinite(params.delta)) {
        return '尚未建立校正參數。';
    }
    const segments = [`閾值 δ = ${formatNumber(params.delta, 4)}`];
    if (Number.isFinite(params.thresholdHits)) {
        segments.push(`觸發次數 ${params.thresholdHits}`);
    }
    if (Number.isFinite(params.finalCumulative)) {
        segments.push(`最終累積誤差 ${formatNumber(params.finalCumulative, 4)}`);
    }
...
function computeMse(predicted, actuals) {
    if (!Array.isArray(predicted) || !Array.isArray(actuals)) {
        return NaN;
    }
    let sumSq = 0;
    let count = 0;
...
function applyThresholdCorrection({ rawPreds, actuals, delta, initialCumulative = 0 }) {
    if (!Array.isArray(rawPreds) || !Array.isArray(actuals)) {
        return { corrected: [], mse: NaN, finalCumulative: initialCumulative, thresholdHits: 0 };
    }
    const corrected = [];
    ...
    for (let idx = 0; idx < rawPreds.length; idx += 1) {
        const predicted = rawPreds[idx];
        const actual = actuals[idx];
        if (!Number.isFinite(predicted)) {
            corrected.push(predicted);
            continue;
        }
        let adjusted = predicted + cumulative;
        if (!Number.isFinite(actual)) {
            corrected.push(adjusted);
            continue;
        }
        const error = actual - adjusted;
        const threshold = Math.abs(actual) * safeDelta;
        if (Number.isFinite(threshold) && Math.abs(error) > threshold) {
            cumulative += error;
            thresholdHits += 1;
            adjusted = predicted + cumulative;
        }
        corrected.push(adjusted);
    }
    const mse = computeMse(corrected, actuals);
    return { corrected, mse, finalCumulative: cumulative, thresholdHits };
}

function runDeltaOptimization({ rawPreds, actuals }) {
    if (!Array.isArray(rawPreds) || rawPreds.length === 0 || !Array.isArray(actuals) || actuals.length !== rawPreds.length) {
        return { delta: 0.02, mse: NaN };
    }
    ...
    const maxDelta = 0.25;
    const mutationScale = (maxDelta - minDelta) * 0.1;

    const randomDelta = () => minDelta + Math.random() * (maxDelta - minDelta);
    const clampDelta = (value) => {
        if (!Number.isFinite(value)) return randomDelta();
    ...
    const evaluate = (delta) => {
        const result = applyThresholdCorrection({ rawPreds, actuals, delta, initialCumulative: 0 });
        return Number.isFinite(result.mse) ? result.mse : Infinity;
    };
    ...
    const selectParent = (scores) => {
        ...
        for (let i = 0; i < tournamentSize; i += 1) {
            const pick = Math.floor(Math.random() * population.length);
            if (scores[pick] < candidateScore) {
                candidateIndex = pick;
                candidateScore = scores[pick];
            }
        }
...
function updateForecastMetrics(result) {
    ...
    const correctionEl = document.getElementById('forecastCorrectionInfo');
    ...
    if (correctionEl) correctionEl.textContent = describeCorrectionParameters(result.correction);
}
...
function summariseForecastCompletion(result) {
    ...
    const summary = `完成預測：命中率 ${formatRatio(result.metrics.baselineHitRate)} → ${formatRatio(result.metrics.hitRate)}，MSE ${formatNumber(result.metrics.baselineMse, 2)} → ${formatNumber(result.metrics.mse, 2)}，δ=${formatNumber(result.correction?.delta, 4)}，耗時 ${formatNumber(durationSeconds, 1)} 秒。`;
...
async function runForecastWorkflow() {
    ...
    const deltaResult = trainPredActuals.length >= 5
        ? runDeltaOptimization({ rawPreds: trainPredActuals, actuals: trainActuals })
        : { delta: 0.02, mse: NaN };

    const trainingCorrection = applyThresholdCorrection({
    ...
    const correctedResult = applyThresholdCorrection({
        ...
        delta: deltaResult.delta,
        initialCumulative: trainingCorrection.finalCumulative,
    });

    const mse = correctedResult.mse;
    ...
    const correctionSummary = {
        ...
        thresholdHits: (trainingCorrection.thresholdHits || 0) + (correctedResult.thresholdHits || 0),
    };
