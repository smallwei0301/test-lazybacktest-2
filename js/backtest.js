...
function buildSensitivityScoreAdvice(result) {
    ...
    if (averageDrift !== null) {
        if (averageDrift <= 5) {
            segments.push('平均漂移守在 ±5pp，穩得像練功掛網');
        } else if (averageDrift <= 12) {
    ...
    if (positiveDrift !== null || negativeDrift !== null) {
        ...
        if (Number.isFinite(dominantMagnitude)) {
            if (dominantMagnitude > 10) {
                segments.push(`${dominantDirection}方向平均偏移超過 10pp，該方向參數等於被掛上 Debuff，快排程調整`);
            } else if (dominantMagnitude > 6) {
                segments.push(`${dominantDirection}方向平均偏移落在 6～10pp，建議再做滾動驗證避免下個版本翻車`);
            } else if (Number.isFinite(oppositeMagnitude) && oppositeMagnitude <= 6 && dominantMagnitude <= 6) {
                segments.push('調高與調低方向平均偏移皆在 6pp 內，穩到可以邊刷副本邊調參');
            } else {
...
function displayBacktestResult(result) {
    ...
    const sensitivityHtml = (() => {
        ...
        const driftClass = (value) => {
            ...
            if (abs <= 12) return 'text-amber-500';
            return 'text-rose-600';
        };
        ...
        const directionAdvice = (() => {
            ...
            if (dominantAbs !== null && dominantAbs <= safeThreshold && (dominantDirection === '調高'
                ...
                directionSafeTooltip = '兩側平均偏移都在 ±6pp 內，方向調整相對安全。';
                return '兩側偏移都不大，照現在的節奏繼續跑即可。';
            }
            if (dominantAbs !== null && dominantAbs > warnThreshold) {
                return `${dominantDirection}側平均偏移超過 10pp，請優先針對該方向調整風控或做批量優化。`;
            }
            return `${dominantDirection}側平均偏移落在 6～10pp，建議多補樣本再做方向驗證。`;
        })();
        const summarySentence = (() => {
            ...
            if (stabilityScore !== null && stabilityScore >= 75 && (driftAbs === null || driftAbs <= 5) && (maxAbs === null || maxAbs <= 8)) {
                return '整體偏移小又穩，維持原參數觀察即可。';
            }
            if (stabilityScore !== null && stabilityScore >= 55 && (driftAbs === null || driftAbs <= 12)) {
                return '漂移開始放大，搭配分段風控或拉長觀察期會更安心。';
            }
            return '漂移明顯偏大（年化差異超過 12pp），先縮小部位並重新檢視參數設定。';
        })();
                ...
                <div class="p-6 rounded-xl border shadow-sm" style="background: linear-gradient(135deg, color-mix(in srgb, var(--secondary) 8%, var(--background)) 0%, color-mix(in srgb, var(--secondary) 4%, var(--background)) 100%); border-color: color-mix(in srgb, var(--secondary) 25%, transparent);">
                    <div class="flex flex-col items-center text-center gap-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="tooltip">
                                <span class="info-icon inline-flex items-center justify-center w-4 h-4 text-[10px] rounded-full cursor-help" style="background-color: var(--primary); color: var(--primary-foreground);">?</span>
                                <span class="tooltiptext tooltiptext--sensitivity">平均漂移 = 各擾動樣本的年化報酬差異絕對值平均。<br><strong>&le; 5%</strong>：偏移小，策略對參數變動較不敏感。<br><strong>5%～12%</strong>：建議延長樣本或用批量優化比對不同視窗。<br><strong>&gt; 12%</strong>：對參數相當敏感，需特別留意過擬合。</span>
                            </span>
                        </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <ul style="margin: 0; padding-left: 1.1rem; color: var(--muted-foreground); font-size: 12px; line-height: 1.6; list-style: disc;">
                            ...
                            <li><strong>偏移方向</strong>：比較調高（▲）與調低（▼）的平均 PP，雙側落在 ±6pp 內屬於常見穩健區間，超過 10pp 則建議針對該方向再驗證。</li>
