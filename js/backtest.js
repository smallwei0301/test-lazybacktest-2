...
function buildStrategyAdviceFlow(result = {}) {
    ...
    const returnRatioValue = Number.isFinite(health.returnRatio)
        ? Number(health.returnRatio)
        : null;
    if (returnRatioValue === null) {
        advice.push('報酬比分數缺資料，請補齊期間樣本。');
    } else if (returnRatioValue < 0.5) {
        advice.push('報酬比低於0.5，請啟用滾動驗證。');
    } else if (returnRatioValue > 1.5) {
        advice.push('報酬比高於1.5，請檢查是否過擬合。');
    } else {
        advice.push('報酬比接近1，維持並記錄持續性。');
    }

    const sharpeRatioValue = Number.isFinite(health.sharpeHalfRatio)
        ? Number(health.sharpeHalfRatio)
        : null;
    if (sharpeRatioValue === null) {
        advice.push('夏普比分數缺資料，請確認指標輸出。');
    } else if (sharpeRatioValue < 0.5) {
        advice.push('夏普比低於0.5，請強化風控機制。');
    } else if (sharpeRatioValue > 1.5) {
        advice.push('夏普比高於1.5，請確認是否過擬合。');
    } else {
        advice.push('夏普比接近1，持續追蹤風險調整。');
    }
...
function renderPerformanceAnalysis(result) {
    ...
    const baseResult = (result && typeof result === 'object' && Object.keys(result).length > 0)
        ? result
        : lastOverallResult;
    ...
    const appendPeriod = (key, label) => {
        ...
        const raw = Object.prototype.hasOwnProperty.call(subPeriodResults, key)
    ...
    const hasPeriodData = periodEntries.some((entry) => entry.data !== null);
    if (!hasPeriodData && !baseResult) {
    ...
    const bodyRows = periodEntries
        .map((entry) => {
            ...
            return `
                <tr>
                    <th scope="row" class="px-3 py-2 text-left font-medium" style="color: var(--foreground);">${escapeHtml(entry.label)}</th>
                    ${cells}
                </tr>`;
        })
        .join('');
    ...
    if (baseResult && typeof baseResult === 'object') {
        const overallAnnualized = baseResult.annualizedReturn;
        const overallReturn = baseResult.returnRate;
        let overallBuyHoldReturn = null;
        if (Array.isArray(baseResult.buyHoldReturns) && baseResult.buyHoldReturns.length > 0) {
            const last = Number(baseResult.buyHoldReturns[baseResult.buyHoldReturns.length - 1]);
            if (Number.isFinite(last)) overallBuyHoldReturn = last;
        } else if (Number.isFinite(baseResult.buyHoldReturn)) {
            overallBuyHoldReturn = Number(baseResult.buyHoldReturn);
        }
        const overallSharpe = baseResult.sharpeRatio;
        const overallSortino = baseResult.sortinoRatio;
        const overallMaxDrawdown = baseResult.maxDrawdown;

        const cells = [
            formatPercent(overallAnnualized, { showSign: true, positiveIsGood: true }),
            formatPercent(overallReturn, { showSign: true, positiveIsGood: true }),
            formatPercent(overallBuyHoldReturn, { showSign: true, positiveIsGood: true }),
            formatNumber(overallSharpe),
            formatNumber(overallSortino),
            formatPercent(overallMaxDrawdown, { showSign: false, positiveIsGood: false }),
        ]
            .map((html) => `<td class="px-3 py-2 text-center">${html}</td>`)
            .join('');
        overallRow = `
            <tr class="bg-muted/20">
                <th scope="row" class="px-3 py-2 text-left font-semibold" style="color: var(--foreground);">整體期間</th>
                ${cells}
            </tr>`;
    }
    ...
    const tableHtml = [
        ...
        `<tbody class="divide-y divide-border">${bodyRows}${overallRow}</tbody>`,
        '</table>',
        '</div>',
    ].join('');
