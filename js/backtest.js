...
function displayBacktestResult(result) {
    ...
    const exitKeyNormalised = normaliseStrategyIdForRole('exit', exitKeyRaw);
    const exitInternalKey = (strategyDescriptions && strategyDescriptions[exitKeyNormalised]) ? exitKeyNormalised : exitKeyRaw;
    const entryDesc = strategyDescriptions[entryKey] || { name: entryKey || result.entryStrategy || 'N/A', desc: 'N/A' };
    const exitDesc = strategyDescriptions[exitInternalKey] || { name: exitInternalKey || exitKeyNormalised || exitKeyRaw || 'N/A', desc: 'N/A' };
    let shortEntryDesc = null;
    let shortExitDesc = null;
    if (result.enableShorting && result.shortEntryStrategy && result.shortExitStrategy) {
        const shortEntryKey = normaliseStrategyIdForRole('shortEntry', result.shortEntryStrategy);
        const shortExitKey = normaliseStrategyIdForRole('shortExit', result.shortExitStrategy);
    ...
    const avgP = result.completedTrades?.length > 0 ? result.completedTrades.reduce((s, t) => s + (t.profit||0), 0) / result.completedTrades.length : 0;
    const maxCL = result.maxConsecutiveLosses || 0;
    const bhR = parseFloat(result.buyHoldReturns?.[result.buyHoldReturns.length - 1] ?? 0);
    const bhAnnR = result.buyHoldAnnualizedReturn ?? 0;
    const sharpe = result.sharpeRatio?.toFixed(2) ?? 'N/A';
    const sortino = result.sortinoRatio ? (isFinite(result.sortinoRatio) ? result.sortinoRatio.toFixed(2) : '∞') : 'N/A';
    const maxDD = result.maxDrawdown?.toFixed(2) ?? 0;
    const totalTrades = result.tradesCount ?? 0;
    const winTrades = result.winTrades ?? 0;
    const winR = totalTrades > 0 ? (winTrades / totalTrades * 100).toFixed(1) : 0;
    const totalProfit = result.totalProfit ?? 0;
    const returnRate = result.returnRate ?? 0;
    const annualizedReturn = result.annualizedReturn ?? 0;
    const finalValue = result.finalValue ?? result.initialCapital;
    const sensitivityData = result.sensitivityAnalysis ?? result.parameterSensitivity ?? result.sensitivityData ?? null;
    let annReturnRatioStr = 'N/A';
    let sharpeRatioStr = 'N/A';
    if (result.annReturnHalf1 !== null && result.annReturnHalf2 !== null && result.annReturnHalf1 !== 0) { annReturnRatioStr = (result.annReturnHalf2 / result.annReturnHalf1).toFixed(2); }
    if (result.sharpeHalf1 !== null && result.sharpeHalf2 !== null && result.sharpeHalf1 !== 0) { sharpeRatioStr = (result.sharpeHalf2 / result.sharpeHalf1).toFixed(2); }
    const overfittingTooltip = "將回測期間前後對半分，計算兩段各自的總報酬率與夏普值，再計算其比值 (後段/前段)。比值接近 1 較佳，代表策略績效在不同時期較穩定。一般認為 > 0.5 可接受。";
    let performanceHtml = `
        <div class="mb-8">
...
function runOptimizationInternal(optimizeType) { 
    ...
    if (isRiskOpt) { 
        ...
        if (optimizeType === 'entry') { 
            ...
            const normalisedExitKey = normaliseStrategyIdForRole('exit', targetStratKey);
            configKey = (strategyDescriptions && strategyDescriptions[normalisedExitKey]) ? normalisedExitKey : targetStratKey; 
...
function updateStrategyParams(type) {
    ...
    const normalisedKey = normaliseStrategyIdForRole(type, strategyKey);
    if (normalisedKey && strategyDescriptions?.[normalisedKey]) {
        internalKey = normalisedKey;
    } else if (type === 'shortEntry') {
        const fallbackKey = `short_${strategyKey}`;
        if (strategyDescriptions?.[fallbackKey]) {
            internalKey = fallbackKey;
        }
    } else if (type === 'shortExit') {
        const fallbackKey = `cover_${strategyKey}`;
        if (strategyDescriptions?.[fallbackKey]) {
            internalKey = fallbackKey;
        }
    }
...
const STRATEGY_ROLE_LEGACY_ID_MAP = Object.freeze({
    exit: {
...
function normaliseStrategyIdForRole(roleType, rawId) {
    if (!rawId) {
        return rawId;
    }
    const roleMap = STRATEGY_ROLE_LEGACY_ID_MAP[roleType];
    if (roleMap && Object.prototype.hasOwnProperty.call(roleMap, rawId)) {
        return roleMap[rawId];
    }
    return rawId;
}
...
function normaliseStrategySettingsFromStorage(settings) {
    ...
    const entryStrategyId = normaliseStrategyIdForRole('entry', settings.entryStrategy);
    if (entryStrategyId && entryStrategyId !== settings.entryStrategy) {
        updated.entryStrategy = entryStrategyId;
        changed = true;
    }
    const exitStrategyId = normaliseStrategyIdForRole('exit', settings.exitStrategy || 'ma_cross_exit') || 'ma_cross_exit';
    if (exitStrategyId !== settings.exitStrategy) {
        updated.exitStrategy = exitStrategyId;
        changed = true;
    }
    const shortEntryStrategyId = normaliseStrategyIdForRole('shortEntry', settings.shortEntryStrategy);
    if (shortEntryStrategyId && shortEntryStrategyId !== settings.shortEntryStrategy) {
        updated.shortEntryStrategy = shortEntryStrategyId;
        changed = true;
    }
    const shortExitStrategyId = normaliseStrategyIdForRole('shortExit', settings.shortExitStrategy);
    if (shortExitStrategyId && shortExitStrategyId !== settings.shortExitStrategy) {
        updated.shortExitStrategy = shortExitStrategyId;
        changed = true;
    }

    return { settings: updated, changed };
}
function getSavedStrategies() { const strategies = localStorage.getItem(SAVED_STRATEGIES_KEY); try { const parsed = strategies ? JSON.parse(strategies) : {}; const cleaned = {}; let updated = false; for (const [name, data] of Object.entries(parsed)) { if (data && typeof data === 'object' && data.settings) { const normalised = normaliseStrategySettingsFromStorage(data.settings); const settings = normalised.settings; if (normalised.changed) { updated = true; } cleaned[name] = { ...data, settings }; } else { console.warn(`[Storage] Removing corrupted strategy: ${name}`, data); updated = true; } } if (updated || Object.keys(cleaned).length !== Object.keys(parsed).length) { localStorage.setItem(SAVED_STRATEGIES_KEY, JSON.stringify(cleaned)); } return cleaned; } catch (e) { console.error("讀取策略時解析JSON錯誤:", e); return {}; } }
...
function saveStrategy() { 
    ...
    const exitStrategy = normaliseStrategyIdForRole('exit', rawExitStrategy) || rawExitStrategy;
    ...
    if (enableShorting) {
        ...
        const shortEntryStrategy = normaliseStrategyIdForRole('shortEntry', rawShortEntryStrategy) || rawShortEntryStrategy;
        const rawShortExitStrategy = document.getElementById('shortExitStrategy').value;
        const shortExitStrategy = normaliseStrategyIdForRole('shortExit', rawShortExitStrategy) || rawShortExitStrategy;
...
function loadStrategy() { const selectElement = document.getElementById('loadStrategySelect'); const strategyName = selectElement.value; if (!strategyName) { showInfo("請先從下拉選單選擇要載入的策略。"); return; } const strategies = getSavedStrategies(); const strategyData = strategies[strategyName]; if (!strategyData || !strategyData.settings) { showError(`載入策略 "${strategyName}" 失敗：找不到策略數據。`); return; } let settings = strategyData.settings; console.log(`[Main] Loading strategy: ${strategyName}`, settings); try {
        ...
        const exitModeSelect = document.getElementById('exitStagingMode');
        if (exitModeSelect) exitModeSelect.value = settings.exitStagingMode || 'signal_repeat'; document.getElementById('stopLoss').value = settings.stopLoss ?? 0; document.getElementById('takeProfit').value = settings.takeProfit ?? 0; const positionBasisInput = document.querySelector(`input[name="positionBasis"][value="${settings.positionBasis || 'initialCapital'}"]`); if (positionBasisInput) positionBasisInput.checked = true; document.getElementById('entryStrategy').value = settings.entryStrategy || 'ma_cross'; updateStrategyParams('entry'); if(settings.entryParams) { for (const pName in settings.entryParams) { let idSfx = pName.charAt(0).toUpperCase() + pName.slice(1); let finalIdSfx = idSfx; if (settings.entryStrategy === 'k_d_cross' && pName === 'thresholdX') finalIdSfx = 'KdThresholdX'; else if ((settings.entryStrategy === 'macd_cross') && pName === 'signalPeriod') finalIdSfx = 'SignalPeriod'; const inputElement = document.getElementById(`entry${finalIdSfx}`); if (inputElement) inputElement.value = settings.entryParams[pName]; else console.warn(`[Load] Entry Param Input not found: entry${finalIdSfx}`); } } const exitStrategyId = normaliseStrategyIdForRole('exit', settings.exitStrategy || 'ma_cross_exit') || 'ma_cross_exit'; settings.exitStrategy = exitStrategyId; document.getElementById('exitStrategy').value = exitStrategyId; updateStrategyParams('exit'); if (settings.exitParams) { const exitInternalKey = normaliseStrategyIdForRole('exit', exitStrategyId); for (const pName in settings.exitParams) { let idSfx = pName.charAt(0).toUpperCase() + pName.slice(1); let finalIdSfx = idSfx; if (exitInternalKey === 'k_d_cross_exit' && pName === 'thresholdY') finalIdSfx = 'KdThresholdY'; else if (exitInternalKey === 'turtle_stop_loss' && pName === 'stopLossPeriod') finalIdSfx = 'StopLossPeriod'; else if (exitInternalKey === 'macd_cross_exit' && pName === 'signalPeriod') finalIdSfx = 'SignalPeriod'; const inputElement = document.getElementById(`exit${finalIdSfx}`); if (inputElement) inputElement.value = settings.exitParams[pName]; else console.warn(`[Load] Exit Param Input not found: exit${finalIdSfx}`); } } const shortCheckbox = document.getElementById('enableShortSelling'); const shortArea = document.getElementById('short-strategy-area'); shortCheckbox.checked = settings.enableShorting || false; shortArea.style.display = shortCheckbox.checked ? 'grid' : 'none'; if (settings.enableShorting) { const shortEntryStrategyId = normaliseStrategyIdForRole('shortEntry', settings.shortEntryStrategy || 'short_ma_cross') || 'short_ma_cross'; settings.shortEntryStrategy = shortEntryStrategyId; document.getElementById('shortEntryStrategy').value = shortEntryStrategyId; updateStrategyParams('shortEntry'); if (settings.shortEntryParams) { const shortEntryInternalKey = normaliseStrategyIdForRole('shortEntry', shortEntryStrategyId); for (const pName in settings.shortEntryParams) { let idSfx = pName.charAt(0).toUpperCase() + pName.slice(1); let finalIdSfx = idSfx; if (shortEntryInternalKey === 'short_k_d_cross' && pName === 'thresholdY') finalIdSfx = 'ShortKdThresholdY'; else if (shortEntryInternalKey === 'short_macd_cross' && pName === 'signalPeriod') finalIdSfx = 'ShortSignalPeriod'; else if (shortEntryInternalKey === 'short_turtle_stop_loss' && pName === 'stopLossPeriod') finalIdSfx = 'ShortStopLossPeriod'; const inputElement = document.getElementById(`shortEntry${finalIdSfx}`); if (inputElement) inputElement.value = settings.shortEntryParams[pName]; else console.warn(`[Load] Short Entry Param Input not found: shortEntry${finalIdSfx}`); } } const shortExitStrategyId = normaliseStrategyIdForRole('shortExit', settings.shortExitStrategy || 'cover_ma_cross') || 'cover_ma_cross'; settings.shortExitStrategy = shortExitStrategyId; document.getElementById('shortExitStrategy').value = shortExitStrategyId; updateStrategyParams('shortExit'); if (settings.shortExitParams) { const shortExitInternalKey = normaliseStrategyIdForRole('shortExit', shortExitStrategyId); for (const pName in settings.shortExitParams) { let idSfx = pName.charAt(0).toUpperCase() + pName.slice(1); let finalIdSfx = idSfx; if (shortExitInternalKey === 'cover_k_d_cross' && pName === 'thresholdX') finalIdSfx = 'CoverKdThresholdX'; else if (shortExitInternalKey === 'cover_macd_cross' && pName === 'signalPeriod') finalIdSfx = 'CoverSignalPeriod'; else if (shortExitInternalKey === 'cover_turtle_breakout' && pName === 'breakoutPeriod') finalIdSfx = 'CoverBreakoutPeriod'; else if (shortExitInternalKey === 'cover_trailing_stop' && pName === 'percentage') finalIdSfx = 'CoverTrailingStopPercentage'; const inputElement = document.getElementById(`shortExit${finalIdSfx}`); if (inputElement) inputElement.value = settings.shortExitParams[pName]; else console.warn(`[Load] Short Exit Param Input not found: shortExit${finalIdSfx}`); } } } else { document.getElementById('shortEntryStrategy').value = 'short_ma_cross'; updateStrategyParams('shortEntry'); document.getElementById('shortExitStrategy').value = 'cover_ma_cross'; updateStrategyParams('shortExit'); } showSuccess(`策略 "${strategyName}" 已載入！`); 
