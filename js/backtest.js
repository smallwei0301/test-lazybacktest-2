...
function displayBacktestResult(result) {
    ...
    const sensitivityHtml = (() => {
        ...
        const stabilityTooltipLines = [
            '穩定度分數 = 100 − 漂移懲罰 − Sharpe 下滑懲罰（平均下滑 × 100，上限 40 分）。',
            `漂移懲罰依年化漂移門檻分段：≤ ${driftStable}pp 最高扣約 12 分，落在 ${driftStable}～${driftCaution}pp 會放大到 30 分，超過 ${driftCaution}pp 會往 45 分上限推進並視超標幅度持續加重。`,
