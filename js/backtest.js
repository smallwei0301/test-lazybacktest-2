...
const STRATEGY_DSL_ROLE_MAP_FOR_LOAD = Object.freeze({
    entry: 'longEntry',
    exit: 'longExit',
...
function cloneStrategyDslForStorage(strategyDsl) {
    if (!strategyDsl || typeof strategyDsl !== 'object') {
        return null;
    }
    try {
        return JSON.parse(JSON.stringify(strategyDsl));
    } catch (error) {
        console.warn('[Storage] 無法序列化策略 DSL，已略過', error);
        return null;
    }
}
    ...
    try {
        ...
        strategies[name] = {
            settings: {
                ...
                strategyDsl: cloneStrategyDslForStorage(settings.strategyDsl),
            },
            metrics: metrics,
...
// Patch Tag: LB-STRATEGY-DSL-LOAD-20261005A
function resolveStrategyDslFromSettings(settings) {
    if (!settings || typeof settings !== "object") {
        return null;
    }
    const storedDsl = settings.strategyDsl;
    if (storedDsl && typeof storedDsl === "object") {
        return storedDsl;
    }
    const dslBuilder = typeof window !== "undefined" ? window.lazybacktestStrategyDsl : null;
    const buildFromParams = dslBuilder && typeof dslBuilder.buildFromParams === "function"
        ? dslBuilder.buildFromParams
        : null;
    if (!buildFromParams) {
        return null;
    }
    const selection = {
        ...
        exitStrategy: settings.exitStrategy,
        exitParams: settings.exitParams,
        enableShorting: Boolean(settings.enableShorting),
    };
    if (selection.enableShorting) {
        selection.shortEntryStrategy = settings.shortEntryStrategy;
        selection.shortEntryParams = settings.shortEntryParams;
        selection.shortExitStrategy = settings.shortExitStrategy;
        selection.shortExitParams = settings.shortExitParams;
    }
    return buildFromParams(selection);
}

function applyStrategyDslControllers(definition, options = {}) {
    const controllers = typeof window !== "undefined" ? window.lazybacktestStrategyDslControllers : null;
    if (!controllers || typeof controllers !== "object") {
        return false;
    }
    const enableShorting = Boolean(options.enableShorting);
    const map = STRATEGY_DSL_ROLE_MAP_FOR_LOAD || {};
    const dsl = definition && typeof definition === "object" ? definition : null;
    let applied = false;

    Object.keys(map).forEach((type) => {
        const controller = controllers[type];
        if (!controller || typeof controller.applyDsl !== "function") {
            return;
        }
        const isShortType = type === "shortEntry" || type === "shortExit";
        if (isShortType && !enableShorting) {
            try {
                controller.applyDsl(null);
            } catch (error) {
                console.warn(`[StrategyDSL] 重置 ${type} 控制器失敗`, error);
            }
            return;
        }
        const roleKey = map[type];
        const nodeDefinition = dsl && roleKey && dsl[roleKey] ? dsl[roleKey] : null;
        try {
            controller.applyDsl(nodeDefinition || null);
            if (nodeDefinition) {
                applied = true;
            }
        } catch (error) {
            console.warn(`[StrategyDSL] 套用 ${type} DSL 失敗`, error);
            try {
                controller.applyDsl(null);
            } catch (fallbackError) {
                console.warn(`[StrategyDSL] ${type} DSL 回退失敗`, fallbackError);
            }
        }
    });
...
function loadStrategy() {
    ...
    try {
        ...
        if (initialCapitalInput) {
            initialCapitalInput.value = settings.initialCapital || 100000;
        }

        const positionSizeInput = document.getElementById("positionSize");
        if (positionSizeInput) {
            positionSizeInput.value = settings.positionSize || 100;
        }

        const recentYearsInput = document.getElementById("recentYears");
        if (recentYearsInput) {
            recentYearsInput.value = 5;
        }
        const tradeTimingValue = settings.tradeTiming || "close";
        const tradeTimingInput = document.querySelector(`input[name="tradeTiming"][value="${tradeTimingValue}"]`);
        if (tradeTimingInput) {
            tradeTimingInput.checked = true;
        }

        const buyFeeInput = document.getElementById("buyFee");
        if (buyFeeInput) {
            buyFeeInput.value = (settings.buyFee !== undefined)
                ? settings.buyFee
                : (buyFeeInput.value || 0.1425);
        }
        const sellFeeInput = document.getElementById("sellFee");
        if (sellFeeInput) {
            sellFeeInput.value = (settings.sellFee !== undefined)
                ? settings.sellFee
        ...
        const entryModeSelect = document.getElementById("entryStagingMode");
        if (entryModeSelect) {
            entryModeSelect.value = settings.entryStagingMode || "signal_repeat";
        }
        ...
        const exitModeSelect = document.getElementById("exitStagingMode");
        if (exitModeSelect) {
            exitModeSelect.value = settings.exitStagingMode || "signal_repeat";
        }
        const stopLossInput = document.getElementById("stopLoss");
        if (stopLossInput) {
            stopLossInput.value = settings.stopLoss ?? 0;
        }
        const takeProfitInput = document.getElementById("takeProfit");
        if (takeProfitInput) {
            takeProfitInput.value = settings.takeProfit ?? 0;
        }

        const positionBasisValue = settings.positionBasis || "initialCapital";
        const positionBasisInput = document.querySelector(`input[name="positionBasis"][value="${positionBasisValue}"]`);
        if (positionBasisInput) {
            positionBasisInput.checked = true;
        }

        const entrySelect = document.getElementById("entryStrategy");
        if (entrySelect) {
            const normalized = normaliseStrategyIdForRole("entry", settings.entryStrategy);
            if (normalized) {
                entrySelect.value = normalized;
            } else if (settings.entryStrategy) {
                entrySelect.value = settings.entryStrategy;
            }
        }

        const exitSelect = document.getElementById("exitStrategy");
        if (exitSelect) {
            const normalized = normaliseStrategyIdForRole("exit", settings.exitStrategy);
            if (normalized) {
                exitSelect.value = normalized;
            } else if (settings.exitStrategy) {
                exitSelect.value = settings.exitStrategy;
            }
        }

        const shortEntrySelect = document.getElementById("shortEntryStrategy");
        if (shortEntrySelect) {
            const normalized = normaliseStrategyIdForRole("shortEntry", settings.shortEntryStrategy);
            if (normalized) {
                shortEntrySelect.value = normalized;
            } else if (settings.shortEntryStrategy) {
                shortEntrySelect.value = settings.shortEntryStrategy;
            }
        }

        const shortExitSelect = document.getElementById("shortExitStrategy");
        if (shortExitSelect) {
            const normalized = normaliseStrategyIdForRole("shortExit", settings.shortExitStrategy);
            if (normalized) {
                shortExitSelect.value = normalized;
            } else if (settings.shortExitStrategy) {
                shortExitSelect.value = settings.shortExitStrategy;
            }
        }

        const shortCheckbox = document.getElementById("enableShortSelling");
        const shortArea = document.getElementById("short-strategy-area");
        const enableShorting = Boolean(settings.enableShorting);
        if (shortCheckbox) {
            shortCheckbox.checked = enableShorting;
        }
        if (shortArea) {
            shortArea.classList.toggle("hidden", !enableShorting);
        }

        const dslDefinition = resolveStrategyDslFromSettings(settings);
        const appliedDsl = applyStrategyDslControllers(dslDefinition, { enableShorting });
        if (!appliedDsl && typeof updateStrategyParams === "function") {
            updateStrategyParams("entry", { force: true });
            updateStrategyParams("exit", { force: true });
            if (enableShorting) {
                updateStrategyParams("shortEntry", { force: true });
                updateStrategyParams("shortExit", { force: true });
            }
        }
