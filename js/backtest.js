...
function resolveStrategyLookupKey(strategyId, roleHint) {
    ...
    if (helper && typeof helper.resolveRoleAlias === 'function') {
        const aliasInfo = helper.resolveRoleAlias(strategyId, roleHint, strategyDescriptions);
        if (aliasInfo && aliasInfo.normalized) {
            candidates.push(aliasInfo.normalized);
        }
    }
    if (!candidates.includes(strategyId)) {
        candidates.push(strategyId);
    }

    const tryRole = (role, id) => {
        if (!role || typeof normaliseStrategyIdForRole !== 'function') {
            return null;
        }
        const migrated = normaliseStrategyIdForRole(role, id);
    ...
    const attemptResolve = (id) => {
        if (!id) return null;

        const primary = tryRole(roleHint, id);
        if (primary && strategyDescriptions?.[primary]) {
            return primary;
        }

        if (strategyDescriptions?.[id]) {
            return id;
        }
        ...
        for (const role of rolesToTry) {
            ...
            const migrated = tryRole(role, id);
            if (migrated && strategyDescriptions?.[migrated]) {
                return migrated;
            }
        }
        ...
        if (typeof normaliseStrategyIdAny === 'function') {
            const fallback = normaliseStrategyIdAny(id);
    ...
    for (const candidate of candidates) {
        const resolved = attemptResolve(candidate);
        if (resolved) {
            return resolved;
        }
    }
