...
function normaliseStrategyIdAny(strategyId, roleHint) {
    if (!strategyId) return strategyId;
    ...
    if (typeof window !== 'undefined' && window.LazyStrategyId) {
        if (typeof window.LazyStrategyId.normaliseAny === 'function') {
            return window.LazyStrategyId.normaliseAny(strategyId, hint);
        }
    }
    if (hint) {
        return normaliseStrategyIdForRole(hint, strategyId);
    }
    if (strategyDescriptions?.[strategyId]) {
        return strategyId;
    }
    for (const role of ['exit', 'shortEntry', 'shortExit']) {
...
function resolveStrategyDisplayName(key, role) {
    if (!key) return '';
    const normalized = normaliseStrategyIdAny(key, role);
    if (normalized && strategyDescriptions?.[normalized]?.name) {
        return strategyDescriptions[normalized].name;
    }
    const direct = strategyDescriptions?.[key];
    if (direct?.name) return direct.name;
    if (!role) {
        for (const fallbackRole of ['exit', 'shortEntry', 'shortExit']) {
            const migrated = normaliseStrategyIdAny(key, fallbackRole);
            if (migrated !== key && strategyDescriptions?.[migrated]?.name) {
                return strategyDescriptions[migrated].name;
            }
        }
    }
...
function displayBacktestResult(result) {
    ...
    const entryKey = normaliseStrategyIdAny(result.entryStrategy, 'entry');
    const exitKey = normaliseStrategyIdForRole('exit', result.exitStrategy);
    const entryDesc = strategyDescriptions[entryKey] || { name: result.entryStrategy || 'N/A', desc: 'N/A' };
    const exitDesc = strategyDescriptions[exitKey] || { name: result.exitStrategy || 'N/A', desc: 'N/A' };
    ...
    if (result.enableShorting && result.shortEntryStrategy && result.shortExitStrategy) {
        ...
        shortEntryDesc = strategyDescriptions[shortEntryKey] || { name: result.shortEntryStrategy, desc: 'N/A' };
        shortExitDesc = strategyDescriptions[shortExitKey] || { name: result.shortExitStrategy, desc: 'N/A' };
    }
...
function saveStrategy() { 
    ...
    const entryStrategy = normaliseStrategyIdAny(entryStrategyRaw, 'entry') || entryStrategyRaw;
