...
// Patch Tag: LB-RANDOM-GUARD-20250629A
// 確保 zoom 插件正確註冊
...
function randomizeSettings() {
    ...
    const getRandomValue = (min, max, step) => {
        ...
        const steps = Math.max(0, Math.floor(range / step));
    ...
    const resolveFallbackValue = (selectEl) => {
        if (!selectEl) return '';
    ...
    const pickStrategyWithFallback = (label, candidates, fallbackKey) => {
        const available = Array.isArray(candidates)
            ? candidates.filter((key) => typeof key === 'string' && key.trim() !== '')
            : [];
        if (available.length === 0) {
            if (fallbackKey) {
                console.warn(`[Random] ${label} 策略清單為空，沿用 ${fallbackKey}`);
                return fallbackKey;
            }
            console.error(`[Random] ${label} 策略清單為空，請檢查策略配置`);
            return '';
        }
        const candidate = getRandomElement(available);
        if (candidate) return candidate;
        if (fallbackKey && available.includes(fallbackKey)) {
            console.warn(`[Random] ${label} 隨機選項無效，改用 ${fallbackKey}`);
            return fallbackKey;
        }
        const fallback = available[0];
        console.warn(`[Random] ${label} 隨機選項無效，改用 ${fallback}`);
        return fallback;
    };

    const allKeys = Object.keys(strategyDescriptions);
    const entryKeys = allKeys.filter(
        (k) => !k.startsWith('short_') && !k.startsWith('cover_') && !k.endsWith('_exit') && k !== 'fixed_stop_loss'
    );
    const exitKeysRaw = allKeys.filter(
        (k) =>
            (k.endsWith('_exit') ||
                [
                    'ma_below',
    ...
    const exitKeys = exitKeysRaw.map((k) => k.replace('_exit', '')).filter((k) => k !== 'fixed_stop_loss');
    const shortEntryKeys = allKeys.filter((k) => k.startsWith('short_') && k !== 'short_fixed_stop_loss');
    const coverKeys = allKeys.filter((k) => k.startsWith('cover_') && k !== 'cover_fixed_stop_loss');
    ...
    if (!entrySelect || !exitSelect) {
        console.error('[Random] 找不到進出場策略選單');
        showError('找不到進出場策略選單，請重新整理頁面。');
        return;
    }

    const randomEntryKey = pickStrategyWithFallback('做多進場', entryKeys, resolveFallbackValue(entrySelect));
    const randomExitKey = pickStrategyWithFallback('做多出場', exitKeys, resolveFallbackValue(exitSelect));

    if (!randomEntryKey || !randomExitKey) {
        showError('目前缺少可用的做多進出場策略，請稍後再試。');
        return;
    }

    const setRandomParams = (type, strategyKey) => {
        let internalKey = strategyKey;
        ...
        const config = strategyDescriptions[internalKey];
        if (!config || !config.defaultParams) return;
        const params = {};
        for (const pName in config.defaultParams) {
            const target = config.optimizeTargets?.find((t) => t.name === pName);
            let randomVal;
            if (target?.range) {
                randomVal = getRandomValue(target.range.from, target.range.to, target.range.step);
            } else {
                if (pName.includes('Period') || pName.includes('period')) randomVal = getRandomValue(5, 100, 1);
                else if (pName === 'threshold' && internalKey.includes('rsi')) randomVal = getRandomValue(10, 90, 1);
                else if (pName === 'threshold' && internalKey.includes('williams')) randomVal = getRandomValue(-90, -10, 1);
                else if (pName === 'thresholdX' || pName === 'thresholdY') randomVal = getRandomValue(10, 90, 1);
                else if (pName === 'deviations') randomVal = getRandomValue(1, 3, 0.1);
                else if (pName === 'multiplier') randomVal = getRandomValue(1.5, 5, 0.1);
                else if (pName === 'percentage') randomVal = getRandomValue(1, 25, 0.5);
                else randomVal = config.defaultParams[pName];
            }
            params[pName] = randomVal;
        }
        if (
            ['ma_cross', 'ema_cross', 'short_ma_cross', 'short_ema_cross', 'cover_ma_cross', 'cover_ema_cross'].some((prefix) =>
                internalKey.startsWith(prefix)
            )
        ) {
            if (params.shortPeriod && params.longPeriod && params.shortPeriod >= params.longPeriod) {
                params.shortPeriod = getRandomValue(3, Math.max(4, params.longPeriod - 1), 1);
                console.log(`[Random] Adjusted ${type} shortPeriod to ${params.shortPeriod} (long: ${params.longPeriod})`);
            }
        }
        for (const pName in params) {
            ...
            if (inputEl) {
                inputEl.value = params[pName];
            } else {
                console.warn(`[Random] Input element not found for ${type} - ${pName}: #${inputId}`);
            }
        }
    };

    entrySelect.value = randomEntryKey;
    exitSelect.value = randomExitKey;
    updateStrategyParams('entry');
    updateStrategyParams('exit');
    setRandomParams('entry', randomEntryKey);
    setRandomParams('exit', randomExitKey);
    ...
    if (shortCheckbox?.checked) {
        ...
        if (!shortEntrySelect || !shortExitSelect) {
            console.error('[Random] 找不到做空策略選單');
            showError('找不到做空策略選單，請重新整理頁面。');
            return;
        }
        const randomShortEntryKey = pickStrategyWithFallback(
            '做空進場',
            shortEntryKeys,
            resolveFallbackValue(shortEntrySelect)
        );
        const randomCoverKey = pickStrategyWithFallback('回補出場', coverKeys, resolveFallbackValue(shortExitSelect));
        if (!randomShortEntryKey || !randomCoverKey) {
            showError('目前缺少可用的做空進出場策略，請稍後再試。');
            return;
        }
        shortEntrySelect.value = randomShortEntryKey;
        shortExitSelect.value = randomCoverKey;
        updateStrategyParams('shortEntry');
        updateStrategyParams('shortExit');
        setRandomParams('shortEntry', randomShortEntryKey.replace('short_', ''));
        setRandomParams('shortExit', randomCoverKey.replace('cover_', ''));
    }
