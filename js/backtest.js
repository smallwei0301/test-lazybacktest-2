...
// Patch Tag: LB-PERFORMANCE-TABLE-20260815A
// Patch Tag: LB-STRATEGY-OVERFIT-20260815A
...
function buildStrategyAdviceFlow(result = {}) {
    ...
    if (Number.isFinite(health.returnRatio)) {
        const ratioText = formatRatio(health.returnRatio);
        const ratioGap = Math.abs(health.returnRatio - 1);
        if (ratioText) {
            if (ratioGap > 0.5) {
                advice.push(`報酬前後段比 ${ratioText}，請進行滾動驗證檢查。`);
            } else {
                advice.push(`報酬前後段比 ${ratioText}，維持現有驗證節奏。`);
            }
        }
    }
    if (Number.isFinite(health.sharpeHalfRatio)) {
        const sharpeText = formatRatio(health.sharpeHalfRatio);
        const sharpeGap = Math.abs(health.sharpeHalfRatio - 1);
        if (sharpeText) {
            if (sharpeGap > 0.5) {
                advice.push(`夏普前後段比 ${sharpeText}，請增加樣本與風控。`);
            } else {
                advice.push(`夏普前後段比 ${sharpeText}，風險表現維持穩定。`);
            }
        }
    }
...
function renderPerformanceAnalysis(result) {
    ...
    baseTimeframes.forEach((frame) => {
        const entry = Object.prototype.hasOwnProperty.call(subPeriodResults, frame.key)
            ? subPeriodResults[frame.key]
            : null;
        timeframeEntries.push({ key: frame.key, label: frame.label, data: entry && typeof entry === 'object' ? entry : null });
    });
    for (let year = 1; year <= yearsSetting; year += 1) {
        ...
        const entry = Object.prototype.hasOwnProperty.call(subPeriodResults, key)
            ...
            : null;
        timeframeEntries.push({ key, label, data: entry && typeof entry === 'object' ? entry : null });
    }

    const hasResult = timeframeEntries.some((entry) => entry.data !== null);
    if (!hasResult) {
    ...
    const headerCells = metrics
        ...
        .join('');
    const bodyRows = timeframeEntries
        .map((metric) => {
            const cells = metrics
                .map((column) => {
                    const raw = metric.data ? metric.data[column.key] : null;
                    return `<td class="px-3 py-2 text-center">${column.formatter(raw)}</td>`;
                })
                .join('');
            return `
                <tr>
                    <th scope="row" class="px-3 py-2 text-left font-medium whitespace-nowrap" style="color: var(--foreground);">${escapeHtml(metric.label)}</th>
                    ${cells}
                </tr>`;
        })
        .join('');
