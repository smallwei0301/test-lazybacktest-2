...
function updateStrategyParams(type) {
    ...
    const formLib = (typeof window !== 'undefined' && window.LazyStrategyParamsForm)
        ...
        : null;
    const registry = (typeof window !== 'undefined' && window.StrategyPluginRegistry)
        ? window.StrategyPluginRegistry
        : null;
    const meta = registry && typeof registry.getStrategyMetaById === 'function'
        ? registry.getStrategyMetaById(internalKey)
        : null;
    ...
    let initialValues = {};
    if (formLib) {
        fields = formLib.buildFieldDescriptors(schema, { role: type, strategyId: internalKey });
        initialValues = formLib.buildInitialValues(schema, defaults);
        if (fields.length === 0 && Object.keys(defaults).length > 0) {
            const fallbackSchema = { type: 'object', properties: {} };
            Object.keys(defaults).forEach((key) => {
                const value = defaults[key];
                const valueType = typeof value === 'boolean' ? 'boolean' : (Number.isFinite(Number(value)) ? 'number' : 'string');
                fallbackSchema.properties[key] = { type: valueType, default: value };
            });
            fields = formLib.buildFieldDescriptors(fallbackSchema, { role: type, strategyId: internalKey });
            initialValues = formLib.buildInitialValues(fallbackSchema, defaults);
        }
    }

    if (!formLib || fields.length === 0) {
        paramsContainer.innerHTML = '<p class="text-xs text-gray-400 italic">此策略無需參數</p>';
    } else {
        formLib.renderFields(paramsContainer, fields, initialValues);
        if (config?.optimizeTargets) {
            config.optimizeTargets.forEach((target) => {
                const field = fields.find((entry) => entry.name === target.name);
                if (!field || !target?.range) return;
                const label = paramsContainer.querySelector(`label[for="${field.inputId}"]`);
                if (!label) return;
                const hint = document.createElement('span');
                hint.className = 'ml-2 text-[11px] text-blue-500';
                hint.textContent = `範圍: ${target.range.from}-${target.range.to}`;
    ...
    if (typeof window !== 'undefined') {
        const store = window.lazybacktestStrategyForms || {};
        store[type] = {
            fields,
            schema,
        ...
        window.lazybacktestStrategyForms = store;
    }
    ...
    if (typeof window !== 'undefined' && window.lazybacktestDslEditorUi && typeof window.lazybacktestDslEditorUi.syncRoleFromSelect === 'function') {
        window.lazybacktestDslEditorUi.syncRoleFromSelect(type, internalKey);
    }
}
