...
function resolveWorkerCachePayload(params, options = {}) {
    ...
    let rangeFailureReason = 'rangeNotCovered';
    ...
    if (!fetchRequired) {
        if (Number.isFinite(targetStart) && Number.isFinite(lastStart) && lastStart > targetStart) {
            fetchRequired = true;
            rangeFailureReason = 'lastFetchStartsTooLate';
        }
        if (!fetchRequired && Number.isFinite(targetEnd) && Number.isFinite(lastEnd) && lastEnd < targetEnd) {
            fetchRequired = true;
            rangeFailureReason = 'lastFetchEndsTooEarly';
        }
    }

    if (!fetchRequired && hasGlobalCache && Number.isFinite(targetStart) && Number.isFinite(targetEnd)) {
        let datasetStart = Number.POSITIVE_INFINITY;
        let datasetEnd = Number.NEGATIVE_INFINITY;
        for (let i = 0; i < cachedStockData.length; i += 1) {
            const row = cachedStockData[i];
            if (!row || typeof row !== 'object') continue;
            const rowDateCandidate = row.date || row.Date || row.tradeDate || row.trade_date || row.timestamp || row.time || row.t;
            const ts = parseIsoDateToUtcSafe(rowDateCandidate);
            if (!Number.isFinite(ts)) continue;
            if (ts < datasetStart) datasetStart = ts;
            if (ts > datasetEnd) datasetEnd = ts;
        }
        if (Number.isFinite(datasetStart) && datasetStart > targetStart) {
            fetchRequired = true;
            rangeFailureReason = 'cachedRowsMissingWarmup';
        }
        if (!fetchRequired && Number.isFinite(datasetEnd) && datasetEnd < targetEnd) {
            fetchRequired = true;
            rangeFailureReason = 'cachedRowsMissingTail';
        }
    }

    if (fetchRequired) {
        result.reason = rangeFailureReason;
        return result;
    }
    ...
    if (typeof buildCacheKey === 'function' && cachedDataStore instanceof Map) {
        ...
        const cacheEntry = typeof ensureDatasetCacheEntryFresh === 'function'
    ...
    result.useCachedData = true;
    result.cachedData = cachedStockData;
