// --- 批量策略優化功能 - v1.2.9 ---
// Patch Tag: LB-BATCH-CATALOG-20260918B
...
const BatchStrategyUtils = (typeof window !== 'undefined' && window.LazyBatchStrategyUtils)
    ? window.LazyBatchStrategyUtils
    : (typeof globalThis !== 'undefined' && globalThis.LazyBatchStrategyUtils ? globalThis.LazyBatchStrategyUtils : null);

const DEATH_CROSS_STRATEGIES = new Set(['ma_cross_exit', 'macd_cross_exit', 'k_d_cross_exit']);

const BATCH_DEBUG_VERSION_TAG = 'LB-BATCH-CATALOG-20260918B';

const BATCH_STRATEGY_DEFAULTS = Object.freeze({
    longEntries: [
...
function normaliseBatchStrategyId(role, strategyId) {
    ...
    const traceEvents = [];
    const strategyRoleMap = (typeof LazyStrategyId !== 'undefined' && LazyStrategyId.map)
        ? LazyStrategyId.map
        : (typeof STRATEGY_ID_ROLE_MIGRATIONS !== 'undefined' ? STRATEGY_ID_ROLE_MIGRATIONS : {});

    if (BatchStrategyUtils && typeof BatchStrategyUtils.resolveWorkerStrategyId === 'function') {
        try {
            const resolved = BatchStrategyUtils.resolveWorkerStrategyId(strategyId, roleKey, {
                strategyDescriptions,
                lookupKey: typeof resolveStrategyLookupKey === 'function' ? resolveStrategyLookupKey : null,
                normaliseForRole: typeof normaliseStrategyIdForRole === 'function' ? normaliseStrategyIdForRole : null,
                normaliseAny: typeof normaliseStrategyIdAny === 'function' ? normaliseStrategyIdAny : null,
                roleMap: strategyRoleMap,
            ...
            console.warn('[Batch Optimization] BatchStrategyUtils resolveWorkerStrategyId failed:', error);
            if (batchDebugSession) {
                recordBatchDebug('strategy-utils-resolve-error', {
                    role: roleKey || null,
                    strategyId,
    ...
    if (BatchStrategyMapper && typeof BatchStrategyMapper.canonicalize === 'function') {
        const mapped = BatchStrategyMapper.canonicalize(strategyId, roleKey);
        if (mapped) {
            return mapped;
        }
    }
    ...
    if (typeof resolveStrategyLookupKey === 'function') {
        const resolved = resolveStrategyLookupKey(strategyId, roleKey);
        if (resolved) {
            normalized = resolved;
        }
    } else if (roleKey && typeof normaliseStrategyIdForRole === 'function') {
        const migrated = normaliseStrategyIdForRole(roleKey, strategyId);
        if (migrated) {
            normalized = migrated;
        }
    } else if (roleKey && roleKey !== 'entry' && typeof normaliseStrategyIdAny === 'function') {
    ...
    if (roleKey === 'entry' && normalized && normalized.endsWith('_exit')) {
    ...
    if (roleKey === 'shortEntry' && normalized && !normalized.startsWith('short_')) {
    ...
    if (roleKey === 'shortExit' && normalized && !normalized.startsWith('cover_')) {
    ...
    if (traceEvents.length && batchDebugSession) {
        recordBatchDebug('strategy-normalise-trace', {
            role: roleKey || null,
            original: strategyId,
            normalized,
            steps: traceEvents,
...
function getWorkerStrategyName(batchStrategyName, roleHint) {
    ...
    if (BatchStrategyUtils && typeof BatchStrategyUtils.resolveWorkerStrategyId === 'function') {
        try {
            const resolved = BatchStrategyUtils.resolveWorkerStrategyId(batchStrategyName, roleHint, {
                strategyDescriptions,
                lookupKey: typeof resolveStrategyLookupKey === 'function' ? resolveStrategyLookupKey : null,
                normaliseForRole: typeof normaliseStrategyIdForRole === 'function' ? normaliseStrategyIdForRole : null,
                normaliseAny: typeof normaliseStrategyIdAny === 'function' ? normaliseStrategyIdAny : null,
            ...
            if (resolved) {
                workerStrategy = resolved;
            }
        } catch (error) {
            console.warn('[Batch Optimization] resolveWorkerStrategyId failed:', error);
            if (batchDebugSession) {
                recordBatchDebug('strategy-utils-worker-error', {
                    role: roleHint || null,
    ...
    if (!workerStrategy && BatchStrategyMapper) {
    ...
    if (batchDebugSession) {
        if (traceEvents.length > 0) {
            recordBatchDebug('strategy-utils-trace', {
                requested: batchStrategyName,
                role: roleHint || null,
                mapped: workerStrategy,
                trace: traceEvents.slice(0, 10),
            }, { console: false });
        } else if (workerStrategy !== batchStrategyName) {
...
function generateStrategyOptions() {
    try {
        ...
        const appendOption = (container, strategy, rolePrefix) => {
            const info = strategyDescriptions?.[strategy];
            if (!info) return;
            const wrapper = document.createElement('div');
            wrapper.className = 'flex items-center';
            const inputId = `${rolePrefix}-${strategy}`;
            wrapper.innerHTML = `
                <input type="checkbox" id="${inputId}" value="${strategy}" data-strategy-role="${rolePrefix}" class="h-4 w-4 text-blue-600 border-gray-300 rounded mr-2">
                <label for="${inputId}" class="text-sm text-gray-700 cursor-pointer">
                    ${info.name}
                </label>
            `;
        ...
        catalog.longEntries.forEach((strategy) => appendOption(buyStrategiesList, strategy, 'buy'));
        catalog.longExits.forEach((strategy) => appendOption(sellStrategiesList, strategy, 'sell'));

        if (batchDebugSession) {
            recordBatchDebug('strategy-options-generated', {
                longEntries: catalog.longEntries.slice(0, 20),
                longExits: catalog.longExits.slice(0, 20),
                totals: {
                    longEntries: catalog.longEntries.length,
                    longExits: catalog.longExits.length,
                },
            }, { console: false });
        }
        ...
        if (batchDebugSession) {
            recordBatchDebug('strategy-options-error', {
...
function getStrategyCatalog() {
    ...
    if (BatchStrategyUtils && typeof BatchStrategyUtils.deriveStrategyCatalog === 'function') {
        try {
            return BatchStrategyUtils.deriveStrategyCatalog(strategyDescriptions || {}, {
                roleMap: strategyRoleMap,
                defaults: BATCH_STRATEGY_DEFAULTS,
            });
        } catch (error) {
            console.warn('[Batch Optimization] deriveStrategyCatalog failed, falling back to defaults:', error);
            if (batchDebugSession) {
                recordBatchDebug('strategy-catalog-fallback', {
                    error: error?.message || String(error),
                }, { level: 'warn', consoleLevel: 'warn' });
            }
        }
    ...
    const append = (bucket, id) => {
        if (!id || !strategyDescriptions?.[id]) return;
        if (!fallback[bucket].includes(id)) {
            fallback[bucket].push(id);
            if (bucket === 'longExits' && ['fixed_stop_loss', 'cover_fixed_stop_loss', 'trailing_stop', 'cover_trailing_stop'].includes(id)) {
                if (!fallback.riskExits.includes(id)) {
                    fallback.riskExits.push(id);
                }
            }
        }
    };

    BATCH_STRATEGY_DEFAULTS.longEntries.forEach((id) => append('longEntries', id));
    BATCH_STRATEGY_DEFAULTS.longExits.forEach((id) => append('longExits', id));
    BATCH_STRATEGY_DEFAULTS.shortEntries.forEach((id) => append('shortEntries', id));
    BATCH_STRATEGY_DEFAULTS.shortExits.forEach((id) => append('shortExits', id));

    if (strategyRoleMap.exit) {
        Object.keys(strategyRoleMap.exit).forEach((legacy) => {
            const mapped = strategyRoleMap.exit[legacy];
            append('longEntries', legacy);
            append('longExits', mapped);
        });
    }
    if (strategyRoleMap.shortEntry) {
        Object.values(strategyRoleMap.shortEntry).forEach((id) => append('shortEntries', id));
    }
    if (strategyRoleMap.shortExit) {
        Object.values(strategyRoleMap.shortExit).forEach((id) => append('shortExits', id));
    }
