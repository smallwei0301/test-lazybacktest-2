// --- 批量策略優化功能 - v1.2.9 ---
// Patch Tag: LB-BATCH-MAPREFILL-20260918A
...
const BATCH_DEBUG_VERSION_TAG = 'LB-BATCH-MAPREFILL-20260918A';

let batchDebugSession = null;
const batchDebugListeners = new Set();

const EXIT_STRATEGY_FALLBACK_IDS = new Set([
    'ma_below',
    'rsi_overbought',
    'bollinger_reversal',
    'price_breakdown',
...
function collectBatchStrategyGroups(strategies = strategyDescriptions || {}) {
    const byRole = {
        entry: new Set(),
        exit: new Set(),
        shortEntry: new Set(),
    ...
    Object.keys(strategies).forEach((strategyId) => {
        ...
        if (strategyId.startsWith('short_')) {
            byRole.shortEntry.add(strategyId);
            return;
        }

        if (strategyId.startsWith('cover_')) {
            byRole.shortExit.add(strategyId);
            return;
        }

        if (strategyId.endsWith('_exit') || EXIT_STRATEGY_FALLBACK_IDS.has(strategyId)) {
            byRole.exit.add(strategyId);
            return;
        }

        byRole.entry.add(strategyId);
    });

    const sortByDisplayName = (set) => Array.from(set).sort((a, b) => {
        const nameA = (strategies[a]?.name || a);
        const nameB = (strategies[b]?.name || b);
        const localeCompare = nameA.localeCompare(nameB, 'zh-Hant');
        if (localeCompare !== 0) {
    ...
    return {
        entry: sortByDisplayName(byRole.entry),
        exit: sortByDisplayName(byRole.exit),
        shortEntry: sortByDisplayName(byRole.shortEntry),
        shortExit: sortByDisplayName(byRole.shortExit),
    };
}
...
function generateStrategyOptions() {
    try {
        ...
        const groups = collectBatchStrategyGroups(strategyDescriptions);

        if (typeof window !== 'undefined') {
            window.LazyBatchStrategyGroups = groups;
        } else if (typeof globalThis !== 'undefined') {
            globalThis.LazyBatchStrategyGroups = groups;
        }

        if (batchDebugSession) {
            recordBatchDebug('strategy-options-refresh', {
                entryCount: groups.entry.length,
                exitCount: groups.exit.length,
                shortEntryCount: groups.shortEntry.length,
                shortExitCount: groups.shortExit.length,
            }, { phase: 'init', console: false });
        }

        const buyStrategies = groups.entry;
        const sellStrategies = groups.exit;

        if (buyStrategies.length === 0 || sellStrategies.length === 0) {
            const message = '[Batch Optimization] Missing strategy groups for batch configuration';
            console.warn(message);
            if (batchDebugSession) {
                recordBatchDebug('strategy-options-missing', {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports.collectBatchStrategyGroupsForTest = collectBatchStrategyGroups;
}
