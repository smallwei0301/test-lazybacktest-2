...
const BatchStrategyCatalog = (typeof window !== 'undefined' && window.LazyBatchStrategyCatalog)
    ? window.LazyBatchStrategyCatalog
    : (typeof globalThis !== 'undefined' && globalThis.LazyBatchStrategyCatalog ? globalThis.LazyBatchStrategyCatalog : null);
...
const DEFAULT_ENTRY_STRATEGIES = Array.isArray(BatchStrategyCatalog?.defaults?.entry)
    ? BatchStrategyCatalog.defaults.entry.slice()
    : [
...
const DEFAULT_EXIT_STRATEGIES = Array.isArray(BatchStrategyCatalog?.defaults?.exit)
    ? BatchStrategyCatalog.defaults.exit.slice()
    : [
...
function hydrateStrategyNameMap(options = {}) {
    if (strategyCatalogCache && !options.force) {
    ...
    try {
        ...
        if (BatchStrategyCatalog && typeof BatchStrategyCatalog.hydrate === 'function') {
            catalogResult = BatchStrategyCatalog.hydrate({
                includeMissing: true,
                descriptions,
            });
        }

        const ensureList = (list, fallback) => {
            if (Array.isArray(list) && list.length > 0) {
                return list.slice();
            }
            return fallback.filter((id) => descriptions[id]);
        };

        const entryList = ensureList(catalogResult?.entry, DEFAULT_ENTRY_STRATEGIES);
        const exitList = ensureList(catalogResult?.exit, DEFAULT_EXIT_STRATEGIES);

        let missingEntry = Array.isArray(catalogResult?.missing?.entry)
            ? catalogResult.missing.entry.slice()
            : [];
        let missingExit = Array.isArray(catalogResult?.missing?.exit)
            ? catalogResult.missing.exit.slice()
            : [];

        if (missingEntry.length === 0) {
            missingEntry = DEFAULT_ENTRY_STRATEGIES.filter((id) => !descriptions[id]);
        }
        if (missingExit.length === 0) {
            missingExit = DEFAULT_EXIT_STRATEGIES.filter((id) => !descriptions[id]);
        }

        const finalMissingEntry = Array.from(new Set(missingEntry));
        const finalMissingExit = Array.from(new Set(missingExit));

        const debugDetail = {
            entryCount: entryList.length,
            exitCount: exitList.length,
            missingEntry: finalMissingEntry,
            missingExit: finalMissingExit,
        };

        if (entryList.length === 0 || exitList.length === 0) {
            recordBatchDebug('strategy-catalog-fallback', debugDetail, { level: 'warn', console: false });
        } else if (!options.quiet) {
            recordBatchDebug('strategy-catalog-hydrated', debugDetail, { console: false });
        }

        strategyCatalogCache = {
            entry: entryList,
            exit: exitList,
            missing: {
                entry: finalMissingEntry,
                exit: finalMissingExit,
            }
        };

        return strategyCatalogCache;
    } catch (error) {
        console.error('[Batch Optimization] Strategy catalog hydration failed:', error);
        recordBatchDebug('strategy-catalog-error', {
            message: error?.message || String(error),
        }, { level: 'error', console: false });

        strategyCatalogCache = {
            entry: DEFAULT_ENTRY_STRATEGIES.filter((id) => descriptions[id]),
            exit: DEFAULT_EXIT_STRATEGIES.filter((id) => descriptions[id]),
            missing: {
                entry: DEFAULT_ENTRY_STRATEGIES.filter((id) => !descriptions[id]),
                exit: DEFAULT_EXIT_STRATEGIES.filter((id) => !descriptions[id]),
            }
...
function generateStrategyOptions() {
    try {
        ...
        const catalog = hydrateStrategyNameMap({ quiet: true });
        const descriptions = (typeof strategyDescriptions === 'object' && strategyDescriptions)
            ? strategyDescriptions
            : {};

        const buyStrategies = Array.isArray(catalog?.entry) && catalog.entry.length > 0
            ? catalog.entry
            : DEFAULT_ENTRY_STRATEGIES.filter((id) => descriptions[id]);

        const sellStrategies = Array.isArray(catalog?.exit) && catalog.exit.length > 0
            ? catalog.exit
            : DEFAULT_EXIT_STRATEGIES.filter((id) => descriptions[id]);

        if ((!buyStrategies || buyStrategies.length === 0) || (!sellStrategies || sellStrategies.length === 0)) {
            const missing = catalog?.missing || {};
            recordBatchDebug('strategy-options-empty', {
                buyCount: Array.isArray(buyStrategies) ? buyStrategies.length : 0,
                sellCount: Array.isArray(sellStrategies) ? sellStrategies.length : 0,
                missingEntry: missing.entry || [],
                missingExit: missing.exit || [],
            }, { level: 'warn', console: false });
            console.warn('[Batch Optimization] Strategy catalog is empty, falling back to defaults');
        }
...
window.batchOptimization = {
    ...
    getStrategyCatalog: (options = {}) => hydrateStrategyNameMap({ ...options }),
    refreshStrategyCatalog: () => {
        strategyCatalogCache = null;
        return hydrateStrategyNameMap({ force: true });
    }
};
