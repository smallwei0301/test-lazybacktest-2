...
## 5. Walk-Forward 評分公式（LB-ROLLING-TEST-20251028B）

滾動測試評分仍採「OOS 品質 × 統計可信度 × 穩健度」乘積流程，並依據最新回饋進一步優化資訊呈現：

- **OOS 品質**：年化門檻改採每個視窗的買入持有年化報酬，只要策略高於持有即給 1 分，未達則依距離線性遞減至 0；Sharpe、Sortino、勝率與最大回撤仍採達標滿分、未達線性遞減，權重維持 0.35／0.25／0.20／0.10／0.10，並提供每窗品質、加權原值、達標比與統計權重的明細表。
- **統計可信度**：以 Web Worker 回傳的 OOS 報酬序列計算 PSR95 與 DSR，兩者平均後得到可信度並換算成統計權重 (0.5 + 0.5 × 可信度)，同時揭露 MinTRL、PSR／DSR／樣本數，協助判斷樣本是否足夠。
...
若要於文件或 UI 提及此版本，請註記補丁代碼 `LB-ROLLING-TEST-20251028B`，以利對照 `log.md` 追蹤更新內容。
