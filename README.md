...
新版滾動測試評分維持「OOS 品質 × 統計可信度 × 穩健度」的乘積流程，並額外強化門檻對齊與呈現方式：

- **OOS 品質**：以使用者門檻作為 0 分起點，根據 15% 年化報酬、Sharpe 1.2、Sortino 1.5、MaxDD 10%～25%、勝率門檻 +10% 的範圍做線性轉換至 0～1，再以 0.35／0.25／0.20／0.10／0.10 權重加總；若大多數指標未達門檻，品質分數會依權重同步折減，確保需達標才會被視為合格。
...
- **穩健度調整**：每個視窗計算 Walk-Forward Efficiency（WFE = OOS 年化 ÷ IS 年化），跨視窗取中位數並截斷於 0.8～1.2，形成 WFE 調整因子；總分最終會截斷在 0～100 分之內，避免超過滿分。
...
- **介面提示**：Walk-Forward 總分卡片提供整體評級文字，其他指標卡片則以「合格／未達標」等白話說明呈現品質、統計可信度、WFE 與通過率，協助新手快速判讀。
- **資料更新**：滾動測試面板會隨主回測快取更新自動刷新視窗規畫，避免使用者已回測仍顯示「請先執行回測」的提示。
