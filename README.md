...
- **OOS 品質**：以使用者門檻作為 0 分起點，根據 15% 年化報酬、Sharpe 1.2、Sortino 1.5、MaxDD 10%～25%、勝率門檻 +10% 的範圍做線性轉換至 0～1，再以 0.35／0.25／0.20／0.10／0.10 權重加總；若達標權重比例低於 70%，會同步壓低品質分數，避免多數指標未達門檻仍出現滿分。
...
- **總分**：每個視窗的 `WindowScore = OOS 品質 × StatWeight`，跨窗取中位數後再乘上 WFE 調整因子，即為最後的 Walk-Forward 分數；結果會換算成 0～100 分顯示並截斷 100 分上限，評級邏輯維持 TotalScore ≥ 0.70（專業合格）、≥0.50（可進一步觀察）兩級門檻，未達條件則建議調整。
- **介面提示**：Walk-Forward 總分卡片提供整體評級文字，其他指標卡片會以「合格／待加強」的白話說明呈現品質、統計可信度與視窗通過率；OOS 品質卡片會在達標權重不足時明確提醒「多數指標未達門檻」。
- **資料快取**：主畫面剛完成回測時，滾動測試會優先沿用最新回測結果的價格列，避免誤判成「尚未執行主回測」。
