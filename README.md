...
## 5. Walk-Forward 評分公式（LB-ROLLING-TEST-20251024A）
...
- **總分**：每個視窗的 `WindowScore = OOS 品質 × StatWeight`，跨窗取中位數後再乘上 WFE 調整因子，即為最後的 Walk-Forward 分數。分數會被截斷在 0～100 分範圍，確保最高僅顯示 100 分，同時維持 TotalScore ≥ 0.70（專業合格）、≥0.50（可進一步觀察）的評級門檻。
- **品質檢核**：OOS 品質會同步追蹤各指標是否達到使用者門檻，僅有多數指標達標時才可能拿到高分，中位品質 < 0.70 或達標比率低時會在摘要提示「建議檢查弱項」。
- **介面提示**：Walk-Forward 總分卡片提供整體評級文字，其他指標卡片會直接以「合格／未達標、建議……」等白話提示呈現品質、統計可信度與視窗通過率，協助新手快速判讀。

若要於文件或 UI 提及此版本，請註記補丁代碼 `LB-ROLLING-TEST-20251024A`，以利對照 `log.md` 追蹤更新內容。
