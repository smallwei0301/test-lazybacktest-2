...
## 5. Walk-Forward 評分公式（LB-ROLLING-TEST-20251105A）

滾動測試評分仍採「OOS 品質 × 統計可信度 × 穩健度」乘積流程，並依據最新回饋進一步優化資訊呈現：

- **OOS 品質**：年化報酬門檻改以各測試視窗的買入持有年化報酬為基準，只要達標即給滿分，未達則依差距線性遞減至 0；Sharpe／Sortino／勝率則沿用既有門檻與緩衝區，最大回撤維持門檻加緩衝的反向線性換算，並在儀表卡提供可展開的分數明細列出中位值、基準與 0～1 分數。
...
若要於文件或 UI 提及此版本，請註記補丁代碼 `LB-ROLLING-TEST-20251105A`，以利對照 `log.md` 追蹤更新內容。
