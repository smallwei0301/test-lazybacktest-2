...
滾動測試評分仍採「OOS 品質 × 統計可信度 × 穩健度」乘積流程，本次針對評分邏輯與報告呈現進行以下優化：

- **OOS 品質**：仍沿用年化報酬 15%、Sharpe 1.2、Sortino 1.5、MaxDD 10%～25%、勝率門檻 +10% 的正規化，但最終分數會乘上「指標達標權重比」，確保多數指標未達門檻時不會出現滿分。每一個指標的分數、門檻、目標與權重都會收錄在卡片的摺疊明細中，方便追蹤缺口。
- **統計可信度**：PSR95 與 DSR 仍由 Web Worker 回傳的 OOS 報酬矩產生；卡片會顯示可信度中位數、統計權重中位數與 PSR≥95% 視窗比等細項，協助研判樣本是否足夠。
- **穩健度調整**：Walk-Forward Efficiency 採中位數並截斷於 0.8～1.2，評分卡同時顯示當前 WFE 與調整係數範圍，方便理解總分折算過程。
- **總分顯示**：跨窗中位 `WindowScore` 乘上 WFE 調整後仍會截斷於 0～1，最後換算成 0～100 分；總分卡的摺疊明細會列出窗分數、WFE 係數與等級判定摘要。報告上方新增專門的「Walk-Forward 判定」卡片，只展示專業合格／可觀察／未通過等級。
- **報表排版**：逐窗指標表改為「指標 × 視窗」的轉置格式，列出訓練期間、測試期間、年化報酬、Sharpe、PSR、DSR、WFE、參數摘要與評語等資訊，利於視窗間比較。
- **操作體驗**：取消「測試至少成交筆數」門檻，通過視窗比例僅依既有績效門檻計算；主回測完成後會最多自動重試三次刷新滾動測試規劃，避免仍顯示「請先執行一次主回測」。
