...
async function trainAndEval(Xtr, ytr, Xte, yte, epochs = 60, batchSize = 32) {
  ...
  const summaryLines = [];
  if (typeof model.summary === 'function') {
    model.summary(undefined, undefined, (line) => summaryLines.push(line));
  }

  return { model, acc, confusion: { TP, TN, FP, FN }, summaryText: summaryLines.join('\n') };
}
...
async function runANNPredictionWithCached(rows) {
  ...
  if (Xte.length === 0 || yte.length === 0) {
    throw new Error('測試資料不足，請拉長日期區間');
  }

  const { model, acc, confusion, summaryText } = await trainAndEval(Xtr, ytr, Xte, yte);
  ...
  return { acc, confusion, kelly, model, summaryText };
}

if (typeof window !== 'undefined') {
  if (!window.LB_ANN) window.LB_ANN = {};
  window.LB_ANN.runANNPredictionWithCached = runANNPredictionWithCached;
}
