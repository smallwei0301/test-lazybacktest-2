    ...
    if (clearAllBtn) {
      clearAllBtn.addEventListener("click", () => {
        const checkboxes = document.querySelectorAll(
          '#buy-strategies-list input[type="checkbox"], #sell-strategies-list input[type="checkbox"]',
        )
...
function validateBatchStrategies() {
  ...
  // 檢查選擇的策略是否為 null 或無效
  const invalidBuyStrategies = buyStrategies.filter(
    (strategy) => !strategy || strategy === "null" || !strategyDescriptions[strategy],
  )

  const invalidSellStrategies = sellStrategies.filter(
    (strategy) => !strategy || strategy === "null" || !strategyDescriptions[strategy],
  )
...
function getBatchOptimizationConfig() {
  try {
    ...
    if (parameterTrialsElement && parameterTrialsElement.value) {
      config.parameterTrials = Number.parseInt(parameterTrialsElement.value) || 100
    }
    ...
    if (concurrencyElement && concurrencyElement.value) {
      config.concurrency = Number.parseInt(concurrencyElement.value) || 4
    }
    ...
    if (iterationLimitElement && iterationLimitElement.value) {
      config.iterationLimit = Number.parseInt(iterationLimitElement.value) || 6
    }
...
function updateBatchProgress(currentCombination = null) {
  ...
  if (progressText && progressBar && progressDetail) {
    // 計算精確的百分比（每1%更新）
    const rawPercentage =
      currentBatchProgress.total > 0 ? (currentBatchProgress.current / currentBatchProgress.total) * 100 : 0
...
async function executeBatchOptimization(config) {
  ...
  try {
    ...
    console.log(
      `[Batch Optimization] Completed per-combination parameter optimization for ${optimizedCombinations.length} combinations`,
    )
...
function getMetricFromResult(result, metric) {
  ...
  if (val === undefined || val === null || isNaN(val)) return Number.NaN
  return val
}
...
async function optimizeCombinationIterative(combination, config) {
  console.log(
    `[Batch Optimization] Starting iterative combination optimization for ${combination.buyStrategy} + ${combination.sellStrategy}`,
  )

  const maxIterations =
    config && typeof config.iterationLimit !== "undefined" ? Number.parseInt(config.iterationLimit, 10) || 6 : 6
  ...
  try {
    ...
    console.error(
      `[Batch Optimization] Error in iterative optimization for ${combination.buyStrategy} + ${combination.sellStrategy}:`,
      error,
    )
    // 返回原始組合作為備用
    return combination
...
async function optimizeStrategyWithInternalConvergence(
  strategy,
  strategyType,
  strategyInfo,
  targetMetric,
  ...
  // 初始化參數：使用組合中的當前參數
  let currentParams = strategyType === "entry" ? { ...baseCombo.buyParams } : { ...baseCombo.sellParams }
  ...
  for (let iter = 0; iter < maxInternalIterations; iter++) {
    ...
    for (let i = 0; i < optimizeTargets.length; i++) {
      ...
      const bestParam = await optimizeSingleStrategyParameter(
        ...
        strategyType,
        targetMetric,
        Math.max(1, Number.parseInt(trials, 10) || 1),
      )

      if (bestParam.value !== undefined) {
        currentParams[optimizeTarget.name] = bestParam.value
        console.log(
          `[Batch Optimization] Updated ${strategy}.${optimizeTarget.name}: ${bestParam.value} (${targetMetric}: ${bestParam.metric.toFixed(4)})`,
        )
      }
    }
...
async function optimizeCombinations(combinations, config) {
  ...
  batchWorkerStatus.inFlightCount = 0
  batchWorkerStatus.entries = combinations.map((c, idx) => ({
    index: idx + 1,
    buyStrategy: c.buyStrategy,
    sellStrategy: c.sellStrategy,
    status: "queued",
    startTime: null,
    endTime: null,
  }))
      ...
      while (index < combinations.length && inFlight.size < maxConcurrency) {
        ...
        const p = optimizeCombinationIterative(combo, config)
          .then((res) => {
            ...
            updateBatchProgress(
              30 + (completedCount / combinations.length) * 30,
              `優化組合 ${completedCount}/${combinations.length}`,
            )
          })
          .catch((err) => {
...
function processBatch(batches, batchIndex, config) {
  ...
  console.log(
    `[Batch Optimization] Processing batch ${batchIndex + 1}/${batches.length} with ${currentBatch.length} combinations`,
  )
  ...
  // 處理當前批次
  processStrategyCombinations(currentBatch, config)
    .then(() => {
      // 檢查是否被停止
      if (isBatchOptimizationStopped) {
        console.log("[Batch Optimization] Process stopped by user")
...
async function processStrategyCombinations(combinations, config) {
  ...
  for (let i = 0; i < combinations.length; i++) {
    ...
    if (currentBatchProgress.current % 10 === 0) {
      // 每10個更新一次進度
  ...
  console.log(
    `[Batch Optimization] Processed ${combinations.length} combinations, total results: ${batchOptimizationResults.length}`,
  )
}
        ...
        tempWorker.onmessage = (e) => {
          if (e.data.type === "result") {
            ...
            if (result) {
              ...
              console.log(
                `[Batch Optimization] Backtest completed with stopLoss: ${params.stopLoss}, takeProfit: ${params.takeProfit}`,
              )
            }
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (isRiskManagementStrategy) {
        ...
        console.log(
          `[Batch Optimization] Risk management optimization ranges (consistent with single optimization):`,
          riskOptimizeTargets,
        )
      ...
      console.log(
        `[Batch Optimization] Found ${strategyInfo.optimizeTargets.length} parameters to optimize:`,
...
async function optimizeMultipleStrategyParameters(
  strategy,
  strategyType,
  strategyInfo,
  targetMetric,
  ...
  try {
    ...
    // 每個參數使用使用者指定的優化次數
    const trialsPerParam = Math.max(1, Number.parseInt(trials, 10) || 1)
    console.log(
      `[Batch Optimization] Optimizing ${optimizeTargets.length} parameters with ${trialsPerParam} trials each`,
    )
    ...
    const optimizedParams = strategyType === "entry" ? { ...baseParams.entryParams } : { ...baseParams.exitParams }
    ...
    for (let i = 0; i < optimizeTargets.length; i++) {
      ...
      if (bestParam.value !== undefined) {
        ...
        console.log(
          `[Batch Optimization] Best ${optimizeTarget.name}: ${bestParam.value}, ${targetMetric}: ${bestParam.metric.toFixed(4)}`,
        )
      } else {
        console.warn(
          `[Batch Optimization] No valid optimization result for ${optimizeTarget.name}, keeping default value`,
        )
      }
    }
...
async function optimizeSingleStrategyParameter(params, optimizeTarget, strategyType, targetMetric, trials) {
  return new Promise((resolve) => {
    if (!workerUrl) {
      ...
      resolve({ value: undefined, metric: Number.NEGATIVE_INFINITY })
      return
    }
    ...
    optimizeWorker.onmessage = (e) => {
      ...
      if (type === "result") {
        ...
        if (validResults.length === 0) {
          console.warn(
            `[Batch Optimization] Optimization returned results but none had usable paramValue/metric for ${optimizeTarget.name}`,
          )
      ...
      if (type === "result") {
        ...
        if (data && data.results && data.results.length > 0) {
          ...
          const sortedResults = data.results.sort((a, b) => {
            const aValue = a[targetMetric] || Number.NEGATIVE_INFINITY
            const bValue = b[targetMetric] || Number.NEGATIVE_INFINITY
          ...
          console.log(
            `[Batch Optimization] Best ${optimizeTarget.name}: ${bestResult.paramValue}, ${targetMetric}: ${bestResult[targetMetric]}`,
          )
        ...
        const promise = performCrossOptimization(
          ...
          .finally(() => {
            ...
            if (inFlight.size === 0 && index >= tasks.length) {
              ...
              console.log(
                `[Cross Optimization] All tasks completed. Valid results: ${validResults.length}/${tasks.length}`,
              )
        ...
        const promise = performCrossOptimization(
          ...
          .finally(() => {
            ...
            if (inFlight.size === 0 && index >= tasks.length) {
              ...
              const validResults = results.filter(Boolean)
              console.log(
                `[Cross Optimization] All exit tasks completed. Valid results: ${validResults.length}/${tasks.length}`,
              )
              resolve(validResults)
            } else {
              // 啟動下一個任務
...
function findBestEntryStrategy() {
  ...
  const sorted = [...batchOptimizationResults].sort((a, b) => {
    const aReturn = a.annualizedReturn || Number.NEGATIVE_INFINITY
    const bReturn = b.annualizedReturn || Number.NEGATIVE_INFINITY
  ...
  try {
    ...
    if (optimizationType === "entry-fixed" && entryParams) {
      ...
      if (exitStrategyInfo && exitStrategyInfo.optimizeTargets) {
        ...
        const optimizedExitParams = await optimizeSingleStrategyParametersFast(
          exitStrategy,
          "exit",
          exitStrategyInfo,
          baseParams,
      ...
      if (entryStrategyInfo && entryStrategyInfo.optimizeTargets) {
        ...
        const optimizedEntryParams = await optimizeSingleStrategyParametersFast(
          entryStrategy,
          "entry",
          entryStrategyInfo,
          baseParams,
...
async function optimizeSingleStrategyParameters(strategy, strategyType, strategyInfo, baseParams) {
  try {
    console.log("[Cross Optimization] optimizeSingleStrategyParameters called:", {
      strategy,
      strategyType,
      strategyInfo: strategyInfo?.name,
    })
...
function loadBatchStrategy(index) {
  ...
  if (exitStrategyElement) {
    ...
    console.log(
      "[Batch Optimization] Available options in select:",
      Array.from(exitStrategyElement.options).map((o) => o.value),
    )
    ...
    if (!optionExists) {
      console.warn(
        `[Batch Optimization] Option '${selectValue}' not found in select, trying original value '${exitStrategy}'`,
      )
      ...
      if (!fallbackExists) {
        console.warn(
          `[Batch Optimization] Neither '${selectValue}' nor original value found, using stop_loss_take_profit`,
        )
    ...
    if (!result.sellStrategy) {
      ...
      if (selectValue === "stop_loss_take_profit") {
        ...
        const strategyName =
          strategyDescriptions[result.sellStrategy]?.name || strategyDescriptions[selectValue]?.name || selectValue
  ...
  if (result.riskManagement) {
    ...
    showInfo(
      `已載入優化的風險管理參數：停損 ${result.riskManagement.stopLoss || 0}%，停利 ${result.riskManagement.takeProfit || 0}%`,
    )
  } else {
    ...
    for (const [key, value] of Object.entries(params)) {
      if (key && value !== undefined && value !== null) {
        ...
        if ((currentStrategy === "k_d_cross" || currentStrategy === "k_d_cross_exit") && key === "thresholdX") {
          ...
          (currentStrategy === "k_d_cross_exit" || currentStrategy.includes("k_d_cross")) &&
          key === "thresholdY"
        ) {
          inputId = `${type}KdThresholdY`
        } else if (
          (currentStrategy === "macd_cross" || currentStrategy === "macd_cross_exit") &&
          key === "signalPeriod"
        ) {
        ...
        if (input) {
          ...
          console.warn(
            `[Batch Optimization] Input element not found: ${inputId} for strategy ${currentStrategy}, key: ${key}`,
          )
        }
      }
    }
...
async function optimizeAllStrategies(buyStrategies, sellStrategies, config) {
  ...
  for (const strategy of buyStrategies) {
    updateBatchProgress(
      5 + (completedStrategies / totalStrategies) * 20,
...
function generateStrategyCombinations(buyStrategies, sellStrategies) {
  ...
  for (const buyStrategy of buyStrategies) {
    ...
    for (const sellStrategy of sellStrategies) {
      ...
      if (
        (sellStrategy === "fixed_stop_loss" || sellStrategy === "cover_fixed_stop_loss") &&
        sellParams &&
        Object.keys(sellParams).length > 0
      ) {
        combination.riskManagement = { ...sellParams }
  ...
  if (batchOptimizationResults && batchOptimizationResults.length > 0) {
    batchOptimizationResults.forEach((result, index) => {
      ...
      if (result.sellStrategy === "fixed_stop_loss" || result.sellStrategy === "cover_fixed_stop_loss") {
        ...
        if (result.usedStopLoss !== undefined || result.usedTakeProfit !== undefined) {
          console.log(
            `[Debug] ✓ Used risk parameters found for result ${index}: stopLoss=${result.usedStopLoss}, takeProfit=${result.usedTakeProfit}`,
          )
        } else {
...
function testParameterRanges() {
  ...
  console.log(
    `[Test] StopLoss: range ${stopLossRange.from}-${stopLossRange.to}, step ${stopLossRange.step}, points: ${stopLossPoints}`,
  )
  console.log(
    `[Test] TakeProfit: range ${takeProfitRange.from}-${takeProfitRange.to}, step ${takeProfitRange.step}, points: ${takeProfitPoints}`,
  )
...
function checkAllStrategyParameters() {
  ...
  strategies.forEach((strategyKey) => {
    ...
    if (strategy.optimizeTargets && strategy.optimizeTargets.length > 0) {
      ...
      strategy.optimizeTargets.forEach((target, index) => {
        console.log(
          `[Debug]   ${index + 1}. ${target.name} (${target.label}): range ${target.range.from}-${target.range.to}, step ${target.range.step}`,
        )
      })
    } else {
  ...
  // 統計
  const strategiesWithParams = strategies.filter(
    (key) => strategyDescriptions[key].optimizeTargets && strategyDescriptions[key].optimizeTargets.length > 0,
  )

  const multiParamStrategies = strategies.filter(
    (key) => strategyDescriptions[key].optimizeTargets && strategyDescriptions[key].optimizeTargets.length > 1,
  )
...
function testFullRiskManagementOptimization() {
  ...
  // 執行測試優化
  executeBatchOptimization(testConfig)
    .then(() => {
...
function updateCrossOptimizationProgress(currentTask = null) {
  try {
    ...
    // 計算進度百分比
    const percentage =
      crossOptimizationProgress.total > 0
        ? Math.floor((crossOptimizationProgress.current / crossOptimizationProgress.total) * 100)
        : 0
...
function addCrossOptimizationResults(newResults) {
  newResults.forEach((newResult) => {
    ...
    const existingIndex = batchOptimizationResults.findIndex(
      (existing) =>
    ...
    if (existingIndex !== -1) {
      ...
      console.log(
        `[Cross Optimization] 合併重複結果: ${newResult.buyStrategy} + ${newResult.sellStrategy}, 優化類型: ${existingTypes.join(", ")}`,
      )
    } else {
      // 沒有重複，直接添加新結果
      ...
      console.log(
        `[Cross Optimization] 添加新結果: ${newResult.buyStrategy} + ${newResult.sellStrategy}, 類型: ${newResult.optimizationType}`,
      )
    }
  })
}
...
async function optimizeSingleStrategyParametersFast(strategy, strategyType, strategyInfo, baseParams) {
  try {
    ...
    let bestParams = getDefaultStrategyParams(strategy)
    let bestMetric = targetMetric === "maxDrawdown" ? Number.POSITIVE_INFINITY : Number.NEGATIVE_INFINITY

    // 根據迭代次數決定優化步驟（來自UI設定）
    const iterationLimit = Number.parseInt(document.getElementById("batch-optimize-iteration-limit")?.value) || 6
