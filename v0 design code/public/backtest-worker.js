...
function calculateMA(prices, period) {
  /* ... (程式碼與 Part 1/2 相同) ... */ if (!Array.isArray(prices) || period <= 0 || prices.length < period) {
  ...
  if (iCount < period && prices.length >= period) {
    let firstValidWindowFound = false
    for (let start = 0; start <= prices.length - period; start++) {
      sum = 0
      iCount = 0
      let windowIsValid = true
      for (let k = 0; k < period; k++) {
        if (prices[start + k] !== null && !isNaN(prices[start + k])) {
          sum += prices[start + k]
          iCount++
        } else {
          windowIsValid = false
      ...
      if (windowIsValid && iCount === period) {
        ma[start + period - 1] = sum / period
        firstValidWindowFound = true
        for (let i = start + period; i < prices.length; i++) {
          if (prices[i] === null || isNaN(prices[i]) || prices[i - period] === null || isNaN(prices[i - period])) {
            ma[i] = null
            sum = Number.NaN
            continue
          }
          if (isNaN(sum)) {
            ...
            for (let j = 0; j < period; j++) {
              ...
              if (priceIndex >= 0 && prices[priceIndex] !== null && !isNaN(prices[priceIndex])) {
                recoverySum += prices[priceIndex]
                recoveryCount++
              } else {
                recoveryCount = 0
                break
    ...
    if (!firstValidWindowFound) {
      /* console.warn(`[Worker MA] No valid window of size ${period} found.`); */ return new Array(prices.length).fill(
        null,
      )
    }
  } else if (prices.length < period) {
    /* console.warn(`[Worker MA] prices length ${prices.length} < period ${period}`); */ return new Array(
      prices.length,
    ).fill(null)
  }
  if (iCount === period) {
  ...
  for (let i = period; i < prices.length; i++) {
    ...
    if (isNaN(sum)) {
      ...
      try {
        ...
        /* console.error(`[Worker MA] Error at index ${i}:`, e); */ ma[i] = null
        sum = Number.NaN
      }
    }
  }
...
function calculateEMA(prices, period) {
  ...
  if (validCount < period) return new Array(prices.length).fill(null)
  emaPrev = sum / period
  ema[period - 1] = emaPrev
  for (let i = period; i < prices.length; i++) {
    if (prices[i] === null || isNaN(prices[i]) || emaPrev === null) {
      ema[i] = null
      emaPrev = null
      continue
    }
...
function calculateRSI(prices, period = 14) {
  ...
  for (let i = 1; i < prices.length; i++) {
    if (prices[i] !== null && !isNaN(prices[i]) && prices[i - 1] !== null && !isNaN(prices[i - 1])) {
      changes.push(prices[i] - prices[i - 1])
      if (firstValidIdx === -1) firstValidIdx = i - 1
    } else {
      changes.push(null)
    }
  }
  if (firstValidIdx === -1 || firstValidIdx > prices.length - period - 1) {
    return new Array(prices.length).fill(null)
  }
  let gains = 0
  let losses = 0
  let validCount = 0
  const startIdx = firstValidIdx
  if (startIdx + period > changes.length) return new Array(prices.length).fill(null)
  for (let i = startIdx; i < startIdx + period; i++) {
    if (changes[i] === null) return new Array(prices.length).fill(null)
  ...
  const firstRsiIdx = startIdx + period
  if (firstRsiIdx >= prices.length) return new Array(prices.length).fill(null)
  try {
    if (avgLoss === 0) {
      rsi[firstRsiIdx] = 100
    } else {
      const rs = avgGain / avgLoss
      rsi[firstRsiIdx] = 100 - 100 / (1 + rs)
    }
  } catch (e) {
    rsi[firstRsiIdx] = null
  }
  for (let i = startIdx + period; i < changes.length; i++) {
    const currentChange = changes[i]
    const targetIdx = i + 1
    if (targetIdx >= prices.length) break
    if (currentChange === null || avgGain === null || avgLoss === null) {
      avgGain = null
      avgLoss = null
      rsi[targetIdx] = null
      continue
...
function calculateDIEMA(diValues, period) {
  /* ... (程式碼與 Part 1/2 相同) ... */ if (!Array.isArray(diValues) || period <= 0 || diValues.length < period) {
    return new Array(diValues.length).fill(null)
  }
  const ema = new Array(diValues.length).fill(null)
  let emaPrev = null
  let firstEmaIdx = -1
  let sum = 0
  let count = 0
  for (let i = 0; i < period; i++) {
    if (diValues[i] !== null && !isNaN(diValues[i])) {
      sum += diValues[i]
      count++
    }
  }
  if (count === period) {
    ...
    emaPrev = ema[period - 1]
    firstEmaIdx = period - 1
  } else {
    return ema
  }
  for (let i = period; i < diValues.length; i++) {
    if (diValues[i] === null || isNaN(diValues[i]) || emaPrev === null) {
      ema[i] = null
      emaPrev = null
      continue
    }
    try {
      ema[i] = (emaPrev * (period - 1) + diValues[i] * 2) / (period + 1)
      emaPrev = ema[i]
    } catch (e) {
      ema[i] = null
      emaPrev = null
...
function calculateMACD(highs, lows, closes, shortP = 12, longP = 26, signalP = 9) {
  /* ... (程式碼與上次 Part 3 相同) ... */ const n = highs.length
  if (
    !Array.isArray(highs) ||
    !Array.isArray(lows) ||
  ...
  const diValues = Array(n).fill(null)
  for (let i = 0; i < n; i++) {
    if (
      highs[i] !== null &&
      !isNaN(highs[i]) &&
      ...
      try {
        diValues[i] = (highs[i] + lows[i] + 2 * closes[i]) / 4
      } catch (e) {
        diValues[i] = null
      }
    }
  }
  const emaN = calculateDIEMA(diValues, shortP)
  const emaM = calculateDIEMA(diValues, longP)
  const difLine = Array(n).fill(null)
  const validDifValues = []
  let firstDifIdx = -1
  for (let i = longP - 1; i < n; i++) {
    if (emaN[i] !== null && emaM[i] !== null) {
      try {
        difLine[i] = emaN[i] - emaM[i]
        if (firstDifIdx === -1) firstDifIdx = i
        validDifValues.push({ index: i, value: difLine[i] })
      } catch (e) {
        difLine[i] = null
      }
    }
  ...
  if (validDifValues.length >= signalP) {
    let signalPrev = null
    for (let i = 0; i < validDifValues.length; i++) {
      const currentDifData = validDifValues[i]
      const targetIdx = currentDifData.index
      if (i === signalP - 1) {
        let sum = 0
        let count = 0
        for (let j = 0; j < signalP; j++) {
          if (validDifValues[j].value !== null && !isNaN(validDifValues[j].value)) {
            sum += validDifValues[j].value
            count++
          }
        }
        if (count === signalP) {
          signalLine[targetIdx] = sum / signalP
          signalPrev = signalLine[targetIdx]
        } else {
          signalLine[targetIdx] = null
          signalPrev = null
        }
      } else if (i >= signalP) {
        if (signalPrev !== null && currentDifData.value !== null && !isNaN(currentDifData.value)) {
          try {
            signalLine[targetIdx] = (signalPrev * (signalP - 1) + currentDifData.value * 2) / (signalP + 1)
            signalPrev = signalLine[targetIdx]
          } catch (e) {
            signalLine[targetIdx] = null
            signalPrev = null
  ...
  for (let i = 0; i < n; i++) {
    if (difLine[i] !== null && signalLine[i] !== null) {
      try {
        hist[i] = difLine[i] - signalLine[i]
      } catch (e) {
        hist[i] = null
      }
    }
  }
  return { macd: difLine, signal: signalLine, histogram: hist }
}
function calculateBollingerBands(prices, period = 20, deviations = 2) {
  ...
  for (let i = period - 1; i < prices.length; i++) {
    ...
    for (let j = i - period + 1; j <= i; j++) {
      ...
      try {
        vSum += Math.pow(prices[j] - middle[i], 2)
        count++
      } catch (e) {
        vSum = Number.NaN
        break
    ...
    if (isNaN(vSum) || count < period) {
      upper[i] = null
      lower[i] = null
      continue
    }
    try {
      const stdDev = Math.sqrt(vSum / period)
...
function calculateKD(highs, lows, closes, period = 9) {
  /* ... (程式碼與上次 Part 3 相同) ... */ const n = closes.length
  if (
    !Array.isArray(highs) ||
    !Array.isArray(lows) ||
    !Array.isArray(closes) ||
    ...
    /* console.warn("[Worker KD] Invalid input data or period."); */ return {
      k: Array(n).fill(null),
      d: Array(n).fill(null),
    }
  }
  const rsvArr = Array(n).fill(null)
  const kLine = Array(n).fill(null)
  const dLine = Array(n).fill(null)
  for (let i = period - 1; i < n; i++) {
    let hh = Number.NEGATIVE_INFINITY
    let ll = Number.POSITIVE_INFINITY
    let validPeriod = true
    for (let j = i - period + 1; j <= i; j++) {
      if (j < 0 || highs[j] === null || isNaN(highs[j]) || lows[j] === null || isNaN(lows[j])) {
        validPeriod = false
        break
      }
    ...
    if (!validPeriod || closes[i] === null || isNaN(closes[i])) {
      rsvArr[i] = null
      continue
    }
    try {
      if (hh === ll) {
        rsvArr[i] = i > 0 && rsvArr[i - 1] !== null ? rsvArr[i - 1] : 50
      } else {
        rsvArr[i] = ((closes[i] - ll) / (hh - ll)) * 100
      }
    } catch (e) {
      /* console.error(`[Worker KD] RSV Error at ${i}:`, e); */ rsvArr[i] = null
    }
  }
  let kPrev = null
  let dPrev = null
  for (let i = 0; i < n; i++) {
    const currentRsv = rsvArr[i]
    if (currentRsv === null) {
      kLine[i] = null
      dLine[i] = null
      kPrev = null
    ...
    const yesterdayK = kPrev !== null ? kPrev : 50.0
    const yesterdayD = dPrev !== null ? dPrev : 50.0
    let kNow = null
    let dNow = null
    try {
      kNow = (1 / 3) * currentRsv + (2 / 3) * yesterdayK
      dNow = (1 / 3) * kNow + (2 / 3) * yesterdayD
      kLine[i] = Math.max(0, Math.min(100, kNow))
      dLine[i] = Math.max(0, Math.min(100, dNow))
    } catch (e) {
      /* console.error(`[Worker KD] K/D Error at ${i}:`, e); */ kLine[i] = null
      dLine[i] = null
    }
    kPrev = kLine[i]
    dPrev = dLine[i]
...
function calculateWilliams(highs, lows, closes, period = 14) {
  ...
  const williams = Array(closes.length).fill(null)
  for (let i = period - 1; i < closes.length; i++) {
    let hh = Number.NEGATIVE_INFINITY
    let ll = Number.POSITIVE_INFINITY
    let validP = true
    for (let j = i - period + 1; j <= i; j++) {
      if (highs[j] === null || isNaN(highs[j]) || lows[j] === null || isNaN(lows[j])) {
        validP = false
        break
      }
      hh = Math.max(hh, highs[j])
    ...
    if (!validP || closes[i] === null || isNaN(closes[i])) {
      williams[i] = null
      continue
    }
    try {
      if (hh === ll) {
        williams[i] = i > 0 && williams[i - 1] !== null ? williams[i - 1] : -50
      } else {
        williams[i] = ((hh - closes[i]) / (hh - ll)) * -100
      }
    } catch (e) {
      williams[i] = null
    }
...
function calculateDailyReturns(portfolioVals, dates) {
  /* ... (程式碼與上次 Part 3 相同) ... */ if (!portfolioVals || portfolioVals.length < 2) return []
  const returns = []
  for (let i = 1; i < portfolioVals.length; i++) {
    if (
      portfolioVals[i] !== null &&
      !isNaN(portfolioVals[i]) &&
      portfolioVals[i - 1] !== null &&
      !isNaN(portfolioVals[i - 1]) &&
      portfolioVals[i - 1] !== 0
    ) {
      returns.push(portfolioVals[i] / portfolioVals[i - 1] - 1)
    } else {
      returns.push(0)
    }
  }
...
function calculateSharpeRatio(dailyReturns, annualReturn) {
  /* ... (程式碼與上次 Part 3 相同) ... */ const rfAnnual = 0.01
  ...
  const annExcessReturn = annualReturn / 100 - rfAnnual
  return annStdDev !== 0 ? annExcessReturn / annStdDev : 0
}
function calculateSortinoRatio(dailyReturns, annualReturn) {
  /* ... (程式碼與上次 Part 3 相同) ... */ const targetAnn = 0.01
  const targetDay = Math.pow(1 + targetAnn, 1 / 252) - 1
  if (!dailyReturns || dailyReturns.length === 0) return 0
  const downsideDiffs = dailyReturns.map((r) => Math.min(0, r - targetDay))
  const downsideVar = downsideDiffs.reduce((s, d) => s + Math.pow(d, 2), 0) / dailyReturns.length
  const downsideDev = Math.sqrt(downsideVar)
  if (downsideDev === 0) return Number.POSITIVE_INFINITY
  const annDownsideDev = downsideDev * Math.sqrt(252)
  const annExcessReturn = annualReturn / 100 - targetAnn
  return annDownsideDev !== 0 ? annExcessReturn / annDownsideDev : Number.POSITIVE_INFINITY
}
function calculateMaxDrawdown(portfolioValues) {
  /* ... (程式碼與上次 Part 3 相同) ... */ let peak = Number.NEGATIVE_INFINITY
  let maxDD = 0
...
function formatTWDateWorker(twDate) {
  /* ... (程式碼與上次 Part 3 相同) ... */ try {
    ...
    if (
      isNaN(yInt) ||
      Number.parseInt(m) < 1 ||
      Number.parseInt(m) > 12 ||
      Number.parseInt(d) < 1 ||
      Number.parseInt(d) > 31
    )
      return null
...
async function fetchStockData(stockNo, start, end) {
  /* ... (程式碼與上次 Part 3 相同) ... */ const startDate = new Date(start)
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`Workspace ${stockNo} (${month.substring(0, 6)}) failed: ${response.status}`)
        continue
      }
      ...
      if (data.stat === "OK" && Array.isArray(data.data)) {
        const filtered = data.data
          .map((item) => {
            ...
            if (!isNaN(itemDate) && itemDate >= startDate && itemDate <= endDate) {
              const o = Number.parseFloat(item[3].replace(/,/g, ""))
              const h = Number.parseFloat(item[4].replace(/,/g, ""))
              const l = Number.parseFloat(item[5].replace(/,/g, ""))
              const c = Number.parseFloat(item[6].replace(/,/g, ""))
              const v = Number.parseFloat(item[1].replace(/,/g, ""))
        ...
        /* console.warn(`API status not OK (${stockNo}, ${month.substring(0,6)}): ${data.stat}`); */
      }
    } catch (e) {
      console.error(`Workspace ${stockNo} (${month.substring(0, 6)}) error:`, e)
      continue
    }
  ...
  if (sortedData.length === 0) throw new Error(`指定範圍 (${start} ~ ${end}) 無 ${stockNo} 交易數據`)
  return sortedData
}

// --- 計算所有指標 ---
function calculateAllIndicators(data, params) {
  /* ... (程式碼與上次 Part 3 相同) ... */ self.postMessage({ type: "progress", progress: 55, message: "計算指標..." })
  ...
  try {
    ...
    const shortMAPeriod = ep?.shortPeriod || 5
    const longMAPeriod = ep?.longPeriod || 20
    const exitMAPeriod = xp?.period || ep?.period || longMAPeriod
    indic.maShort = maCalculator(closes, shortMAPeriod)
    indic.maLong = maCalculator(closes, longMAPeriod)
    ...
    if (
      !enableShorting ||
      (macdEntryShort === macdCoverShort && macdEntryLong === macdCoverLong && macdEntrySignal === macdCoverSignal)
    ) {
...
function runStrategy(params, data) {
  ...
  const kdNeedLong =
    entryStrategy === "k_d_cross" || exitStrategy === "k_d_cross_exit"
      ...
      : 0
  const kdNeedShort =
    enableShorting && (shortEntryStrategy === "short_k_d_cross" || shortExitStrategy === "cover_k_d_cross")
  ...
  const macdNeedLong =
    entryStrategy === "macd_cross" || exitStrategy === "macd_cross_exit"
      ...
      : 0
  const macdNeedShort =
    enableShorting && (shortEntryStrategy === "short_macd_cross" || shortExitStrategy === "cover_macd_cross")
      ? (shortEntryParams?.longPeriod || shortExitParams?.longPeriod || 26) +
        (shortEntryParams?.signalPeriod || shortExitParams?.signalPeriod || 9) -
        1
      : 0
  let startIdx = Math.max(1, longestLookback, kdNeedLong, kdNeedShort, macdNeedLong, macdNeedShort) + 1
  ...
  let shortShares = 0
  let lastShortP = 0
  let currentLowSinceShort = Number.POSITIVE_INFINITY
  const shortTrades = []
  const shortCompletedTrades = []
  const shortSigs = []
  const coverSigs = []
  ...
  for (let i = startIdx; i < n; i++) {
    ...
    const canTradeOpen = tradeTiming === "open" && i + 1 < n && check(nextO)
      ...
      try {
        ...
        switch (exitStrategy) {
          ...
          case "ema_cross":
            ...
            if (sellSignal)
              exitIndicatorValues = {
                短SMA: [indicators.maShort[i - 1], indicators.maShort[i], indicators.maShort[i + 1] ?? null],
                長SMA: [indicators.maLong[i - 1], indicators.maLong[i], indicators.maLong[i + 1] ?? null],
              }
            break
          case "ma_below":
            sellSignal =
            ...
            if (sellSignal)
              exitIndicatorValues = {
                ...
                SMA: [indicators.maExit[i - 1], indicators.maExit[i], indicators.maExit[i + 1] ?? null],
              }
            break
          case "rsi_overbought":
            ...
            if (sellSignal) exitIndicatorValues = { RSI: [rPX, rX, indicators.rsiExit[i + 1] ?? null] }
            break
          case "macd_cross":
            ...
            sellSignal = check(difX) && check(deaX) && check(difPX) && check(deaPX) && difX < deaX && difPX >= deaPX
            if (sellSignal)
              exitMACDValues = {
                difPrev: difPX,
                deaPrev: deaPX,
          ...
          case "bollinger_reversal":
            ...
            sellSignal = check(midX) && check(prevC) && check(midPX) && curC < midX && prevC >= midPX
            if (sellSignal)
              exitIndicatorValues = {
                收盤價: [prevC, curC, closes[i + 1] ?? null],
                中軌: [midPX, midX, indicators.bollingerMiddleExit[i + 1] ?? null],
              }
            break
          case "k_d_cross":
            ...
            sellSignal = check(kX) && check(dX) && check(kPX) && check(dPX) && kX < dX && kPX >= dPX && dX > thY
            if (sellSignal)
              exitKDValues = {
                kPrev: kPX,
                dPrev: dPX,
          ...
          case "trailing_stop":
            ...
            if (sellSignal)
              exitIndicatorValues = {
                收盤價: [null, curC, null],
                觸發價: [null, (curPeakP * (1 - trailP / 100)).toFixed(2), null],
              }
            break
          case "price_breakdown":
            ...
            if (i >= bpX) {
              ...
              if (sellSignal)
                exitIndicatorValues = {
                  收盤價: [prevC, curC, closes[i + 1] ?? null],
                  前低: [null, Math.min(...lows.slice(i - bpX, i).filter(check)), null],
                }
            }
            break
          case "williams_overbought":
            ...
            if (sellSignal) exitIndicatorValues = { "%R": [wrPX, wrX, indicators.williamsExit[i + 1] ?? null] }
            break
          case "turtle_stop_loss":
            ...
            if (sellSignal)
              exitIndicatorValues = {
                收盤價: [prevC, curC, closes[i + 1] ?? null],
                N日低: [null, Math.min(...lows.slice(i - slP, i).filter(check)), null],
              }
            break
        ...
        if (sellSignal || slTrig || tpTrig) {
          ...
          if (check(tradePrice) && tradePrice > 0 && longShares > 0) {
            ...
            const profP = entryCostWithFee > 0 ? (prof / entryCostWithFee) * 100 : 0
            longCap += rev
            const tradeData = {
              type: "sell",
              date: tradeDate,
            ...
            if (exitIndicatorValues) tradeData.indicatorValues = exitIndicatorValues
            longTrades.push(tradeData)
            ...
            if (lastBuyIdx !== -1 && longTrades[lastBuyIdx].shares === longShares) {
              ...
              console.warn(`[Worker LONG] Sell @ ${tradeDate} could not find matching buy trade.`)
            }
            console.log(
        ...
        console.error(`[Worker LONG EXIT] Error at index ${i} (${dates[i]}):`, exitError)
      }
    }
      ...
      try {
        ...
        switch (shortExitStrategy) {
          ...
          case "cover_rsi_oversold":
            ...
            if (coverSignal) coverIndicatorValues = { RSI: [rPC, rC, indicators.rsiCover[i + 1] ?? null] }
            break
          case "cover_macd_cross":
            ...
            coverSignal = check(difC) && check(deaC) && check(difPC) && check(deaPC) && difC > deaC && difPC <= deaPC
            if (coverSignal)
              coverMACDValues = {
                difPrev: difPC,
                deaPrev: deaPC,
          ...
          case "cover_bollinger_breakout":
            ...
            coverSignal = check(upperC) && check(prevC) && check(upperPC) && curC > upperC && prevC <= upperPC
            if (coverSignal)
              coverIndicatorValues = {
                收盤價: [prevC, curC, closes[i + 1] ?? null],
                上軌: [upperPC, upperC, indicators.bollingerUpperCover[i + 1] ?? null],
              }
            break
          case "cover_k_d_cross":
            ...
            coverSignal = check(kC) && check(dC) && check(kPC) && check(dPC) && kC > dC && kPC <= dPC && dC < thXC
            if (coverSignal)
              coverKDValues = {
                kPrev: kPC,
                dPrev: dPC,
          ...
          case "cover_price_breakout":
            ...
            if (i >= bpC) {
              ...
              if (coverSignal)
                coverIndicatorValues = {
                  收盤價: [prevC, curC, closes[i + 1] ?? null],
                  前高: [null, Math.max(...highs.slice(i - bpC, i).filter(check)), null],
                }
            }
            break
          case "cover_williams_oversold":
            ...
            if (coverSignal) coverIndicatorValues = { "%R": [wrPC, wrC, indicators.williamsCover[i + 1] ?? null] }
            break
          case "cover_turtle_breakout":
            ...
            if (i >= tpC) {
              ...
              if (coverSignal)
                coverIndicatorValues = {
                  收盤價: [prevC, curC, closes[i + 1] ?? null],
                  N日高: [null, Math.max(...highs.slice(i - tpC, i).filter(check)), null],
                }
            }
            break
          case "cover_trailing_stop":
            ...
            if (check(curL) && lastShortP > 0) {
              ...
              coverSignal = curC > currentLowSinceShort * (1 + shortTrailP / 100)
            }
            if (coverSignal)
              coverIndicatorValues = {
                收盤價: [null, curC, null],
                觸發價: [null, (currentLowSinceShort * (1 + shortTrailP / 100)).toFixed(2), null],
              }
            break
        ...
        if (coverSignal || shortSlTrig || shortTpTrig) {
          ...
          if (check(tradePrice) && tradePrice > 0 && shortShares > 0) {
            ...
            const coverCostWithFee = shortShares * tradePrice * (1 + buyFee / 100)
            ...
            if (coverIndicatorValues) tradeData.indicatorValues = coverIndicatorValues
            shortTrades.push(tradeData)
            // 修正：隔日開盤價交易時，訊號應顯示在實際交易日
            if (tradeTiming === "close" || !canTradeOpen) {
            ...
            if (lastShortIdx !== -1 && shortTrades[lastShortIdx].shares === shortShares) {
              ...
              console.warn(`[Worker SHORT] Cover @ ${tradeDate} could not find matching short trade.`)
            }
            console.log(
            ...
            shortShares = 0
            lastShortP = 0
            currentLowSinceShort = Number.POSITIVE_INFINITY
          } else {
            console.warn(
        ...
        console.error(`[Worker SHORT EXIT] Error at index ${i} (${dates[i]}):`, coverError)
      }
    }
    if (longPos === 0 && shortPos === 0) {
      ...
      switch (entryStrategy) {
        ...
        case "rsi_oversold":
          ...
          if (buySignal) entryIndicatorValues = { RSI: [rPE, rE, indicators.rsiEntry[i + 1] ?? null] }
          break
        case "macd_cross":
          ...
          buySignal = check(difE) && check(deaE) && check(difPE) && check(deaPE) && difE > deaE && difPE <= deaPE
          if (buySignal)
            entryMACDValues = {
              difPrev: difPE,
              deaPrev: deaPE,
        ...
        case "k_d_cross":
          ...
          buySignal = check(kE) && check(dE) && check(kPE) && check(dPE) && kE > dE && kPE <= dPE && dE < thX
          if (buySignal)
            entryKDValues = {
              kPrev: kPE,
              dPrev: dPE,
        ...
        case "volume_spike":
          ...
          if (buySignal)
            entryIndicatorValues = {
              ...
              均量: [indicators.volumeAvgEntry[i - 1] ?? null, vAE, indicators.volumeAvgEntry[i + 1] ?? null],
            }
          break
        case "price_breakout":
          ...
          if (i >= bpE) {
            ...
            if (hsE.length > 0) {
              ...
              if (buySignal)
                entryIndicatorValues = { 收盤價: [prevC, curC, closes[i + 1] ?? null], 前高: [null, periodHigh, null] }
            }
          }
          break
        case "williams_oversold":
          ...
          if (buySignal) entryIndicatorValues = { "%R": [wrPE, wrE, indicators.williamsEntry[i + 1] ?? null] }
          break
        case "turtle_breakout":
          ...
          if (i >= tpE) {
            ...
            if (buySignal)
              entryIndicatorValues = {
                收盤價: [prevC, curC, closes[i + 1] ?? null],
                N日高: [null, Math.max(...highs.slice(i - tpE, i).filter(check)), null],
              }
          }
          break
      }
    ...
    if (enableShorting && shortPos === 0 && longPos === 0) {
      ...
      switch (shortEntryStrategy) {
        ...
        case "short_rsi_overbought":
          ...
          if (shortSignal) shortEntryIndicatorValues = { RSI: [rPSE, rSE, indicators.rsiShortEntry[i + 1] ?? null] }
          break
        case "short_macd_cross":
          ...
          shortSignal =
            check(difSE) && check(deaSE) && check(difPSE) && check(deaPSE) && difSE < deaSE && difPSE >= deaSE
          if (shortSignal)
        ...
        case "short_bollinger_reversal":
          ...
          shortSignal = check(midSE) && check(prevC) && check(midPSE) && curC < midSE && prevC >= midPSE
          if (shortSignal)
            shortEntryIndicatorValues = {
              收盤價: [prevC, curC, closes[i + 1] ?? null],
              中軌: [midPSE, midSE, indicators.bollingerMiddleShortEntry[i + 1] ?? null],
            }
          break
        case "short_k_d_cross":
          ...
          shortSignal = check(kSE) && check(dSE) && check(kPSE) && check(dPSE) && kSE < dSE && kPSE >= dPSE && dSE > thY
          if (shortSignal)
            shortEntryKDValues = {
              kPrev: kPSE,
              dPrev: dPSE,
        ...
        case "short_price_breakdown":
          ...
          if (i >= bpSE) {
            ...
            if (shortSignal)
              shortEntryIndicatorValues = {
                收盤價: [prevC, curC, closes[i + 1] ?? null],
                前低: [null, Math.min(...lows.slice(i - bpSE, i).filter(check)), null],
              }
          }
          break
        case "short_williams_overbought":
          ...
          shortSignal = check(wrSE) && check(wrPSE) && wrSE < wrThSE && wrPSE >= wrThSE
          if (shortSignal)
            shortEntryIndicatorValues = { "%R": [wrPSE, wrSE, indicators.williamsShortEntry[i + 1] ?? null] }
          break
        case "short_turtle_stop_loss":
          ...
          if (shortSignal)
            shortEntryIndicatorValues = {
              收盤價: [prevC, curC, closes[i + 1] ?? null],
              N日低: [null, Math.min(...lows.slice(i - slPSE, i).filter(check)), null],
            }
          break
      }
      if (shortSignal) {
        ...
        if (check(tradePrice) && tradePrice > 0 && shortCap > 0) {
          ...
          if (adjustedTradePrice <= 0) {
            ...
            shortShares = Math.floor(actualInvestmentLimit / adjustedTradePrice)
          }
          if (shortShares > 0) {
            ...
            if (shortEntryMACDValues) tradeData.macdValues = shortEntryMACDValues
            if (shortEntryIndicatorValues) tradeData.indicatorValues = shortEntryIndicatorValues
            shortTrades.push(tradeData)
            // 修正：隔日開盤價交易時，訊號應顯示在實際交易日
            if (tradeTiming === "close" || !canTradeOpen) {
              shortSigs.push({ date: dates[i], index: i })
          ...
          console.warn(`[Worker SHORT] Invalid trade price (${tradePrice}) for Short Signal on ${dates[i]}`)
        }
      }
    }
    ...
    maxDD = Math.max(maxDD, drawdown)
    if (i > startIdx && n > startIdx && i % Math.floor((n - startIdx) / 20 || 1) === 0) {
  ...
  try {
    const lastIdx = n - 1
    const finalP = lastIdx >= 0 && check(closes[lastIdx]) ? closes[lastIdx] : null
    if (longPos === 1 && finalP !== null && longShares > 0) {
      ...
      if (!sellSigs.some((s) => s.index === lastIdx)) sellSigs.push({ date: dates[lastIdx], index: lastIdx })
      ...
      longPos = 0
      longShares = 0
      console.log(`[Worker LONG] Final Sell Executed: ${finalTradeData.shares}@${finalP} on ${dates[lastIdx]}`)
    } else if (longPos === 1) {
    ...
    if (shortPos === 1 && finalP !== null && shortShares > 0) {
      ...
      if (!coverSigs.some((s) => s.index === lastIdx)) coverSigs.push({ date: dates[lastIdx], index: lastIdx })
      ...
      shortPos = 0
      shortShares = 0
      console.log(`[Worker SHORT] Final Cover Executed: ${finalTradeData.shares}@${finalP} on ${dates[lastIdx]}`)
    } else if (shortPos === 1) {
    ...
    strategyReturns[lastIdx] =
      initialCapital > 0 ? ((portfolioVal[lastIdx] - initialCapital) / initialCapital) * 100 : 0
    ...
    allTrades = [...longTrades, ...shortTrades].sort((a, b) => new Date(a.date) - new Date(b.date))
    totalWinTrades = allCompletedTrades.filter((t) => (t.profit || 0) > 0).length
    ...
    if (firstDateStr && lastDateStr) {
      ...
      if (firstValidPriceIdxBH !== -1 && lastValidPriceIdxBH !== -1 && lastValidPriceIdxBH >= firstValidPriceIdxBH) {
        ...
        const bhTotalReturn =
          firstValidPriceBH !== 0 ? ((lastValidPriceBH - firstValidPriceBH) / firstValidPriceBH) * 100 : 0
        if (bhYears > 1 / (365.25 * 2) && firstValidPriceBH > 0) {
          try {
            buyHoldAnnualizedReturn = (Math.pow(lastValidPriceBH / firstValidPriceBH, 1 / bhYears) - 1) * 100
          } catch {
    ...
    if (validDataLength >= 4) {
      ...
      if (firstHalfPortfolio.length > 1) {
        ...
        const firstHalfEndVal = firstHalfPortfolio[firstHalfPortfolio.length - 1]
        const totalReturnHalf1 = firstHalfStartVal !== 0 ? (firstHalfEndVal / firstHalfStartVal - 1) * 100 : 0
        annReturnHalf1 = totalReturnHalf1
        const avgDailyReturn1 = firstHalfDailyReturns.reduce((s, r) => s + r, 0) / firstHalfDailyReturns.length
        const variance1 =
          firstHalfDailyReturns.reduce((s, r) => s + Math.pow(r - avgDailyReturn1, 2), 0) / firstHalfDailyReturns.length
        const stdDev1 = Math.sqrt(variance1)
        const annStdDev1 = stdDev1 * Math.sqrt(252)
        const approxAnnReturn1 = firstHalfDailyReturns.length > 0 ? avgDailyReturn1 * 252 * 100 : 0
      ...
      if (secondHalfPortfolio.length > 1) {
        ...
        const secondHalfEndVal = secondHalfPortfolio[secondHalfPortfolio.length - 1]
        const totalReturnHalf2 = secondHalfStartVal !== 0 ? (secondHalfEndVal / secondHalfStartVal - 1) * 100 : 0
        ...
        const variance2 =
          secondHalfDailyReturns.reduce((s, r) => s + Math.pow(r - avgDailyReturn2, 2), 0) /
          secondHalfDailyReturns.length
        const stdDev2 = Math.sqrt(variance2)
        const annStdDev2 = stdDev2 * Math.sqrt(252)
        const approxAnnReturn2 = secondHalfDailyReturns.length > 0 ? avgDailyReturn2 * 252 * 100 : 0
    ...
    const totalDaysApprox = Math.max(1, totalDurationMillis / (1000 * 60 * 60 * 24))
    ...
    for (const [label, months] of Object.entries(periodsToCalculate)) {
      ...
      if (subStartIdx <= lastIdx) {
        ...
        const subPortfolioVals = portfolioVal.slice(subStartIdx, subEndIdx + 1).filter((v) => check(v))
        const subBHRawPrices = closes.slice(subStartIdx, subEndIdx + 1).filter((v) => check(v))
        const subDates = dates.slice(subStartIdx, subEndIdx + 1)
        if (subPortfolioVals.length > 1 && subDates.length > 1 && subBHRawPrices.length > 1) {
          const subStartVal = subPortfolioVals[0]
          const subEndVal = subPortfolioVals[subPortfolioVals.length - 1]
          const subTotalReturn = subStartVal !== 0 ? ((subEndVal - subStartVal) / subStartVal) * 100 : 0
          const subStartBHPrice = subBHRawPrices[0]
          const subEndBHPrice = subBHRawPrices[subBHRawPrices.length - 1]
          const subBHTotalReturn =
            subStartBHPrice !== 0 ? ((subEndBHPrice - subStartBHPrice) / subStartBHPrice) * 100 : 0
          const subDailyReturns = calculateDailyReturns(subPortfolioVals, subDates)
          const subAnnualizedReturn = 0
          const subSharpe = calculateSharpeRatio(subDailyReturns, subAnnualizedReturn)
...
async function runOptimization(baseParams, optimizeTargetStrategy, optParamName, optRange, useCache, cachedData) {
  ...
  for (let val = range.from; val <= range.to; val += range.step) {
    const curVal = Number.parseFloat(val.toFixed(4))
    ...
    if (optimizeTargetStrategy === "risk") {
      ...
      if (optimizeTargetStrategy === "entry") targetObjKey = "entryParams"
        ...
        console.warn(`[Worker Opt] Unknown strategy optimization type: ${optimizeTargetStrategy}`)
        continue
      }
  ...
  results.sort((a, b) => {
    const rA =
      a?.annualizedReturn !== null && isFinite(a.annualizedReturn) ? a.annualizedReturn : Number.NEGATIVE_INFINITY
    const rB =
      b?.annualizedReturn !== null && isFinite(b.annualizedReturn) ? b.annualizedReturn : Number.NEGATIVE_INFINITY
    if (rB !== rA) return rB - rA
    const dda = a?.maxDrawdown ?? Number.POSITIVE_INFINITY
    const ddb = b?.maxDrawdown ?? Number.POSITIVE_INFINITY
    if (dda !== ddb) return dda - ddb
    const sA = isFinite(a?.sortinoRatio) ? a.sortinoRatio : Number.NEGATIVE_INFINITY
    const sB = isFinite(b?.sortinoRatio) ? b.sortinoRatio : Number.NEGATIVE_INFINITY
    return sB - sA
  })
...
function runSuggestionSimulation(params, recentData) {
  ...
  const checkParamLookback = (pObj) => {
    Object.values(pObj || {}).forEach((v) => {
      if (typeof v === "number" && !isNaN(v) && v > minLookbackSuggestion) minLookbackSuggestion = v
    })
  }
  ...
  if (
    ...
    (enableShorting && (shortEntryStrategy.includes("macd") || shortExitStrategy.includes("macd")))
  )
  ...
  if (
    ...
    (enableShorting && (shortEntryStrategy.includes("k_d") || shortExitStrategy.includes("k_d")))
  )
    minLookbackSuggestion = Math.max(minLookbackSuggestion, entryParams?.period || 9)
  if (
    entryStrategy.includes("turtle") ||
    exitStrategy.includes("turtle") ||
    (enableShorting && (shortEntryStrategy.includes("turtle") || shortExitStrategy.includes("turtle")))
  )
    minLookbackSuggestion = Math.max(
      minLookbackSuggestion,
  ...
  const lastShortP = 0
  const curPeakP = 0
  const currentLowSinceShort = Number.POSITIVE_INFINITY
    ...
    switch (shortEntryStrategy) {
      ...
      case "short_macd_cross":
        const difSE = indicators.macdShortEntry[i],
          ...
          difPSE = indicators.macdShortEntry[i - 1],
          deaPSE = indicators.macdSignalShortEntry[i - 1]
        shortSignal = check(difSE) && check(deaSE) && check(difPSE) && check(deaPSE) && difSE < deaSE && difPSE >= deaSE
        break
      case "short_bollinger_reversal":
        const midSE = indicators.bollingerMiddleShortEntry[i]
        const midPSE = indicators.bollingerMiddleShortEntry[i - 1]
  ...
  try {
    if (type === "runBacktest") {
      ...
      const optOutcome = await runOptimization(
        ...
        optimizeRange,
        useCachedData,
        cachedData || self.workerCachedStockData,
      )
      self.postMessage({ type: "result", data: optOutcome })
    } else if (type === "getSuggestion") {
      console.log("[Worker] Received getSuggestion request.")
      if (!self.workerCachedStockData) {
        throw new Error("Worker 中無可用快取數據，請先執行回測。")
      }
      if (self.workerCachedStockData.length < lookbackDays) {
        throw new Error(`Worker 快取數據不足 (${self.workerCachedStockData.length})，無法回看 ${lookbackDays} 天。`)
      }
