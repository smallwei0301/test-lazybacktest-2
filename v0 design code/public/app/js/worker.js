...
/**
 * [Patch: LB-WORKER-BATCH-FETCH-20251212A] 將年份陣列分組為連續區間
 * 例如: [2021, 2022, 2024] -> [{start: 2021, end: 2022}, {start: 2024, end: 2024}]
 * @param {number[]} years - 年份陣列
 * @returns {{start: number, end: number}[]} - 連續區間陣列
 */
function groupConsecutiveYears(years) {
...
async function tryFetchRangeFromBlob({
  ...
  if (startGapExceeded || endGapExceeded) {
    ...
    console.warn(
      `[Worker] ${stockNo} Netlify Blob 範圍資料覆蓋不足 (startGap=${startGap ?? "N/A"
  ...
  if (
    ...
    if (rangeFetchInfo.patch?.status === 'partial-fetch') {
      ...
      if (reason === 'before-2pm-data-not-updated-yet') {
        ...
        rangeFetchInfo.status = "current-month-stale";
        console.warn(
          `[Worker] ${stockNo} 當月補齊被跳過 (${reason})，缺少 ${normalizedCurrentMonthGap} 天資料 (last=${lastDate || "N/A"
      ...
      console.warn(
        `[Worker] ${stockNo} Netlify Blob 範圍資料仍缺少當月最新 ${normalizedCurrentMonthGap} 天 (last=${lastDate || "N/A"
...
async function fetchStockData(
  ...
  if (adjusted) {
    ...
    // Patch: LB-INVALID-DATA-FALLBACK-20251202E — 針對還原股價的無效資料進行補救
    const invalidAdjustedRows = detectInvalidRows(adjustedRows);
    if (invalidAdjustedRows.length > 0) {
      console.warn(`[Worker] 檢測到 ${invalidAdjustedRows.length} 筆無效還原價資料，嘗試補齊...`);

      const fallbackData = await fetchFallbackForInvalidDates({
        stockNo,
        marketKey,
        invalidRows: invalidAdjustedRows,
        adjusted: true,
      });

      if (fallbackData.length > 0) {
        console.log(`[Worker] 成功補齊 ${fallbackData.length} 筆還原價資料`);
        const merged = [...adjustedRows, ...fallbackData];
        adjustedRows = dedupeAndSortData(merged);
      }
    }

    // 最終過濾無效收盤價
    adjustedRows = filterInvalidClosePriceRows(adjustedRows);
  ...
  if (invalidRows.length > 0) {
    ...
    if (fallbackData.length > 0) {
      console.log(`[Worker] 成功補齊 ${fallbackData.length} 筆原始資料`);
      const merged = [...deduped, ...fallbackData];
      deduped = dedupeAndSortData(merged);
    }
  }

  // 最終過濾無效收盤價
  deduped = filterInvalidClosePriceRows(deduped);
    ...
    if (i > startIdx && tradeTiming === "open") {
      ...
      if (pendingTrade && pendingTrade.executeOnDate === dates[i]) {
        ...
        if (pendingTrade.type === "buy") {
          ...
          // LB-STAGED-EXIT-FIX-20251209A: 執行隔日分批出場
          const tradePrice = actualTradePrice;
          const executedShares = pendingTrade.sharesToSell;
          const exitStrategy = pendingTrade.strategy;
          const slTrig = pendingTrade.slTrig;
          const tpTrig = pendingTrade.tpTrig;
          const exitTriggerLabel = pendingTrade.stageTrigger;
          const stageIndexForPlan = pendingTrade.stageIndex;
          const riskExit = pendingTrade.riskExit;
          ...
          const stagePercent =
            exitStagePercents[
            Math.min(stageIndexForPlan, exitStagePercents.length - 1)
            ] || 0;
          ...
          const tradeData = {
            ...
            allocationPercent: stagePercent,
            cumulativeStagePercent: cumulativePercent,
            plannedShares: pendingTrade.plannedShares,
            consumedEntryStages: consumption.stages || [],
          };
          ...
          console.log(
            `[Worker LONG] Delayed Sell Executed: ${executedShares}@${tradePrice} on ${dates[i]}`,
          );
        }
        pendingNextDayTrade = null;
      }
      ...
      try {
        ...
        if (sellSignal || slTrig || tpTrig || priceRallyTrigger) {
          ...
          if (tradeTiming === "close") {
            ...
            // LB-STAGED-EXIT-FIX-20251209A: 隔日開盤交易改為 pending
            isPending = true;
            tradePrice = nextO;
            tradeDate = dates[i + 1];
          ...
          // check(tradePrice) 在 pending 時檢查 nextO，在當日成交時檢查 curC
          if (check(tradePrice) && tradePrice > 0 && longShares > 0) {
            ...
            if (isPending) {
              ...
              pendingNextDayTrade = {
                ...
                plannedShares:
                  Array.isArray(currentLongExitPlan) &&
                    ...
                    : sharesToSell,
                kdValues: exitKDValues,
                macdValues: exitMACDValues,
                indicatorValues: exitIndicatorValues,
              };
              console.log(`[Worker LONG] Pending Sell Created for ${tradeDate}`);
            } else {
              // 立即執行交易 (原邏輯)
              ...
              if (executedShares > 0) {
                ...
                if (longShares > 0 && longPositionCostWithoutFee > 0) {
                  longAverageEntryPrice =
                    longPositionCostWithoutFee / longShares;
                  longPos = 1;
                  lastBuyP = longAverageEntryPrice;
                } else {
                  longAverageEntryPrice = 0;
    ...
    if (type === "runBacktest") {
      ...
      if (!evaluation && finalStateDiagnostics?.snapshot) {
        ...
        evaluationRecoveryNotes = [
          `finalEvaluation 由 finalState 快照重建：${fallbackMeta.fallbackFromDate || derivedDate || "未知日期"
      ...
      if (!evaluation || !latestDate) {
        ...
        if (Array.isArray(coverage) && coverage.length > 0) {
          developerNotes.push(
            `覆蓋區段 ${coverage.length} 段，fingerprint ${coverageFingerprint || "N/A"
            }`,
          );
        }
      ...
      if (evaluationMeta && evaluationMeta.fallback) {
        ...
        developerNotes.push(
          `finalEvaluation fallback：${fallbackFromDate || "N/A"} ← ${requestedLastDate || "N/A"
          }`,
        );
        if (
          ...
          developerNotes.push(
            `fallback 落後 ${evaluationMeta.fallbackLagDays} 日（bars=${Number.isFinite(evaluationMeta.fallbackLagBars)
