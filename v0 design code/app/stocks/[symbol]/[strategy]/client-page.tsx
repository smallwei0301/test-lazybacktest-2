...
export default function StrategyClientPage({ stock, strategy }: StrategyClientPageProps) {
  ...
  // 2. Determine Performance Context (Modular Logic: ROI, WinRate, MDD)
  // This logic allows future data reading to judge based on these 3 criteria
  const getPerformanceContext = (s: Strategy) => {
    const mdd = s.maxDrawdown || 15 // Default mock MDD if missing
    let score = 0
    
    // ROI Criteria (Annualized)
    if (s.roi > 15) score += 2
    else if (s.roi > 5) score += 1
    else if (s.roi < 0) score -= 1

    // WinRate Criteria
    if (s.winRate > 60) score += 2
    else if (s.winRate > 50) score += 1
  ...
  const performanceContext = getPerformanceContext(strategy)
  // Map context to existing text bank categories
  ...
  const isFailure = strategy.roi < 5
  const statusQuestion = isFailure ? "失效了嗎" : "真的準嗎"
  const adverb = strategy.roi < 5 ? "僅" : (strategy.roi > 20 ? "高達" : "為")
    ...
    const qas = [
      {
        ...
        a: `根據我們的回測數據，${strategy.name} 在 ${stock.name} 上的年化報酬率為 ${strategy.roi}%。${
          strategy.roi > 10 
            ? "這是一個表現優異的策略，值得納入投資組合參考。" 
            : "目前的表現低於市場平均，建議觀望或尋找其他替代策略。"
        } 數據顯示「冠軍策略」通常能提供更穩定的年化報酬。`
      },
      {
        q: `為什麼 ${strategy.name} 的年化報酬率${strategy.roi < 5 ? "只有" : "是"} ${strategy.roi}%？`,
        a: `年化報酬率反映了策略在特定市場環境下的長期獲利能力。${
          strategy.roi < 5 
