## 2026-09-10 — Patch LB-OPT-ENTRY-20240910A
- **Scope**: 參數優化 DSL 同步與買入紀錄指標顯示。
- **Updates**:
  - `js/worker.js`（`LB-OPT-DSL-SYNC-20240910A`）為單參數優化建立策略 DSL 深拷貝並在每個步進值同步目標參數，避免倍數或其他參數於優化表格出現相同績效。
  - `js/worker.js`（`LB-TRADE-ENTRY-INDICATOR-20240910A`）匯總做多進場時的指標、KD、MACD 診斷資訊，讓交易紀錄的買入段可與賣出段一樣顯示前中後三日數值。
