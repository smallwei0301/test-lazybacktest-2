## 2025-09-30 — Patch LB-ROLLING-OPT-20250930A
- **Scope**: Walk-Forward 滾動測試新增訓練期參數最佳化流程。
- **Updates**:
  - 新增訓練期自動最佳化控制項，可指定目標指標與最大樣本數，於每個訓練視窗前搜尋最優參數後再進行測試。
  - `js/rolling-test.js` 擴充最佳化取樣、進度估算與報表摘要，逐窗評語會附上採用的最佳參數組合。
  - 調整進度條步數計算與 Worker 參數覆寫流程，確保訓練期與測試期皆採用相同的最佳化結果。
- **Testing**: 待前端實際執行 Walk-Forward 測試確認瀏覽器環境無錯誤。
