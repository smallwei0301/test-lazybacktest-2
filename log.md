## 2026-08-02 — Patch LB-STRATEGY-VERIFY-20250715A
- **Scope**: 策略註冊驗證清單擴充與抽樣回測隨機組合。
- **Updates**:
  - `js/main.js` 建立完整策略驗證清單並依名稱分組排序，新增可捲動樣式與結果渲染邏輯，抽樣回測改為隨機挑選做多／做空策略組合並於狀態訊息揭露選用策略。
  - `index.html` 更新開發者卡片註記，提醒可用滾輪瀏覽驗證清單並說明抽樣回測改為隨機策略組合。
  - `js/strategy-plugins/*` 與 `js/strategy-plugin-manifest.js` 調整策略顯示名稱，使其與 UI 下拉選項一致以避免驗證時產生名稱不符的提醒。
- **Testing**: 容器環境無法連線 Proxy／啟動瀏覽器，僅靜態檢查腳本語法；待實機透過開發者卡片驗證策略清單與抽樣回測。
