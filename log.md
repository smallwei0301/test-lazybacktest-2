...
## 2025-09-30 — Patch LB-STAGE4-REFINE-20250930A
- **Scope**: 批量優化第四階段（局部微調）入口與演算法。
- **Updates**:
  - 新增 `spsa-runner.js`、`cem-runner.js` 兩個 ES module，實作 SPSA 與 CEM 的局部微調流程，並提供進度回報 hook。
  - `batch-optimization.js` 匯出 `runStage4`/`paramKey` 與 Stage4 評估上下文，結果自動去重並沿用原始結果表呈現與匯出流程。
  - `main.js` 加入「第四階段：局部微調」面板與按鈕事件，顯示進度與完成提示；`index.html` 補上對應 UI。
- **Testing**: 前端本地載入檢查（需後續於實際回測流程驗證）。
