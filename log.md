...
- **Issue recap**: GA 校正流程僅以訓練誤差作為參考，未將 MSE 轉為適應函數，導致演化方向與均方誤差最小化脫節、UI 亦缺乏基準對照。
- **Fix**: 以 `1 / (1 + MSE)` 建立 GA 適應值並保留菁英個體，回傳基準與最佳 MSE 供後續測試沿用；預測摘要與指標卡同步揭露 MSE 改善幅度與訓練期基準。
- **Diagnostics**: 於本地多次重跑預測流程，觀察 GA 搜尋改善時 `forecastStatus` 與誤差卡片皆會標示基準與改善幅度，確認沒有硬編碼或提前引用未來資料。
- **Testing**: `node - <<'NODE' const fs=require('fs');const vm=require('vm');['js/backtest.js','js/forecast.js'].forEach((file)=>{const code=fs.readFileSync(file,'utf8');new vm.Script(code,{filename:file});});console.log('scripts compile');NODE`
