...
- **Issue recap**: 插件化後仍需手動 import 個別策略模組，缺少統一清冊供 UI 查詢與核心動態載入，也未對 meta.id 與 `paramsSchema` 進行一致性檢查。
- **Fix**:
  - `js/strategy-plugin-registry.js` 新增 `registerStrategyLoader`、`getStrategyById`、`listStrategies` 等 API，強制 meta.id 唯一並檢查 `paramsSchema` 結構；同時保留 `register`／`get`／`list` 舊介面供 shim 過渡。
  - 新增 `js/strategy-plugin-manifest.js` 建立 20 支原子策略的註冊清單與同步 loader，支援 Web Worker `importScripts` 與瀏覽器同步 XHR 載入，並釋出 `LazybacktestStrategyCatalog` 供 UI 測試卡片快速列舉。
  - `js/worker.js` 改為載入集中式 manifest，由 Registry 取得策略執行體；`index.html` 預載 Registry 與 manifest，讓主執行緒也能呼叫 `listStrategies`。
- **Diagnostics**: 待於可連線 Proxy 的環境以 UI 呼叫 `StrategyPluginRegistry.listStrategies()` 建立策略列表並逐一回測，確認未註冊策略不會出現且透過 Registry 呼叫時訊號與診斷與舊流程一致。
- **Testing**: `npm run typecheck`（驗證 Registry 與 manifest JSDoc 型別）；Proxy 受限暫無法實際回測，已列入後續人工覆核清單。
