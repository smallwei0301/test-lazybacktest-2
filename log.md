...
## 2025-12-12 — Patch LB-FORECAST-LSTMGA-20251212A
- **Issue recap**: 原始 LSTM+GA 流程僅以 RMSE 評估並直接調整殘差權重，未落實文獻所述的累積誤差閾值校正法，且無法針對均方誤差 (MSE) 尋找最佳閾值。
- **Fix**: 導入以 MSE 為適應函數的遺傳演算法，演化單一閾值 δ，依據實際股價乘上 δ 的門檻累加誤差，並保留訓練尾端累積值於測試階段接續使用，避免回溯修正既有預測。
- **Diagnostics**: 以本地可見股價資料實測，確認 GA 會輸出 δ 與觸發次數、曲線改以 MSE 衡量，預測摘要與版面提示皆更新為「無未來資料回寫」。
