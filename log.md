## 2026-09-10 — Patch LB-VOLUME-OPT-ENTRYMETA-20240910A
- **Scope**: 成交量暴增優化參數同步與交易紀錄指標補齊。
- **Updates**:
  - `js/worker.js` 在單參數優化流程同步更新策略 DSL 節點，使倍率等設定能隨掃描值改寫，修正批量／參數優化結果一致的問題。
  - `js/worker.js` 聚合多段進場資料時保留指標、KD/MACD 三日快照，讓買入交易紀錄可顯示前中後三日的指標資訊。
