...
## 2026-09-18 — Patch LB-BATCH-MAP-HYDRATE-20260918A
- **Issue recap**: 批量優化的映射模組無法取得策略清單，因 `strategyDescriptions` 只存在於模組內部常數，`LazyBatchStrategyMapper` 初始化時拿到空集合，導致所有策略都被視為未知項目。
- **Fix**:
  - `js/config.js` 將策略描述同步寫入 `globalThis.strategyDescriptions`，必要時合併既有對照，確保批量映射與其他腳本能共用同一份資料來源。
  - `tests/batch-mapper.test.js` 新增「global mapper hydrates strategy descriptions from config」案例，透過重新載入模組驗證全域映射確實包含新舊死亡交叉策略，避免回歸。
- **Diagnostics**: 建議重新載入批量優化面板並啟用批量偵錯日誌，確認 `strategy-map-hydrated` 事件中的 `missingCount` 變為 0，策略選單可正常呈現。
- **Testing**: `npm test`（涵蓋新增測試）、`npm run typecheck`
