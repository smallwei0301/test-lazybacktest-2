...
- **Issue recap**: OOS 品質在多數指標未達門檻時仍可能顯示高分、Walk-Forward 總分需統一為 0～100 分，報告摘要與表格也缺乏細節與清楚的等級呈現。
- **Fix**:
  - `js/rolling-test.js` 將 OOS 品質分數改為「正規化平均 × 指標達標權重比」，並回傳各指標的門檻、目標、權重與評分，供卡片摺疊明細顯示。
  - 儀表板新增獨立的等級卡片，所有分數卡支援折疊說明，並以 `clamp01` 確保 Walk-Forward 總分換算後落在 0～100 分。
  - 逐窗指標表改為「指標 × 視窗」的轉置形式，列出訓練／測試期間、樣本日數、PSR／DSR、WFE、窗分數、參數摘要與評語。
  - 移除測試成交筆數門檻與相關敘述，通過視窗比例僅依既有績效門檻計算。
  - 調整滾動測試規劃更新流程，主回測完成後最多自動重試三次刷新，避免殘留「請先執行一次主回測」。
- **Docs**: `README.md` 更新為 LB-ROLLING-TEST-20251028A，說明 OOS 品質乘上達標權重比、儀表板摺疊明細、轉置表格與自動刷新策略。
