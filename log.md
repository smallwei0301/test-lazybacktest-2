...
## 2026-08-20 — Patch LB-CACHE-COVERAGE-20260820A
- **Issue recap**: 使用者將結束日期設為當日時，即便資料來源已有最新收盤價，仍可能沿用舊快取，導致回測停留在前一交易日。快取覆蓋狀態僅依請求區間標記，未反映實際資料行尾日期。
- **Fix**:
  - `js/backtest.js` 在主執行緒合併資料與落地快取時改以 `computeCoverageFromRows` 重新計算覆蓋區段，並記錄 `rangeAvailability` 與 `lastAvailableDate`，確保後續覆查能辨識快取資料最新可用日。
  - 年度快取、Superset 快取、Session 回灌與還原流程同步攜帶新欄位，避免再次把尚未取得的結束日視為已覆蓋，必要時觸發重新抓取。
- **Diagnostics**: 待於可連線 Proxy 的環境針對 2330、0050、2412 將結束日設為當日重跑回測，確認舊快取會被偵測為過期並補齊當日資料。
- **Testing**: 尚未執行（容器無法連線 Proxy，需於 Netlify 實機驗證 console 無錯誤）。
