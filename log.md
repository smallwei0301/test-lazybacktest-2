...
# 2026-07-30 — Patch LB-UI-TABS-20260730A
- **Issue recap**: 選單標籤在切換分頁時會同步縮小，摘要卡片文字語氣過於戲謔，與專業使用情境不符。
- **Fix**: `js/backtest.js` 僅針對使用者點擊的分頁調整字級與強調色，其他分頁維持原始大小；`index.html` 的回測摘要卡更新為精簡、口語的專業說明，移除俏皮用語並強調操作引導。
- **Testing**: 受限於容器環境無法連線資料來源，尚未啟動瀏覽器進行互動測試，後續將於本地瀏覽器實測回測流程並確認分頁樣式行為。
