## 2025-12-05 — Patch LB-ML-LSTM-20250625A
- **Scope**: 依據 arXiv:2505.05325 更新預測分頁，改以多維技術/價量特徵與可調式 LSTM（序列長度、隱藏單元、Dropout、批次大小）作為隔日方向預測主力模型。
- **Implementation**: 以 Adam + Dropout 訓練原生 LSTM，採迷你批次梯度、序列化標準化與新交易指標，並重新繪製門檻/超參數/命中率/累積報酬卡片。
...
## 2025-12-12 — Patch LB-ML-LSTM-20251212A
- **Issue recap**: 使用者需以單一滑桿設定訓練/測試切分並以隔日開盤到收盤的日內報酬滾動資金；舊版仍以兩個輸入欄位控制日數、採收盤對收盤報酬且缺乏自訂門檻。
