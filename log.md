...
## 2026-08-17 — Patch LB-EXIT-COMPAT-20260817A
- **Issue recap**: 出場策略選單改成註冊 ID 後，均線死亡交叉、KD 死亡交叉、MACD 死亡交叉等策略在回測與優化流程中無法觸發訊號，且自動帶入做空策略的對應表也失效。
- **Fix**:
  - `js/worker.js` 長線出場流程同時支援舊 ID 與註冊 ID，並在策略指標建置器加入新 ID 的別名，確保插件與回測判斷能產生死亡交叉訊號。
  - `js/config.js` 的長線出場對應表補齊 `*_exit` 新 ID，讓啟用做空時仍能自動帶入對應的空單進場策略。
  - `js/loader.js` 參數複製工具補上 `internalKey` 定義，避免在處理新 ID 時因未宣告變數而中止。
- **Diagnostics**: 需於具備 Proxy 的環境實際執行一次手動回測、滾動測試、批量優化與參數優化，確認死亡交叉策略能成功觸發出場訊號並檢查 console 無錯誤。
- **Testing**: `npm run typecheck`（靜態型別檢查確保改動未破壞既有定義）。
