...
## 2026-07-30 — Patch LB-SENSITIVITY-ANNUAL-SCORE-20250717A
- **Issue recap**: 年化敏感度改用 6/12pp 漂移門檻後，總分仍以「扣分 30 內算穩定」的線性公式計算，導致漂移僅 6pp 也會拿到 94 分以上，與新版漂移等級不匹配。
- **Fix**:
  - `js/worker.js` 導入年化漂移分段懲罰，將 ≤6pp 限制在 12 分內、6～12pp 線性放大至 30 分、12～18pp 漸進到 45 分，超過 18pp 依超標幅度追加懲罰並封頂 60 分，同步更新穩定度版本碼。
  - `js/backtest.js` 的穩定度提示改寫為「漂移懲罰」分段說明，讓前端提示與新的總分公式一致。
- **Diagnostics**: 待可連線 Proxy 的環境重新跑敏感度測試，確認漂移約 6pp 的策略總分落在 80~90 區間、12pp 上下接近 60 分並附帶更新後的漂移扣分明細。
