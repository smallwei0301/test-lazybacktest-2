...
- **Issue recap**: 第二個視窗起的 Walk-Forward 測試期沿用訓練期最佳參數時，發現與同區間手動批量優化的結果不同，推測為訓練區間覆寫時仍殘留「最近 N 年」等日期捷徑，導致優化引擎落在錯誤期間。
- **Fix**:
  - `js/rolling-test.js` 新增視窗覆寫工具，統一在訓練視窗套用開始/結束日並移除 `recentYears` 等日期捷徑，同時在訓練期與 Worker 輸入皆套用此處理以鎖定正確樣本。
  - `js/batch-optimization.js` 於所有批量優化與回測入口清除日期捷徑欄位，確保 `runCombinationOptimization` 與單參數掃描都僅使用 Walk-Forward 設定的絕對日期範圍。
- **Diagnostics**: 以兩個以上視窗重跑滾動測試，並於同區間手動執行批量優化比對進出場與風控參數，確認每輪訓練期皆回傳一致的最佳解；同時檢視報告摘要確保版本碼更新。
- **Testing**: `node - <<'NODE' const fs=require('fs');const vm=require('vm');['js/rolling-test.js','js/batch-optimization.js'].forEach((file)=>{const code=fs.readFileSync(file,'utf8');new vm.Script(code,{filename:file});});console.log('scripts compile');NODE`
