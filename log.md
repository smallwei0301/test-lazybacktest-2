...
## 2026-02-15 — Patch LB-UI-SENSITIVITY-20260215A
- **Scope**: 前端品牌識別、Tooltip 樣式與敏感度評分。
- **Updates**:
  - 將頁首與頁尾 LOGO 改為共用版型，並調整 LazyBacktest 字樣配色，維持品牌一致性。
  - 重整 Tooltip 為黑色空心「i」圓標記，同步調整卡片內圖示繪製方式，讓圖示繼承文字顏色。
  - 敏感度分析改以年化報酬差異計算漂移與分數，Tooltip 與說明文字同步更新為年化口徑。
  - 修補績效卡片資料呈現與策略戰報文字，改為提供具體檢視與調整建議。
  - 整理交易紀錄資料來源，確保所有進出場策略皆顯示前/當/後指標數值。
- **Testing**: 受限於容器無法連線回測 Proxy，已針對 HTML/CSS/JS 進行靜態檢閱與程式碼檢查，預計於 Netlify 預覽站補做實測。
