...
- **Issue recap**: 使用者回報 OOS 品質仍可能在多數指標不達標時顯示高分，Walk-Forward 判定卡片位置不直覺，逐窗表格閱讀困難且最小交易數門檻與實際評分不符。
- **Fix**:
  - `js/rolling-test.js` 將 OOS 品質改為「原始加權 × 達標權重比」，新增中位數統計（各指標得分、樣本、MinTRL）並透過儀表卡摺疊提供完整計算細節，同時更新總分卡與判定卡邏輯。
  - `index.html` 將 Walk-Forward 判定卡片移至儀表板前方僅顯示等級，新增摺疊摘要，並移除「至少成交筆數」欄位；逐窗報表改為橫向視窗 × 指標的轉置呈現。
  - `js/rolling-test.js` 調整逐窗報表生成流程與樣本/MinTRL 標記、動態表頭與中位數欄位，並更新模組版號為 `LB-ROLLING-TEST-20251028A`。
  - `README.md` 改寫 Walk-Forward 章節說明最新加權方式、UI 佈局與表格調整。
