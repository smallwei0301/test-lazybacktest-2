...
## 2026-09-18 — Patch LB-BATCH-MAPREFILL-20260918A
- **Issue recap**: 批量優化仍回報「缺少所有策略映射」，主因是清單仍採手動陣列，沒有隨註冊 ID 更新死亡交叉與短線策略，導致生成的複選框缺項。
- **Fix**:
  - `js/batch-optimization.js` 新增 `collectBatchStrategyGroups`，依 `strategyDescriptions` 自動分類多空進出場與回補策略，並在 `generateStrategyOptions` 直接套用結果，同步寫入 `LazyBatchStrategyGroups` 與偵錯事件 `strategy-options-refresh`/`strategy-options-missing` 方便追蹤。
  - `tests/batch-groups.test.js` 建立批量策略分組的 TDD 覆蓋，驗證死亡交叉、短線與回補策略皆落在正確分類且無重複，`package.json` 將新測試併入 `npm test` 流程。
- **Diagnostics**: 重新載入批量優化分頁並建立偵錯會話，確認日誌包含 `strategy-options-refresh` 且 `entryCount`、`exitCount` 與註冊策略一致；在 UI 勾選死亡交叉出場後啟動批量優化，應不再出現缺少映射的錯誤提示。
- **Testing**: `npm test`、`npm run typecheck`
