...
## 2025-12-09 — Patch LB-FORECAST-LSTMGA-20251116A
- **Issue recap**: GA 誤差校正未將均方誤差納入適應函數，難以穩定收斂出改善後的 δ，前端也無法顯示訓練基準與校正差異。
- **Fix**: 以 `1 / (1 + MSE)` 作為 GA 適應值並保留菁英策略與動態突變，讓 δ 演化直接朝降低 MSE 的方向前進；同步回傳訓練前後 MSE 與適應度供 UI 顯示。
- **Diagnostics**: 本地重複執行預測流程比對 `校正閾值` 訊息可看到訓練 MSE 由基準降至校正值，命中率與 RMSE 會隨 δ 調整而改善。
- **Testing**: `node - <<'NODE' const fs=require('fs');const vm=require('vm');['js/backtest.js','js/forecast.js'].forEach((file)=>{const code=fs.readFileSync(file,'utf8');new vm.Script(code,{filename:file});});console.log('scripts compile');NODE`
