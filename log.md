...
## 2025-09-27 — Patch LB-ROLLING-TEST-20250927A
- **Scope**: Walk-Forward 批量優化覆寫一致性與摘要標籤調整。
- **Updates**:
  - 新增訓練視窗覆寫工具，於每個視窗優化前套用與批量優化相同的日期、資金與交易設定，確保訓練集最佳參數與單獨批量優化結果一致。
  - 滾動測試摘要恢復顯示「隔日開盤」「總資金買入」等字樣，對齊主回測與批量優化頁面用語，避免誤判參數不一致。
