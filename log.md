# 2025-07-24 — Patch LB-UI-DEVTOGGLE-20250724B
- **Issue recap**: 左側面板持續占有開發者專用卡片，與主要操作流程混在一起；同時首頁 Hero 區缺乏一鍵開啟開發者資訊的入口，難以在不影響一般使用者的情況下檢視診斷工具。
- **Fix**: 將開發者區域卡片移至 Hero 區「一鍵回測台積電」按鈕下方，預設隱藏並提供「開發者區域」切換按鈕控制顯示，確保卡片不再佔用左右主版面。
- **Diagnostics**: 逐頁檢視 DOM，確認左側面板僅保留使用者設定卡片，Hero 區按鈕列新增切換控制並能展開/收合卡片；同時驗證 Lucide icon 重新渲染後圖示維持正確。
- **Testing**: 受限於容器無法連線回測代理，未執行實際回測；後續將在 Netlify 預覽站檢查切換互動與 console 狀態。
