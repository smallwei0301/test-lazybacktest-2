## 2026-09-11 — Patch LB-BATCH-VOLUME-20240911A
- **Scope**: 批量優化策略 DSL 複製修復與成交量暴增載入流程。
- **Updates**:
  - `js/worker.js` 引入 `clonePlainObject`/`cloneStageDefinition`，在單參數優化時為 `strategyDsl`、進出場參數與階段定義建立遞迴複本，避免 sweep 期間因共用參考導致指標失效或最佳解重複。
  - `js/batch-optimization.js` 將 `volume_spike` 與 `volume_spike_exit` 對應到最新出場選項，確保載入批量結果時能定位正確選單並觸發回測。
  - `js/config.js` 延伸 `longExitToShortMap`，納入 `volume_spike_exit` 映射，維持短空自動帶入邏輯的一致性。
