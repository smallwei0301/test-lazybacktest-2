## 2026-09-09 — Patch LB-VOLUME-MODULAR-20240909A
- **Scope**: 成交量暴增策略積木化與做空／回補支援。
- **Updates**:
  - `js/strategy-plugins/volume.js` 將 `volume_spike` 拆分為多空進出場四個插件，依角色載入對應均量並保留倍數診斷資訊。
  - `js/config.js`、`index.html`、`js/main.js`、`js/strategy-plugin-manifest.js` 新增 `volume_spike_exit`、`short_volume_spike`、`cover_volume_spike` 策略 ID 與選項，納入策略描述與驗證樣本。
  - `js/worker.js` 引入倍數正規化與回退工具，統一透過插件與回退判斷多空進出場訊號，並於短線／回補流程回傳成交量診斷；`js/batch-optimization.js` 採用新出場 ID。
