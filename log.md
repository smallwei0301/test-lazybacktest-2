...
- **Issue recap**: 插件雖已拆分，但註冊流程缺乏唯一性驗證，核心亦無法列舉插件清單或依 meta 取得標籤，無法支援 UI 動態載入與診斷。
- **Fix**:
  - `js/strategy-plugin-registry.js` 新增 `registerStrategy`／`getStrategyById`／`listStrategies` API，對 `meta.id` 去重並驗證 `paramsSchema` 結構，維持舊介面別名。
  - 各原子插件改以新 API 註冊，保留舊 `register` 回退；`js/worker.js` 對接 `resolveStrategy` 取得插件並以 `meta.label` 呈現診斷名稱。
- **Diagnostics**: 待於 Netlify 實測 `listStrategies` 動態產生策略下拉選單並隨機執行回測，確認未註冊策略不會出現且 console 仍無錯誤。
- **Testing**: `npm run typecheck`、`node scripts/check-strategy-registry.js`（模擬 Worker 載入插件後列舉清單並抽樣呼叫 `run`；詳見 PR 測試紀錄）。
