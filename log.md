...
## 2025-10-07 — Patch LB-BATCH-OPT-20251007A
- **Scope**: 批量優化載入流程與欄位映射一致性修正。
- **Updates**:
  - `js/batch-optimization.js` 禁止出場策略回退至預設值，改為提示缺少選項映射並中止載入，確保策略對應需明確補齊。
  - 將批量策略參數更新改為表驅動欄位映射，缺少欄位即提示錯誤，不再以 if/else 散落處理。
  - 載入策略時同步設定進場、出場與風險管理參數，套用後再派發一次變更事件並觸發 DOM 對拍驗證，立即檢查與結果列是否一致。
  - 整併重複的進度顯示函式、補充 `batchOptimization:loaded` 事件發佈與版本代碼 `LB-BATCH-OPT-20251007A`。
- **Testing**: 需於瀏覽器環境載入批量優化結果並確認 console 無錯誤（容器環境僅完成靜態檢查）。
