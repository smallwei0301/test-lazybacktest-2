...
## 2026-11-20 — Patch LB-ROLLING-TEST-20251109B / LB-STRATEGY-COMPARE-20260710C
- **Issue recap**: 嚴格模式仍以年化 Sharpe=1 當門檻，導致日頻資料的基準偏高；PSR/DSR 卡片缺乏樣本資訊難以判讀，策略比較表也同時列出達標率與摘要文字，造成使用者閱讀負擔。
- **Fix**:
  - `js/rolling-test.js` 將嚴格模式 SR* 改為 `1/√252` 的日頻門檻，統一傳遞至彙總與逐窗分析；PSR/DSR 卡片補上 SR_hat、SR*、n_eff、γ3、γ4、MinTRL 等欄位，並在 PSR≥95% 視窗比為 0% 時提示「拉長測試區間」。
  - `index.html` 在嚴格模式切換與等級卡片補上「日 Sharpe ≈ 0.063」說明與提示文字，強化 UI 對新門檻的說明。
  - `js/backtest.js` 調整策略比較表的滾動測試欄位僅呈現總分，避免與主報告重複的細節干擾判讀。
- **Testing**: 容器環境僅以 `node` 編譯檢查現有腳本，無法連線 Proxy 實跑滾動測試；待上線前於瀏覽器實測嚴格模式及策略比較頁確認無 console error。
