## 2026-09-02 — Patch LB-VOLUME-PARAM-20240902A
- **Scope**: 成交量暴增策略倍數／週期參數失效。
- **Updates**:
  - `js/worker.js` 以角色區分成交量均線陣列，支援出場與（預留）回補角色使用獨立週期與倍數判斷，修正所有回退路徑引用。
  - `js/strategy-plugins/volume.js` 依策略角色讀取對應的成交量均線指標，確保插件倍數參數直接作用於出場條件。
