...
## 2026-02-26 — Patch LB-ROLLING-TEST-20250927A / LB-BACKTEST-PARAM-20250927A
- **Issue recap**: Walk-Forward 視窗訓練期仍未複製批量優化的最佳參數，原因在於滾動測試啟動時改以目前 UI 內容擷取參數，導致與最近一次實際回測使用的交易設定（如隔日買入、部位基準、分批比例）產生落差。
- **Fix**:
  - `js/backtest.js` 建立 `cloneExecutedParams`/`setLastExecutedBacktestParams`，於回測送往 Worker 前將完整參數快照寫入 `window.lazybacktestLastExecutedParams`，並在重置設定時清除，版本標記為 `LB-BACKTEST-PARAM-20250927A`。
  - `js/rolling-test.js` 新增 `resolveRollingBaseParams`，優先採用 `lazybacktestLastExecutedParams` 建立滾動測試的基礎參數與視窗範圍，確保訓練期與批量優化共用相同交易設定，模組版本更新為 `LB-ROLLING-TEST-20250927A`。
- **Diagnostics**: 於主畫面以隔日買入＋總資金分批設定完成一次回測後，立即執行單視窗批量優化並比對滾動測試輸出的視窗參數摘要，確認交易時點與部位基準與批量優化結果完全一致；重置設定後再次執行，確認滾動測試提示需先建立快取。
