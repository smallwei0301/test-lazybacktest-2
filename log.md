...
## 2026-07-13 — Patch LB-STRATEGY-COMPARE-20260713A
- **Scope**: 策略比較分頁與資金術語調整。
- **Updates**:
  - 將「初始本金」改為「初始本金-固定金額買入」，「總資金」改為「總資金-獲利再投入」，同步更新提示說明與相關指標文字。
  - 新增「策略比較」分頁，可多選儲存策略並依年化報酬率、夏普值、最大回撤、交易次數等指標生成比較表，並預留敏感度、滾動測試、趨勢評估等擴充選項。
  - 擴充策略儲存內容，紀錄敏感度穩定度、滾動測試評分與最新趨勢區間表現，供比較表與後續分析使用。
- **Testing**: 尚未執行（需於瀏覽器啟動回測流程驗證新分頁互動）。
