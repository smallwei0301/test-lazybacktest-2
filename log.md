## 2025-09-23 — Patch LB-AI-LSTM-20250923A
- **Scope**: AI 預測分頁資金控管與評估機制升級。
- **Improvements**:
  - 將凱利公式開關改為即時重算，不需重新訓練即可切換，並導入勝率門檻調整與 50%~100% 一鍵最佳報酬掃描。
  - 交易結果改以「交易報酬%」呈現，指標統計改採報酬中位數、平均報酬% 與標準差；新增隔日預測摘要與回測末日對齊邏輯。
  - 建立亂數種子管理介面，可儲存至 localStorage、批次載入多組種子並記錄預設名稱（訓練勝率／測試正確率）。
- **UI**: 更新表格欄位、勝率門檻與種子面板、卡片說明與統計文案，使設定與結果更符合實務解讀。
- **Testing**: 待瀏覽器環境連線後執行回測驗證（本地容器僅完成程式碼靜態檢查）。
