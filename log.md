## 2026-09-20 — Patch LB-VOLUME-SPIKE-MODULAR-20240915A
- **Scope**: 成交量暴增策略模組化，支援做多出場與空單腳色。
- **Updates**:
  - `js/strategy-plugins/volume.js` 改為一次註冊做多進場、做多出場、做空進場與回補四個插件 ID，並於診斷中揭露倍率、期間與版本碼。
  - `js/strategy-plugin-manifest.js`、`js/config.js`、`js/main.js` 補齊 `volume_spike_exit`、`short_volume_spike`、`cover_volume_spike` 設定與預設參數，更新策略 ID 對映表。
