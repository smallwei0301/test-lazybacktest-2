# 2025-06-23 — Patch LB-USERGUIDE-20250623A
- **Issue recap**: 官方網站缺少統整操作教學，使用者無從得知完整的策略設定、暖身診斷與優化流程，Footer 也未提供有效導覽連結。
- **Fix**: 新增 `tutorial.html` 教學頁面，涵蓋系統架構、基礎設定、策略與風控、資料診斷、參數優化與常見問題；Footer 加入「更多功能」導覽群組並連結至教學、優化與診斷資源；README 新增教學索引。
- **Diagnostics**: 教學頁面附上版本碼、目錄與錨點連結，可供營運端追蹤回報資訊；Footer 連結對應實際頁面/區塊避免死鏈。
- **Testing**: 文檔與靜態頁面調整，未涉及程式執行流程。
