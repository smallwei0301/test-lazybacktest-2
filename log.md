...
## 2026-09-17 — Patch LB-BATCH-DEATHCROSS-20260917C
- **Issue recap**: 批量優化在死亡交叉出場策略下仍出現目標值為 0，研判為進場策略被誤套用死亡交叉映射，使 Worker 失去有效進場訊號。
- **Fix**:
  - `js/batch-optimization.js` 的 `getWorkerStrategyName` 與 `resolveWorkerStrategyName` 改為角色感知，僅在出場/回補角色時轉換舊 ID，並新增死亡交叉異常偵錯記錄 `deathcross-metric-anomaly` 與 `strategy-worker-normalised`。
  - `js/batch-optimization.js` 於迭代優化完成時針對死亡交叉出場輸出警示日誌，協助追蹤仍為 0 的案例。
  - `tests/batch-worker-strategy.test.mjs` 建立最小化測試環境驗證進場維持原 ID、出場能轉換舊 ID，`package.json` 新增 `npm test` 指令整合 Node Test Runner。
- **Diagnostics**: 建議觀察批量偵錯面板的 `deathcross-metric-anomaly` 紀錄，並於 Netlify 實際執行含死亡交叉的批量優化確認最終指標不再歸零。
- **Testing**: `npm test`
