...
## 2026-09-17 — Patch LB-BATCH-EXITSELECT-20260917A
- **Issue recap**: 批量優化在載入採用新出場策略 ID（如 `ma_cross_exit`、`macd_cross_exit`、`k_d_cross_exit`）的結果時，對照表仍回填舊選單值，導致找不到對應選項而拋錯。
- **Fix**:
  - `js/batch-optimization.js` 更新出場策略選單映射，讓新舊 ID 都能回填到最新選單值，並同步標記調整版本碼 `LB-BATCH-EXITSELECT-20260917A`。
- **Diagnostics**: 待於批量優化面板匯入含死亡交叉出場策略的回測結果，確認欄位能正常帶入且不再顯示缺少選項的錯誤訊息。
- **Testing**: `npm run typecheck`（靜態檢查通過）。
