...
## 2026-10-24 — Patch LB-ROLLING-TEST-20251024A
- **Issue recap**: Walk-Forward 評分在多數指標未達門檻時仍可能顯示滿分，總分也會因 WFE 調整超過 100 分，且即使使用者已完成主回測仍持續顯示「請先執行一次主回測」。
- **Fix**:
  - `js/rolling-test.js` 將 OOS 品質改為同時追蹤指標達標比率並與加權分數取小值，僅在達到設定門檻時才有機會拿到高分，並統一卡片說明語彙為「合格／未達標」風格。
  - `js/rolling-test.js` 將 Walk-Forward 總分截斷於 0～100 分，更新評分卡片文字與品質判斷門檻，同時補強快取回填流程，支援從 `lastOverallResult` 取用最新回測列避免誤判未回測。
  - `README.md` 更新補丁章節與說明，註記新版 `LB-ROLLING-TEST-20251024A` 的品質檢核與分數截斷邏輯。
- **Diagnostics**:
  - 以僅達標部分指標的測試資料確認 OOS 品質會被達標比率壓制，卡片顯示「多數指標未達門檻」提醒。
  - 模擬高於 1.0 的 WFE 調整，驗證總分顯示最多 100 分且評級仍依 0.70、0.50 判斷。
  - 在主回測執行後立即切換到滾動測試，確認快取回填可隱藏「請先執行回測」提示並顯示正確資料範圍。
- **Testing**: `node - <<'NODE' const fs=require('fs');const vm=require('vm');['js/rolling-test.js','js/worker.js'].forEach((file)=>{const code=fs.readFileSync(file,'utf8');new vm.Script(code,{filename:file});});console.log('scripts compile');NODE`
