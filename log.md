...
## 2026-09-18 — Patch LB-BATCH-CATALOG-20260918B
- **Issue recap**: 批量優化的策略選擇清單在新版映射後消失，死亡交叉策略仍偶爾回傳 0 目標值且缺乏統一的 ID 解析紀錄，開發者無法追蹤對照過程。
- **Fix**:
  - 建立 `js/lib/batch-strategy-utils.js`，整理策略目錄並提供 `resolveWorkerStrategyId`，讓批量優化可共用角色分類與正規化邏輯。
  - `js/batch-optimization.js` 的選單改由共用目錄自動生成，並在策略轉換時記錄 trace；`getWorkerStrategyName` 同步導入新工具並保留映射回報，確保死亡交叉策略一律指向註冊 ID。
  - `tests/batch-strategy-utils.test.js` 以 TDD 驗證新工具在進出場角色上的行為，`package.json` 更新 `npm test` 串聯第三組測試；`index.html` 引入新的共用腳本。
- **Diagnostics**: 開發者模式下可檢視 `strategy-options-generated` 與 `strategy-utils-trace` 日誌，確認選單載入與 ID 正規化流程；建議再執行一次含死亡交叉策略的批量優化，驗證目標值已回復正常。
- **Testing**: `npm test`、`npm run typecheck`
