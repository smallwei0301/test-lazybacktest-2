...
## 2026-08-12 — Patch LB-STRATEGY-VERIFIER-20260812A
- **Issue recap**: 策略註冊驗證僅顯示未排序的原始識別碼，抽樣回測也固定使用 RSI 預設組合，無法覆蓋新加入的策略多空變化。
- **Fix**:
  - `js/main.js` 依策略描述表推導角色，統一以中文名稱排序顯示驗證結果並在抽樣流程中隨機挑選多空策略，將選用組合與回測摘要同步顯示於狀態列。
  - `index.html` 為開發者卡片的驗證清單加入滾動區域並更新註記，說明抽樣回測會隨機挑選策略。
- **Diagnostics**: 於開發者卡片執行「驗證清單」確認 40+ 策略依中文名稱排序列出；連續點選「抽樣回測」應看到不同策略組合且 console 無錯誤。
- **Testing**: 尚未執行（需於可連線 Proxy 的實機環境觸發抽樣回測，確認狀態訊息與回測流程正常）。
