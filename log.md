# 2025-06-29 — Patch LB-TODAY-ROLLBACK-20250629A
- **Issue recap**: 今日建議卡片改版後的訊息格式與用語與營運既有溝通流程不符，需回復至 LB-TODAY-SUGGEST-20250627A 的簡明樣式。
- **Fix**: Worker 建議模擬改回回傳單一指示字串，前端卡片依字串判斷色系並重新顯示「等待」等標籤，整體邏輯退回 20250627A。
- **Diagnostics**: 卡片仍會在計算失敗時標示錯誤並保留快取診斷，後續若需再開啟精細訊息，可透過版本碼追溯復原節點。
- **Testing**: 受限於容器無法啟動瀏覽器，暫未進行實際回測；部署後須以 2330、0050 等案例確認卡片文字與顏色恢復預期。

# 2025-06-29 — Patch LB-RANDOM-GUARD-20250629A
- **Issue recap**: 隨機參數按鈕在策略清單尚未初始化或資料結構異常時，可能選到 `undefined` 造成進場或出場選單變成空白，使用者會看到「沒有進場/出場策略」。
- **Fix**: 強化 `randomizeSettings` 的候選檢查與後援流程，沿用現有選項或選單預設值做為 fallback，若仍找不到合法策略則中止並提示錯誤，同時保留原有的參數亂數邏輯。
- **Diagnostics**: 新增 console 警告標示何種策略使用後援，協助營運端確認是否因配置異常導致候選清單為空。
- **Testing**: 受限於容器無法啟動瀏覽器，暫未能於實際 UI 驗證；部署後需在 Netlify 版面實測隨機策略按鈕並檢查 console。

# 2025-06-27 — Patch LB-TODAY-SUGGEST-20250627A
- **Issue recap**: 今日建議每次評估皆重新呼叫代理抓取整段資料，既有的主執行緒快取與 Worker 記憶體資料未被重用，熱門標的會重複排隊等待 API 回應。
- **Fix**: 主執行緒在送出今日建議請求時攜帶本地快取資料與 Meta，Worker 先灌入月度／日線快取再判斷缺口，只針對暖身不足或今日尚未覆蓋的區段補抓並合併執行策略。
- **Diagnostics**: Worker 若僅取得前一交易日資料會於 console 警示，快取補灌後下次建議可直接命中記憶體資料，維運端可透過日誌確認月度 coverage 是否同步更新。
- **Testing**: 受限於容器離線，未能進行實際 API 回測；部署後需以 2330、0050 等案例確認 API 請求量下降與建議輸出一致。

# 2025-06-26 — Patch LB-TODAY-SUGGEST-20250626A
- **Issue recap**: 今日建議獨立回測仍沿用強制於最後一日平倉的規則，若當日無觸發訊號就會誤判為已出場，使用者看不到實際持倉狀態。
- **Fix**: `runStrategy` 新增 `forceFinalLiquidation` 參數，維持一般回測預設強制平倉，但在今日建議流程關閉此行為，改以市值評估持倉並回傳真實部位。
- **Diagnostics**: 卡片訊息改以最後交易日的 `positionStates` 與訊號索引判斷，營運端檢視測試卡可直接比對是否仍為多／空單持有。
- **Testing**: 受限於容器無法連線代理，暫未能跑實際回測；待部署至 Netlify 後需以 2330/0050 等案例驗證建議訊息。

# 2025-06-25 — Patch LB-TODAY-SUGGEST-20250625A
- **Issue recap**: 今日建議仍依賴使用者設定的回測終點，若策略回測在先前日期結束，卡片就無法反映今日或最近交易日的持倉狀態與訊號。
- **Fix**: Worker 於 `getSuggestion` 流程獨立抓取暖身起點至今日的資料並重新跑策略，產出最新交易日的倉位與進出場訊號；前端移除「快取不足」終止條件，讓建議計算能主動補齊資料。
- **Diagnostics**: 若策略起始日晚於今日會回傳明確提示，正常情況下建議訊息會標示最新日期、倉位與收盤價以供營運端交叉驗證。
- **Testing**: 受限於容器無法連線外部代理，僅完成程式碼檢閱；待部署至 Netlify 後需實際執行回測驗證。
...
# 2025-06-28 — Patch LB-TODAY-CACHE-20250628A
- **Issue recap**: 重複執行今日建議或重新跑回測時，即使股票代號與起訖日未變更仍會觸發遠端抓取，造成同一資料被多次請求、延長等待時間。
- **Fix**: 主執行緒以 `userStartDate/userEndDate` 標記快取來源，只要核心區間未變且已有本地資料便直接沿用，並將該限制同步傳入 Worker；今日建議流程在核心區間一致時停用暖身與尾端補抓，改以快取資料推算最新持倉。
- **Diagnostics**: 快取項目與 Worker Meta 新增 `userStartDate/userEndDate/market`，建議流程若因僅有歷史資料而無法補齊今日，會輸出警告但仍回傳最新可用倉位；若本地無資料而禁止遠端抓取則回傳友善錯誤提示。
- **Testing**: 受限於容器無法連線代理，未能進行實際 API 回測；待部署後需以 2330、0050 等案例驗證今日建議僅依賴快取且 console 無額外抓取紀錄。
