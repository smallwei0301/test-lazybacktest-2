...
- **Issue recap**: 批量優化與交叉載入沿用 UI 既有的策略 DSL，導致進場／出場組合改為其他策略後仍評估舊插件，造成均線交叉等組合在參數掃描時得到相同績效；另外 `volume_spike_exit` 缺少載入對照，導致載入結果時無法選擇對應出場策略。
- **Fix**:
  - `js/batch-optimization.js` 新增 `synchroniseStrategyDslWithParams`，於複製、補暖身與派送至 worker 前依當前進出場設定重建 DSL，確保批量與交叉優化都能更新組合策略節點。
  - `js/batch-optimization.js` 的 `enrichParamsWithLookback`、`prepareBaseParamsForOptimization` 及出口映射同步導入新工具，並補上 `volume_spike_exit` 的選單鍵，讓載入流程能填入參數並觸發回測。
- **Diagnostics**: 需於批量優化面板測試均線交叉、RSI 等組合，確認不同步進值會得到不同績效，同時載入含 `volume_spike_exit` 的結果可順利填入選單並啟動回測。
- **Testing**: `npm run test`
