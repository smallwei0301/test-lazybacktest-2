...
- **Diagnostics**: Walk-Forward 無法計算時右側會顯示「資料不足」等狀態卡片，揭露交易日、涵蓋月數並提供延長區間建議；初次開啟則提示先執行回測，確保分頁不再是空白。
- **Testing**: `node - <<'NODE' const fs=require('fs');const vm=require('vm');const code=fs.readFileSync('js/backtest.js','utf8');const elements=new Map();const makeEl=(id)=>({id,innerHTML:'',textContent:'',classList:{classes:new Set(),add(c){this.classes.add(c);},remove(c){this.classes.delete(c);},contains(c){return this.classes.has(c);}},style:{},querySelectorAll(){return[];},querySelector(){return null;}});const document={addEventListener(){},getElementById(id){if(!elements.has(id)){elements.set(id,makeEl(id));}return elements.get(id);},querySelectorAll(){return[];},querySelector(){return null;},body:{classList:{add(){},remove(){}}}};elements.set('rolling-test-placeholder',makeEl('rolling-test-placeholder'));elements.set('rolling-test-report',makeEl('rolling-test-report'));const context={console,document,window:{sessionStorage:{getItem(){return null;},setItem(){},removeItem(){}},localStorage:{getItem(){return null;},setItem(){},removeItem(){}},addEventListener(){},document,Chart:{helpers:{getRelativePosition(){return{x:0,y:0};}},registry:{plugins:{items:{}}}},fetch:async()=>({ok:true,json:async()=>({}),text:async()=>''})},Chart:{helpers:{getRelativePosition(){return{x:0,y:0};}},registry:{plugins:{items:{}}}},fetch:async()=>({ok:true,json:async()=>({}),text:async()=>''}),setTimeout,clearTimeout,setInterval,clearInterval};context.self=context.window=context.window||context;context.globalThis=context;vm.runInNewContext(code,context,{filename:'js/backtest.js'});const render=context.renderRollingTestReport;render({dates:[],strategyReturns:[],initialCapital:100000});console.log('empty status renders card',elements.get('rolling-test-report').innerHTML.includes('尚未執行回測'));render({dates:['2020-01-02','2020-01-03'],strategyReturns:['0','0'],initialCapital:100000});console.log('insufficient renders card',elements.get('rolling-test-report').innerHTML.includes('資料不足'));NODE`
