...
## 2026-08-20 — Patch LB-DATASOURCE-TABLE-20260820A
- **Scope**: 開發者區資料來源測試面板的價量檢視能力。
- **Updates**:
  - `index.html` 在資料來源測試結果下方新增「查看資料表格」按鈕、價量表格與提示文字容器，支援切換與顯示欄位標題。
  - `js/main.js` 整合資料正規化工具與表格狀態管理，依測試來源整理日期、開高低收量欄位，提供切換按鈕、提示文字與空資料提醒。
- **Testing**: 未執行（容器無法啟動瀏覽器／Proxy，需於 Netlify 實機確認按鈕切換與資料解析）。
